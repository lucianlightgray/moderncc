...
- **env flags** (≈5–7) — Who: `MCC_AST_REPLAY`/`_DUMP`/`_PROMOTE`/`_NO_CALLFUL`/`_INLINE`/`_TEMPLATES`; all **opt-in** (default off; set to any non-`0` value to open, `0` to keep closed); **`-O1`** (≈7) engages replay + Tier-3 (x86_64), inline excluded. The default-on flip was reverted after `mcctest` exposed Tier-3/Tier-4 unsoundness on the PE target and arm64.
