...
- **opt gating** (≈5–7) — Who: the replay stack is driven by the runtime `-O` level: `-O0` (the default) leaves the backend byte-untouched, **`-O1`+** engages replay + Tier-3 promotion (x86_64 only), inline/templates excluded. The former `MCC_AST_REPLAY` master env gate is gone; the sub-gates `MCC_AST_TEMPLATES`/`_PROMOTE`/`_INLINE` remain env overrides on top of the `-O` defaults (`0` closes, non-`0` opens), `_REPLAY_DUMP`/`_NO_CALLFUL` stay opt-in. The default-on flip was reverted after `mcctest` exposed Tier-3/Tier-4 unsoundness on the PE target and arm64.
