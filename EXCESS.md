...
- **Test-suite knobs:** `MCC_REF_CC`, `MCC_DIFF3_GCC`/`_CLANG`, `MCC_GCCTESTSUITE_PATH`, `MCC_ARM_CROSS_COMPILE`, `MCC_DARWIN_HOST`, `MCC_CROSS_DIR`, `MCC_WINE`, `MCC_QEMU_TESTS`/`_MIRROR`/`_ARCHS`/`_LIBCS`/`_DLDIR`/`_DOCKER_ARCHS`/`_LIBCS`.
