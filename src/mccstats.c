...
static void mccstats_build(McccRows *r) { MCC_TRACE("enter\n");
	...
	if (mcc_stats_on(MCC_STATS_JIT)) { MCC_TRACE("br\n");
		...
		if (mcs.jit_blind_variants) { MCC_TRACE("br\n");
			...
			mccstats_row(r, "          blind promote: %lu vlat-proven  %lu hard-promoted (baseline skipped)  %lu shadow-compared",
									 mcs.jit_blind_proven, mcs.jit_blind_promoted,
									 mcs.jit_blind_variants - mcs.jit_blind_promoted);
			mccstats_row(r, "          blind separability: %lu clean (never diverge)  %lu dirty (>=1 boundary miss -> restore chain)",
...
void mcc_stats_jit_blind(int n, int proven) { MCC_TRACE("enter\n");
	if (!mcs.active)
		{ MCC_TRACE("br\n"); return; }
	mcs.jit_blind_variants += (unsigned long)n;
	if (proven)
		{ MCC_TRACE("br\n"); mcs.jit_blind_proven += (unsigned long)n; }
}

void mcc_stats_jit_blind_promote(void) { MCC_TRACE("enter\n");
	if (!mcs.active)
		{ MCC_TRACE("br\n"); return; }
	mcs.jit_blind_promoted++;
}

void mcc_stats_jit_blind_dirty(void) { MCC_TRACE("enter\n");
	if (!mcs.active)
		{ MCC_TRACE("br\n"); return; }
	mcs.jit_blind_dirty++;
}
...
void mcc_stats_jit_blind_narrow_kind(int is_leaf) { MCC_TRACE("enter\n");
	if (!mcs.active)
		{ MCC_TRACE("br\n"); return; }
	if (is_leaf)
