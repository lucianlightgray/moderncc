...
enum {
	MCC_JIT_OUT_SWAPPED = 0,
	MCC_JIT_OUT_REFUSED,
	MCC_JIT_OUT_KEPT_AOT,
	MCC_JIT_OUT_BUDGET_SKIP,
	MCC_JIT_OUT_OVER_BUDGET
};
void mcc_stats_jit_outcome(int outcome);
void mcc_stats_jit_compile(unsigned ms);
void mcc_stats_jit_capture(unsigned long intent_bytes);
void mcc_stats_jit_gsearch(long cands, long admits, int budget_hit,
													 uint64_t best_mask);
void mcc_stats_jit_kgc_warm(unsigned long tuples);
void mcc_stats_jit_hot(void);
void mcc_stats_jit_prof_spec(void);
