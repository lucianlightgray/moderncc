/*
 * arm64-dis.c - AArch64 instruction disassembler for `-S` output.
 *
 * Decodes the fixed-width A64 subset that arm64-gen.c emits and prints it in
 * standard gas syntax.  The listing must re-assemble byte-identically with
 * mcc's own integrated assembler (arm64-asm.c), whose mnemonic coverage is
 ...
 *   - `mul` assembles with Ra=x0 instead of xzr (ARM64_MUL_REG lacks 0x7C00);
 *   - no fp/simd, csel/cset, sdiv/udiv, madd/msub, sxt/uxt, ldrs forms,
 *     register-offset or pre/post-indexed single-register load/stores.
 */
#include "mcc.h"

#ifdef MCC_TARGET_ARM64

/* decode() result: how the text can be emitted */
#define D_UNK 0   /* unrecognized encoding: bare .long               */
#define D_CMT 1   /* decoded, but assembler can't express: .long + comment */
#define D_ASM 2   /* assembler round-trips the text byte-identically  */
...
/* integer register, 31 = zero register */
...
/* integer register, 31 = stack pointer (base/add-imm contexts) */
static const char *irsp(int sf, int r)
{
    if ((r &= 31) == 31)
        return sf ? "sp" : "wsp"; /* wsp never emitted by the backend */
    return ir(sf, r);
}

/* fp/simd register: kind is 'b','h','s','d' or 'q' */
...
/* value -> (N<<12 | immr<<6 | imms), -1 if not encodable.  Same algorithm as
   arm64_encode_bimm64 in arm64-gen.c; used to verify that re-assembling the
   printed value reproduces the original field bits. */
static int dis_encode_bimm64(uint64_t x)
{
    ...
    pos = 0;
    if (!(x & 0xffffffff)) x >>= 32, pos += 32;
    if (!(x & 0xffff)) x >>= 16, pos += 16;
    if (!(x & 0xff)) x >>= 8, pos += 8;
    if (!(x & 0xf)) x >>= 4, pos += 4;
    if (!(x & 0x3)) x >>= 2, pos += 2;
    if (!(x & 0x1)) x >>= 1, pos += 1;
    ...
    len = 0;
    if (!(~x & 0xffffffff)) x >>= 32, len += 32;
    if (!(~x & 0xffff)) x >>= 16, len += 16;
    if (!(~x & 0xff)) x >>= 8, len += 8;
    if (!(~x & 0xf)) x >>= 4, len += 4;
    if (!(~x & 0x3)) x >>= 2, len += 2;
    if (!(~x & 0x1)) x >>= 1, len += 1;
...
/* (N, immr, imms) -> replicated bitmask value */
static uint64_t dis_decode_bimm(int n, int immr, int imms)
{
    int len, esize, i;
    uint64_t welem, pattern;

    /* highest set bit of N:NOT(imms) gives the element size */
    {
        int v = n << 6 | (~imms & 0x3f);
    ...
    /* rotate right by immr within the element */
    if (immr)
...
/* branch target: registers a local label in pass 1, returns ".L<hex>" text */
...
/* Decode the instruction word at dc->pc into out[]; returns D_ASM/D_CMT/D_UNK.
   Registers local branch targets via disasm_label() in pass 1. */
static int decode(disasm_ctx *dc, uint32_t w, char *out, size_t osz)
{
    ...
    if ((w & 0xfff00000) == 0xd5300000) { /* other mrs (tpidr_el0, ctr_el0) */
        snprintf(out, osz, "mrs\t%s, s3_%d_c%d_c%d_%d", ir(1, rd),
                 (int)(w >> 16) & 7, (int)(w >> 12) & 15,
                 (int)(w >> 8) & 15, (int)(w >> 5) & 7);
        return D_CMT;
    }
    if ((w & 0xfff00000) == 0xd5100000) { /* other msr */
    ...
    if ((w & 0x7c000000) == 0x14000000) { /* b / bl */
        ...
        if (sym && (rtype == R_AARCH64_CALL26 || rtype == R_AARCH64_JUMP26)) {
            if (!(w & 0x03ffffff)) { /* backend leaves imm26 = 0 */
        ...
        }
        {
            ...
            /* the assembler resolves numeric targets as section offsets */
    ...
    if ((w & 0xff000010) == 0x54000000) { /* b.cond */
        ...
        }
        {
            ...
            const char *l = btext(dc, target);
            /* arm64-asm.c places imm19 unshifted, so numeric/label b.cond
               does not round-trip: keep the raw word */
    ...
    if ((w & 0x7e000000) == 0x34000000) { /* cbz / cbnz */
        ...
        }
        {
            ...
            return D_CMT; /* same imm19 assembler bug as b.cond */
        }
    }
    if ((w & 0x7e000000) == 0x36000000) { /* tbz / tbnz (inline asm only) */
    ...
    /* --- move wide ------------------------------------------------------ */
    if ((w & 0x1f800000) == 0x12800000) { /* movn/movz/movk */
        ...
        if (nm && (sf || hw <= 1)) {
            ...
            return sym ? D_CMT : D_ASM; /* MOVW_UABS relocs not expressible */
        }
        return D_UNK;
    }
    ...
    if ((w & 0x1f800000) == 0x11000000) {
        ...
        if (!sf && (rd == 31 || rn == 31) && !S)
            return D_UNK; /* wsp operand: no such token */
        snprintf(out, osz, "%s\t%s, %s, #%llu", nm, d, irsp(sf, rn), val);
        return D_ASM;
    }

    /* --- add/sub shifted / extended register ----------------------------- */
    if ((w & 0x1f200000) == 0x0b000000) { /* shifted register */
        ...
        snprintf(out, osz, "%s\t%s, %s, %s, %s #%d", nm,
                 ir(sf, rd), ir(sf, rn), ir(sf, rm),
                 shift == 0 ? "lsl" : shift == 1 ? "lsr" : "asr", imm6);
        return D_CMT;
    }
    if ((w & 0x1f200000) == 0x0b200000) { /* extended register */
        int op = (w >> 30) & 1, S = (w >> 29) & 1;
    ...
    if ((w & 0x1f000000) == 0x0a000000) {
        ...
        if (!shift && !imm6) {
            if (opc == 1 && rn == 31) { /* orr rd, zr, rm == mov */
    ...
    if ((w & 0x1f800000) == 0x12000000) {
        ...
        if (!sf)
            val &= 0xffffffff;
        /* verify the assembler's encoder reproduces the same fields */
        {
    ...
    /* --- bitfield (lsl/lsr/asr immediate, sxt/uxt) ------------------------- */
        ...
        if (opc == 0) { /* SBFM */
            if (imms == nbits - 1) {
                snprintf(out, osz, "asr\t%s, %s, #%d",
                         ir(sf, rd), ir(sf, rn), immr);
                /* 32-bit asr-imm mis-assembles (arm64-asm.c sets N=1) */
    ...
    /* --- data-processing, 2-source (shifts, divide) ------------------------ */
    ...
    if ((w & 0x7fe08000) == 0x1b000000 || (w & 0x7fe08000) == 0x1b008000) {
        ...
        if (!o0 && ra == 31)
            /* arm64-asm.c encodes `mul` with Ra=x0, so keep the raw word */
    ...
    /* --- conditional select (cset & friends) ------------------------------- */
    ...
    if ((w & 0x3a000000) == 0x28000000) {
        ...
        if (enc == 0)
            return D_UNK; /* ldnp/stnp: not emitted */
        if (V) {
            kind = opcv == 0 ? 's' : opcv == 1 ? 'd' : opcv == 2 ? 'q' : 0;
            scale = opcv == 0 ? 2 : opcv == 1 ? 3 : 4;
            if (!kind)
                return D_UNK;
        } else {
            if (opcv == 0)
        ...
        /* the assembler handles x and d pairs in all three modes */
    ...
    if ((w & 0x3b000000) == 0x39000000) {
        ...
        off = (unsigned long long)((w >> 10) & 0xfff) << scale;
        {
            ...
            if (opc == 0)
                ...
                nm = ldnm[size];
            else { /* sign-extending loads: no assembler support */
                const char *snm = size == 0 ? "ldrsb" : size == 1 ? "ldrsh"
                                 : size == 2 ? "ldrsw" : 0;
                if (!snm)
                    return D_UNK;
                tsf = (opc == 2); /* opc=2: 64-bit target, opc=3: 32-bit */
                if (sym)
    ...
    /* --- load/store, unscaled / pre / post immediate --------------------------- */
    if ((w & 0x3b200000) == 0x38000000 && ((w >> 10) & 3) != 2) {
        int size = w >> 30, V = (w >> 26) & 1, opc = (w >> 22) & 3;
        int idx = (w >> 10) & 3; /* 0 unscaled, 1 post, 3 pre */
        int off = simm(w, 12, 9);
        char addr[64];
        if (idx == 0)
            snprintf(addr, sizeof addr, "[%s, #%d]", irsp(1, rn), off);
        ...
        if (V) {
            ...
            if (idx == 0) {
                /* print as ldr/str: arm64-asm.c re-derives the unscaled
                   form for offsets a scaled ldr cannot encode */
        ...
        }
        {
            ...
            if (idx == 0) {
                if (off < 0 || (off & ((1 << size) - 1))) {
                    /* ldr/str re-assembles to this exact unscaled form */
            ...
            snprintf(out, osz, "%s\t%s, %s",
                     opc ? ldnm[size] : stnm[size], ir(tsf, rd), addr);
            return D_CMT; /* pre/post-indexed: offset mode only in asm */
        }
    }
    ...
    /* --- floating point ------------------------------------------------------------ */
    if ((w & 0xff200c00) == 0x1e200800) { /* fp data-processing, 2 source */
    ...
    if ((w & 0x7fbefc00) == 0x1e220000) { /* scvtf / ucvtf (from integer) */
        int kind = (w & 0x400000) ? 'd' : 's';
        snprintf(out, osz, "%s\t%s, %s", (w & 0x10000) ? "ucvtf" : "scvtf",
                 fr(kind, rd), ir(sf, rn));
        return D_CMT;
    }
    if ((w & 0x7fbefc00) == 0x1e380000) { /* fcvtzs / fcvtzu (to integer) */
        int kind = (w & 0x400000) ? 'd' : 's';
        snprintf(out, osz, "%s\t%s, %s", (w & 0x10000) ? "fcvtzu" : "fcvtzs",
                 ir(sf, rd), fr(kind, rn));
        return D_CMT;
    }
    if ((w & 0xfffffc00) == 0x9e660000) { /* fmov x <- d */
        snprintf(out, osz, "fmov\t%s, %s", ir(1, rd), fr('d', rn));
        return D_CMT;
    }
    if ((w & 0xfffffc00) == 0x9e670000) { /* fmov d <- x */
        snprintf(out, osz, "fmov\t%s, %s", fr('d', rd), ir(1, rn));
        return D_CMT;
    }
    if ((w & 0xfffffc00) == 0x1e260000) { /* fmov w <- s */
        snprintf(out, osz, "fmov\t%s, %s", ir(0, rd), fr('s', rn));
        return D_CMT;
    }
    if ((w & 0xfffffc00) == 0x1e270000) { /* fmov s <- w */
        snprintf(out, osz, "fmov\t%s, %s", fr('s', rd), ir(0, rn));
        return D_CMT;
    }
    if ((w & 0xffeffc00) == 0x4e083c00) { /* umov x, v.d[i] */
        snprintf(out, osz, "mov\t%s, %s.d[%d]", ir(1, rd), fr('v', rn),
                 (int)(w >> 20) & 1);
        return D_CMT;
    }
    if ((w & 0xffe0fc00) == 0x4ea01c00) { /* orr v.16b (vector mov) */
        if (rn == rm)
    ...
    if ((w & 0xbfbf0000) == 0x0c000000 || (w & 0xbfbf0000) == 0x0c400000
     || (w & 0xbf9f0000) == 0x0d000000 || (w & 0xbf9f0000) == 0x0d400000) {
        /* simd multi/single structure load/store (va_arg HFA copies) */
...
ST_FUNC int mcc_disasm_insn(disasm_ctx *dc)
{
    ...
    if (dc->pc + 4 > dc->size) { /* trailing bytes: keep length correct */
        if (!dc->collect)
...
ST_FUNC int mcc_disasm_reloc_size(int type)
{
    ...
    case R_AARCH64_PREL32:
        return 4;
    /* instruction-field relocations patch one 4-byte instruction */
...
{
    /* AArch64 uses RELA everywhere: addends read back as-is. */
    (void)type; (void)size;
    return 0;
}

#endif /* MCC_TARGET_ARM64 */
