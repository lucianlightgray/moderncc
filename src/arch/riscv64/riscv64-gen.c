...
ST_FUNC void load(int r, SValue *sv) { MCC_TRACE("enter\n");
	...
	if (fr & VT_LVAL) { MCC_TRACE("br\n");
		...
		if (is_float_abi(sv->type.t) && bt != VT_LDOUBLE && bt != VT_FLOAT128) { MCC_TRACE("br\n");
			uint64_t val = sv->c.i;
		...
		assert(is_ireg(r) || bt == VT_LDOUBLE || bt == VT_FLOAT128);
...
ST_FUNC void store(int r, SValue *sv) { MCC_TRACE("enter\n");
	...
	assert(!is_float(bt) || is_freg(r) || bt == VT_LDOUBLE || bt == VT_FLOAT128 || IS_HALF_BT(bt));
	if (bt == VT_LDOUBLE || bt == VT_FLOAT128)
		{ MCC_TRACE("br\n"); size = align = 8; }
	if (bt == VT_STRUCT)
...
static void reg_pass_rec(CType *type, int *rc, int *fieldofs, int ofs) { MCC_TRACE("enter\n");
	...
	} else if (rc[0] == 2 || rc[0] < 0 || (type->t & VT_BTYPE) == VT_LDOUBLE || (type->t & VT_BTYPE) == VT_FLOAT128)
		{ MCC_TRACE("br\n"); rc[0] = -1; }
...
ST_FUNC void gfunc_call(int nb_args) { MCC_TRACE("enter\n");
	...
	for (int i = 0; i < nb_args; i++) { MCC_TRACE("br\n");
		...
		if (!(r & 32)) { MCC_TRACE("br\n");
			...
			if (loadt == VT_LDOUBLE || loadt == VT_FLOAT128) { MCC_TRACE("br\n");
				assert(r2);
				r2--;
			...
			if (r2 && loadt != VT_LDOUBLE && loadt != VT_FLOAT128) { MCC_TRACE("br\n");
				r2--;
			...
			if (info[nb_args - 1 - i] & 16) { MCC_TRACE("br\n");
				...
				vtop->r2 = VT_CONST;
			} else if ((loadt == VT_LDOUBLE || loadt == VT_FLOAT128) && vtop->r2 != r2) { MCC_TRACE("br\n");
...
static uint32_t riscv64_ubsan_skip_imm(int kind) { MCC_TRACE("enter\n");
...
static void riscv64_ubsan_div0(int b) { MCC_TRACE("enter\n");
	if (!mcc_state->do_sanitize_undefined || nocode_wanted)
		{ MCC_TRACE("br\n"); return; }
	o(0x63 | 1u << 12 | (uint32_t)b << 15 | riscv64_ubsan_skip_imm(UBK_DIVREM));
...
void gen_ubsan_nullptr(void) { MCC_TRACE("enter\n");
	...
	if ((vtop->r & VT_VALMASK) >= VT_CONST)
		{ MCC_TRACE("br\n"); return; }
	o(0x63 | 1u << 12 | (uint32_t)ireg(vtop->r & VT_VALMASK) << 15 | riscv64_ubsan_skip_imm(UBK_NULLPTR));
...
void gen_asan_shadow_check(int sz) { MCC_TRACE("enter\n");
	...
	ER(0x33, 0, 5, 5, 6, 0);
	EI(0x03, 0, 5, 5, 0);
	o(0x63 | 0u << 12 | 5u << 15 | 0u << 20 | riscv64_btype_imm(24));
	EI(0x13, 7, 6, a, 7);
...
static void riscv64_ubsan_shift(int cnt, uint32_t width) { MCC_TRACE("enter\n");
	...
	o(0x63 | 1u << 12 | 5u << 15 | riscv64_ubsan_skip_imm(UBK_SHIFT));
...
static void riscv64_ubsan_addsub_post(int op, int d) { MCC_TRACE("enter\n");
	...
	o(0x63 | 5u << 12 | 5u << 15 | riscv64_ubsan_skip_imm(op == '+' ? UBK_OVERFLOW : UBK_SUB));
...
static void riscv64_ubsan_mul_ovf(int ll, int d) { MCC_TRACE("enter\n");
	if (ll == 8) { MCC_TRACE("br\n");
		...
		o(0x63 | 0u << 12 | 7u << 15 | (uint32_t)d << 20 | riscv64_ubsan_skip_imm(UBK_MUL));
	} else { MCC_TRACE("br\n");
		o(0x33 | 1u << 12 | 7u << 7 | 5u << 15 | 6u << 20 | 1u << 25);
		o(0x13 | 5u << 12 | 5u << 7 | (uint32_t)d << 15 | 0x43fu << 20);
		o(0x63 | 0u << 12 | 7u << 15 | 5u << 20 | riscv64_ubsan_skip_imm(UBK_MUL));
	}
