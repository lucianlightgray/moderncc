...
- [ ] Replace `#ifdef CONFIG_MCC_BCHECK` → level ≥ 2 tests,
      `CONFIG_MCC_BACKTRACE` → level ≥ 1; delete the CMake dependency warning
      (`CMakeLists.txt:296`). `CONFIG_MCC_BACKTRACE_ONLY` (the amalgamation
...
- [ ] Move into the table: `CONFIG_MCC_SYSINCLUDEPATHS` (`libmcc.c:1021`),
      `CONFIG_MCC_LIBPATHS` (`:1040`), `CONFIG_MCC_CRTPREFIX` (`:1053`),
      `CONFIG_MCC_ELFINTERP(_ARMHF)` (`mcc.h:177-193` → `mccelf.c:111-114` —
      ...
      (`CONFIG_MCC_MUSL` triple string + the `{B}`/`{R}` sysroot overrides,
      keyed by a per-libc table row).
- [ ] `CONFIG_MCC_SWITCHES` (`libmcc.c:950`) folds into the same table as a
...
- [ ] `CONFIG_AST` / `MCC_AST` → the optimizer feature gate (name per item-16
...
- [ ] `CONFIG_MCC_CST` / `MCC_CST` → the LSP feature gate (working proposal
