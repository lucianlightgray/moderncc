...
if(NOT _srcs)
    message(FATAL_ERROR "spvgate-real: no corpus found under ${SRCDIR}/tests/exec")
endif()

set(_dump "${BINDIR}/spvgate_real_arenas.txt")
file(REMOVE "${_dump}")
foreach(_f IN LISTS _srcs)
    execute_process(COMMAND "${CMAKE_COMMAND}" -E env "MCC_ARENA_DUMP=${_dump}"
                            "${MCC}" -w -O2 -c "${_f}" -o "${BINDIR}/spvgate_real.o"
                    RESULT_VARIABLE _rc OUTPUT_QUIET ERROR_QUIET TIMEOUT 20)
endforeach()
if(NOT EXISTS "${_dump}")
    message(FATAL_ERROR "spvgate-real: MCC_ARENA_DUMP produced nothing; the hook "
...
if(NOT _clean EQUAL 0)
    message(FATAL_ERROR "spvgate-real: real slices disagree with the CPU replay")
endif()
if(NOT _out MATCHES "slices=([1-9][0-9]*)")
    message(FATAL_ERROR "spvgate-real: zero slices lowered; a clean result here "
...
message("${_mout}")
if(_mut EQUAL 0)
    message(FATAL_ERROR "spvgate-real: the mutated build still reported OK, so the "
                        "real-slice differential is blind")
endif()
message("spvgate-real: clean OK, mutation detected")
