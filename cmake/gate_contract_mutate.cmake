...
set(_why_stub-prover "an mcc_skip_test echo stub cannot go red, so pointing a row at one must not count as proof")
set(_why_empty "an empty manifest asserts nothing, and must not read as every gate being covered")

foreach(_m drop-row relax-floor forge-prover stub-prover empty)
    execute_process(COMMAND "${PY}" "${TOOL}"
                            "--build=${BINDIR}"
                            "--manifest=${MANIFEST}"
                            "--must-run=${MUSTRUN}"
...
message("ci/gate-contract-known-positive: clean OK, drop-row/relax-floor/"
        "forge-prover/stub-prover/empty all detected")
