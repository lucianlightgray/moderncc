...
static const cli_case_t cli_cases[] = {
		...
		{"asm_data_directives_widths", "",
		 ...
		 "printf '#include <string.h>\\nextern unsigned char base[] __asm__(\"base\");\\nextern unsigned char endp[] __asm__(\"endp\");\\nint main(void){ unsigned short h,v,t; unsigned int f; unsigned long long e,x; float ff; double dd; memcpy(&h,base+0,2); memcpy(&v,base+2,2); memcpy(&t,base+4,2); memcpy(&f,base+6,4); memcpy(&e,base+10,8); memcpy(&x,base+18,8); memcpy(&ff,base+26,4); memcpy(&dd,base+30,8); if((endp-base)!=38) return 100; if(h!=0x1234) return 101; if(v!=0x5678) return 102; if(t!=0xabcd) return 103; if(f!=0xdeadbeefU) return 104; if(e!=0x1122334455667788ULL) return 105; if(x!=0x99aabbccddeeff11ULL) return 106; if(ff!=1.5f) return 107; if(dd!=2.5) return 108; return 42; }\\n' > {W}/dd.c && "
