...
static const cli_case_t cli_cases[] = {
	...
	{"c11_keyword_feature_pedantic", "",
		...
		{"loop_unroll_dowhile", "",
		 "printf 'int main(void){int s=0,i=0;do{s+=i*i;i++;}while(i<8);int p=1,j=1;do{p*=j;j++;}while(j<5);return s+p;}\\n' > {W}/uldw.c && "
		 "{MCC} -B{B} -I{I} -O2 -funroll-loops {W}/uldw.c -o {W}/uldw && {W}/uldw; printf 'R=%d\\n' $?; "
		 "{MCC} -B{B} -I{I} -O2 -funroll-loops -c {W}/uldw.c -o {W}/uldw.on.o; "
		 "{MCC} -B{B} -I{I} -O2 -fno-unroll-loops -c {W}/uldw.c -o {W}/uldw.off.o; "
		 "cmp -s {W}/uldw.on.o {W}/uldw.off.o && echo U=same || echo U=fired",
		 "R=164\nU=fired\n"},

		{"loop_unroll_budget", "",
		 "printf 'int main(void){int s=0;for(int i=0;i<16;i++)s+=i;int p=0;for(int k=0;k<20;k++)p+=k*2;return (s+p)&255;}\\n' > {W}/ulb.c && "
		 "{MCC} -B{B} -I{I} -O2 -funroll-loops {W}/ulb.c -o {W}/ulb && {W}/ulb; printf 'R=%d\\n' $?; "
		 "{MCC} -B{B} -I{I} -O2 -funroll-loops -c {W}/ulb.c -o {W}/ulb.on.o; "
		 "{MCC} -B{B} -I{I} -O2 -fno-unroll-loops -c {W}/ulb.c -o {W}/ulb.off.o; "
		 "cmp -s {W}/ulb.on.o {W}/ulb.off.o && echo U=same || echo U=fired",
		 "R=244\nU=fired\n"},
		...
		{"loop_unroll_shapes", "",
		 ...
		 "{MCC} -B{B} -I{I} -O2 -funroll-loops -c {W}/uls.c -o {W}/uls.on.o; "
		 "{MCC} -B{B} -I{I} -O2 -fno-unroll-loops -c {W}/uls.c -o {W}/uls.off.o; "
		...
		{"loop_unroll_ne", "",
		 "printf 'int main(void){int s=0;for(int i=0;i!=5;i++)s+=i;for(int i=0;i!=10;i+=2)s+=i;for(int i=9;i!=0;i-=3)s+=i;return s;}\\n' > {W}/une.c && "
		 "{MCC} -B{B} -I{I} -O2 -funroll-loops {W}/une.c -o {W}/une && {W}/une; printf 'R=%d\\n' $?; "
		 "{MCC} -B{B} -I{I} -O2 -funroll-loops -c {W}/une.c -o {W}/une.on.o; "
		 "{MCC} -B{B} -I{I} -O2 -fno-unroll-loops -c {W}/une.c -o {W}/une.off.o; "
		...
		{"gcse_comm_ord", "",
		 "printf 'static int h(int a,int b){int t=(a<b);int u=(b>a);int v=(a<=b);int w=(b>=a);return t+u+v+w;}int main(void){return h(5,7)+h(9,3);}\\n' > {W}/gco.c && "
		 "{MCC} -B{B} -I{I} -O2 -fgcse -fgcse-comm-rel {W}/gco.c -o {W}/gco && {W}/gco; printf 'R=%d\\n' $?; "
		 "{MCC} -B{B} -I{I} -O2 -fgcse -fgcse-comm-rel -c {W}/gco.c -o {W}/gco.on.o; "
		 "{MCC} -B{B} -I{I} -O2 -fgcse -fno-gcse-comm-rel -c {W}/gco.c -o {W}/gco.off.o; "
		 "cmp -s {W}/gco.on.o {W}/gco.off.o && echo U=same || echo U=fired",
		 "R=4\nU=fired\n"},
