...
struct seg  { uint32_t cmd,cmdsize; char segname[16]; uint64_t vmaddr,vmsize,fileoff,filesize;
              uint32_t maxprot,initprot,nsects,flags; };
struct main_c { uint32_t cmd,cmdsize; uint64_t entryoff,stacksize; };
...
int main(int argc, char **argv) {
    if (argc < 2) { fprintf(stderr, "usage: %s mach-o\n", argv[0]); return 2; }
    int fd = open(argv[1], O_RDONLY);
    struct stat st; if (fd < 0 || fstat(fd, &st)) { perror("open"); return 2; }
    uint8_t *buf = mmap(0, st.st_size, PROT_READ, MAP_PRIVATE, fd, 0);
    if (buf == MAP_FAILED) { perror("mmap file"); return 2; }

    struct mh *h = (void*)buf;
    if (h->magic != 0xfeedfacf) { fprintf(stderr, "not a 64-bit Mach-O\n"); return 2; }

    uint64_t entry = 0, text_vmaddr = 0; int have_text = 0;
    uint8_t *p = buf + sizeof(struct mh);
    for (uint32_t i = 0; i < h->ncmds; i++) {
        struct lc *c = (void*)p;
        if (c->cmd == LC_SEGMENT_64) {
            struct seg *sg = (void*)p;
            if (strcmp(sg->segname, "__PAGEZERO") && strcmp(sg->segname, "__LINKEDIT")
                && sg->vmsize) {
                void *m = mmap((void*)sg->vmaddr, sg->vmsize,
                               PROT_READ|PROT_WRITE|PROT_EXEC,
                               MAP_PRIVATE|MAP_ANONYMOUS|MAP_FIXED, -1, 0);
                if (m == MAP_FAILED) { perror("mmap seg"); return 2; }
                if (sg->filesize)
                    memcpy((void*)sg->vmaddr, buf + sg->fileoff, sg->filesize);
                if (!strcmp(sg->segname, "__TEXT")) { text_vmaddr = sg->vmaddr; have_text = 1; }
            }
