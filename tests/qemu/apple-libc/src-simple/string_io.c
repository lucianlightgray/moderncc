...
int _simple_vesprintf(_SIMPLE_STRING b, _esc_func esc, const char *fmt, va_list ap) {
...
void _simple_sresize(_SIMPLE_STRING b) {
    ((BUF *)b)->ptr = ((BUF *)b)->buf + strlen(((BUF *)b)->buf);
}

int _simple_sappend(_SIMPLE_STRING b, const char *str) {
...
void _simple_put(_SIMPLE_STRING b, int fd) {
    ((BUF *)b)->fd = fd;
    _flush((BUF *)b);
}

void _simple_putline(_SIMPLE_STRING b, int fd) {
    ((BUF *)b)->fd = fd;
    *((BUF *)b)->ptr++ = '\n';
    _flush((BUF *)b);
    ((BUF *)b)->ptr--;
...
int _simple_vsnprintf(char *str, size_t size, const char *fmt, va_list ap) {
