#!/bin/sh
# §26 2B — build the freestanding arm64 JIT KGC differential-verify stub validator
# with a cross clang + lld and run it under qemu-aarch64. Skips (pass) when the
...
EXE="$WORK/jit_arm64_kgc"
if ! "$CLANG" --target=aarch64-linux-gnu -fuse-ld=lld -nostdlib -static -O1 \
		-o "$EXE" "$SRC" 2>"$WORK/jit_arm64_kgc.log"; then
	echo "SKIP: aarch64 cross-link unavailable"
	cat "$WORK/jit_arm64_kgc.log"
	exit 0
fi

if "$QEMU" "$EXE"; then
	echo "PASS: arm64 KGC differential-verify stub (gather+sxtw+verify+return) validated under qemu"
	exit 0
else
	echo "FAIL: arm64 KGC validator exited $?"
	exit 1
fi
