	...
	for (int i = 0; i < mcc_goldens_count; i++) {
		...
		if (!strcmp(g->mode, "pp")) {
			snprintf(cmd, sizeof cmd,
							 "%s \"%s\" \"-B%s\" -fno-diagnostics-show-caret -Wno-deprecated-declarations %s \"-I%s\" \"-I%s\" -E -P \"%s\" 2>&1",
							 emu, mcc, bdir, optf, idir, sup, path);
			out = run_capture(cmd, &rc);
		} else if (!strcmp(g->mode, "brun")) {

			snprintf(cmd, sizeof cmd,
							 "cd \"%s\" && %s \"%s\" \"-B%s\" -fno-diagnostics-show-caret -Wno-deprecated-declarations %s \"-I%s\" \"-I%s\" -b -run \"%s\" %s 2>&1",
							 work, emu, mcc, bdir, optf, idir, sup, path, g->flags);
			out = run_capture(cmd, &rc);
		} else if (!strcmp(g->mode, "dt")) {

			snprintf(cmd, sizeof cmd,
							 "cd \"%s\" && %s \"%s\" \"-B%s\" -fno-diagnostics-show-caret -Wno-deprecated-declarations %s \"-I%s\" \"-I%s\" -dt -run \"%s\" %s 2>&1",
							 work, emu, mcc, bdir, optf, idir, sup, path, g->flags);
			out = run_capture(cmd, &rc);
		} else if (!strcmp(g->mode, "run2")) {

			snprintf(cmd, sizeof cmd,
							 "cd \"%s\" && ( %s \"%s\" \"-B%s\" -fno-diagnostics-show-caret -Wno-deprecated-declarations %s \"-I%s\" \"-I%s\" -run \"%s\" && "
							 "%s \"%s\" \"-B%s\" -fno-diagnostics-show-caret -Wno-deprecated-declarations %s \"-I%s\" \"-I%s\" -b -run \"%s\" ) 2>&1",
			...
			snprintf(cmd, sizeof cmd,
							 "%s \"%s\" \"-B%s\" -fno-diagnostics-show-caret -Wno-deprecated-declarations %s \"-I%s\" \"-I%s\" %s\"%s\" %s -o \"%s\" 2>&1",
