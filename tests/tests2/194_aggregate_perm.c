...
typedef struct { int arr[4]; char tag; } SA;
typedef struct { int a; long double ld; int b; } SLD;
typedef struct { int a, b, c, d, e; } S5;
struct Anon { int top; union { int u; float f; }; struct { short lo, hi; }; };

static SA mutSA(SA s) { for (int i = 0; i < 4; i++) s.arr[i] *= 2; s.tag++; return s; }
static int sld_chk(SLD s) { return s.a + (int)s.ld + s.b; }
static S5 chain(S5 s) { S5 t; t = s; t.a += t.e; return t; }
