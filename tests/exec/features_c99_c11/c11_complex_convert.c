...
static double _Complex ret_real(void) { return 7.0; }
...
int main(void)
{
    ...
    if (!(__real__ g_real == 4 && __imag__ g_real == 0)) ok = 0;
    if (!(__real__ g_cmplx == 5 && __imag__ g_cmplx == 6)) ok = 0;
    if (!((double)__real__ g_cI == 0 && (double)__imag__ g_cI == 1)) ok = 0;
    if (!((double)__real__ g_int == 6 && (double)__imag__ g_int == 0)) ok = 0;
    ...
    if (!(__real__ ret_real() == 7 && __imag__ ret_real() == 0)) ok = 0;

    if (!check_arg(4.0, 4.0, 0.0)) ok = 0;
    if (!check_arg(w, 1.0, 2.0)) ok = 0;
