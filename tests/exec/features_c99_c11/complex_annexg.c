...
int main(void) {
	...
	z = one / infc;
	if (!(__real__ z == 0 && __imag__ z == 0))
		ok = 0;

	double _Complex inf0 = {inf, 0};
	z = a * inf0;
	if (!isinf_(__real__ z))
		ok = 0;
	...
	fz = fa * fb;
	if (!((float)__real__ fz == -5 && (float)__imag__ fz == 10))
		ok = 0;

	long double _Complex la = {1, 2}, lb = {3, 4}, lz;
	lz = la * lb;
	if (!((long double)__real__ lz == -5 && (long double)__imag__ lz == 10))
		ok = 0;
