...
struct pc { unsigned int base_lo, base_hi, count; };
...
int main(int argc, char **argv) {
    ...
    const char *spv = (argc > 2) ? argv[2] : "rev64.spv";
    ...
    VK(vkMapMemory(dev, mem, 0, 16, 0, (void **)&map));
    memset(map, 0, 16);
    ...
    dbi.buffer = buf; dbi.offset = 0; dbi.range = 16;
    ...
    printf("device: %s\n", props.deviceName);
    printf("dispatch: %d groups x %d threads, chunk=%u, total=%llu\n",
           NGROUPS, WGSIZE, CHUNK, total);
    ...
    while (done < total) {
        ...
        p.base_lo = (unsigned int)(done & 0xFFFFFFFFu);
        p.base_hi = (unsigned int)(done >> 32);
    ...
    res[0] = map[0]; res[1] = map[1]; res[2] = map[2]; res[3] = map[3];

    printf("swept=%llu wall=%.3fs rate=%.1f M/s\n",
           done, t1 - t0, (double)done / (t1 - t0) / 1e6);
    printf("xsum=%08x%08x bad_involution=%u bad_popcount=%u\n",
