...
echo "== Part 1: AOT -O13 search (JIT off) engages ~14s with const-guided ranges =="
t0=$(date +%s.%N)
out1=$(env $CACHE_ISO MCC_JIT=0 MCC_AST_SEARCH=1 MCC_SEARCH_WORKER=1 MCC_STATS_FORCE=1 \
	"$MCC" -O13 --stats $INCS -c "$ROOT/src/mcc.c" -o "$TMP/p1.o" 2>&1) || { echo "$out1" | tail -3; echo "Part1: compile FAILED"; exit 1; }
t1=$(date +%s.%N)
...
out2=$(env $CACHE_ISO MCC_AST_SEARCH=0 MCC_JIT=1 MCC_JIT_SUBMIT_AOT=1 MCC_JIT_HOT_CALLS=200 MCC_JIT_VERBOSE=1 \
	"$MCC" -O13 -run "$TMP/hot.c" 2>&1) || { echo "$out2" | tail -5; echo "Part2: run FAILED"; exit 1; }
