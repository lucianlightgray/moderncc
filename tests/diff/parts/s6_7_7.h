/* C9911 §6.7.7-§6.7.9 Type names/typedef/initialization (s6_7_7) — main-free test unit.
   No #includes: the includer provides the environment (full_language.c
   via mcclib.h; parts/run_s6_7_7.c via <std_env.h>). Compiled 3-way
   (gcc/clang/mcc) as a unit by the parts-suite, and aggregated into
   full_language.c. */
/* §6.7.9p36 EXAMPLE 12 both-ends + p19 override */
...
void s6_7_7_init_both_ends(void)
{
    ...
    int i;
    printf("both:");
    for (i = 0; i < S6_7_7_MAX; i++) printf(" %d", a[i]);
    printf("\n");
    /* p19 later override */
    int b[3] = { [0]=1, [1]=2, [0]=9 };
...
/* §6.7.9p28 EXAMPLE 5 inconsistently bracketed */
...
/* §6.7.9p35 EXAMPLE 11 mixed designators */
...
/* §6.7.9p29 EXAMPLE 6 brace elision multidim */
...
/* §6.7.9p17 fn149 union then next containing subobject; p38 EXAMPLE 13 */
void s6_7_7_init_union_next(void)
{
    struct { union { int a; char b; } u; int c; } s = { { 5 }, 7 };
    printf("un: %d %d\n", s.u.a, s.c);
    union { char c; int i; long l; } u = { .i = 42 };
    printf("uany: %d\n", u.i);
}

/* §6.7.9p33 EXAMPLE 9 enum-constant array designator; p34 EXAMPLE 10 */
enum s6_7_7_e { s6_7_7_one, s6_7_7_two, s6_7_7_three };
...
/* §6.7.9p22 sparse array size from largest index */
...
/* §6.7.9p32 EXAMPLE 8 string init sizes (null kept vs dropped) */
...
/* §6.7.8p8 EXAMPLE 5 VLA typedef fixes size at define time */
...
/* §6.7.8p5 EXAMPLE 2 typedef for incomplete tagged type */
...
{
    s6_7_7_TNODE a, b, c;
    a.count = 1; a.left = &b; a.right = &c;
    b.count = 2; b.left = 0; b.right = 0;
    c.count = 3; c.left = 0; c.right = 0;
    printf("tnode: %d %d %d\n",
        a.count, a.left->count, a.right->count);
}

/* §6.7.9p17 fn148 flat list flows through sub-array without brace */
void s6_7_7_init_flat(void)
{
    struct { int a[2]; int b; } s = { 1, 2, 3 };
    printf("flat: %d %d %d\n", s.a[0], s.a[1], s.b);
}
