/* C9911 §7.28-§7.31 uchar(C11)/wchar/wctype (s7_28) — main-free test unit.
...
void s7_28_uchar(void) {
    ...
    printf("utf32 %d\n", (int)__STDC_UTF_32__);
#endif
    /* §7.28.2.3 mbrtoc32 ASCII single-byte (C locale, identity) */
    mbstate_t st32; char32_t c32 = 0;
    for (int i = 0; i < (int)sizeof(st32); i++) ((char*)&st32)[i] = 0;
    size_t r = mbrtoc32(&c32, "A", 1, &st32);
    printf("mbrtoc32 %d %d\n", (int)r, (int)c32);
    /* §7.28.2.4 c32rtomb ASCII */
    char out[8]; mbstate_t st32b;
    for (int i = 0; i < (int)sizeof(st32b); i++) ((char*)&st32b)[i] = 0;
    size_t w = c32rtomb(out, 0x42, &st32b);
    printf("c32rtomb %d %d\n", (int)w, (int)out[0]);
    /* §7.28.2.1 mbrtoc16 ASCII, §7.28.2.2 c16rtomb */
    mbstate_t st16; char16_t c16 = 0;
    for (int i = 0; i < (int)sizeof(st16); i++) ((char*)&st16)[i] = 0;
    size_t r16 = mbrtoc16(&c16, "Q", 1, &st16);
    printf("mbrtoc16 %d %d\n", (int)r16, (int)c16);
    char o16[8]; mbstate_t st16b;
    for (int i = 0; i < (int)sizeof(st16b); i++) ((char*)&st16b)[i] = 0;
...
void s7_28_wconv(void) {
    ...
    /* §7.29.6.3.2 mbrtowc ASCII; §7.29.6.3.3 wcrtomb ASCII */
    wchar_t wc = 0; mbstate_t s2;
    for (int i = 0; i < (int)sizeof(s2); i++) ((char*)&s2)[i] = 0;
    size_t rr = mbrtowc(&wc, "K", 1, &s2);
    printf("mbrtowc %d %d\n", (int)rr, (int)wc);
    char mb[8]; mbstate_t s3;
    for (int i = 0; i < (int)sizeof(s3); i++) ((char*)&s3)[i] = 0;
    size_t ww = wcrtomb(mb, L'L', &s3);
    printf("wcrtomb %d %d\n", (int)ww, (int)mb[0]);
    /* §7.29.6.3.1 mbrlen */
    mbstate_t s4;
    for (int i = 0; i < (int)sizeof(s4); i++) ((char*)&s4)[i] = 0;
    printf("mbrlen %d\n", (int)mbrlen("Z", 1, &s4));
}

static int s7_28_sgn(int r) { return (r > 0) - (r < 0); }

void s7_28_wstr(void) {
    wchar_t buf[64];
    /* §7.29.4.6.1 wcslen; §7.29.4.2.1 wcscpy; §7.29.4.3.1 wcscat */
    wcscpy(buf, L"Hello");
    printf("wcslen %d\n", (int)wcslen(buf));
    wcscat(buf, L" World");
    printf("wcscat-len %d\n", (int)wcslen(buf));
    /* §7.29.4.4.1 wcscmp; §7.29.4.4.3 wcsncmp */
    printf("wcscmp %d %d %d\n",
        s7_28_sgn(wcscmp(L"abc", L"abc")),
        s7_28_sgn(wcscmp(L"abc", L"abd")),
        s7_28_sgn(wcscmp(L"abd", L"abc")));
    printf("wcsncmp %d\n", s7_28_sgn(wcsncmp(L"abcX", L"abcY", 3)));
    /* §7.29.4.5.1 wcschr; §7.29.4.5.4 wcsrchr */
    ...
    /* §7.29.4.5.2 wcscspn; §7.29.4.5.5 wcsspn; §7.29.4.5.3 wcspbrk */
    ...
    /* §7.29.4.2.2 wcsncpy padding */
    wchar_t nb[6]; wcsncpy(nb, L"ab", 5);
    printf("wcsncpy %d %d %d\n", (int)nb[0], (int)nb[2], (int)nb[4]);
    /* §7.29.4.3.2 wcsncat */
    wchar_t cb[16]; wcscpy(cb, L"ab"); wcsncat(cb, L"cdef", 2);
    printf("wcsncat %d\n", (int)wcslen(cb));
    /* §7.29.4.4.4 wcsxfrm self-consistency */
    wchar_t x1[32], x2[32];
...
void s7_28_wmem(void) {
    /* §7.29.4.2.3 wmemcpy; §7.29.4.2.4 wmemmove; §7.29.4.6.2 wmemset */
    wchar_t a[8] = {1,2,3,4,5,6,7,8}, b[8];
    wmemcpy(b, a, 4);
    printf("wmemcpy %d %d\n", (int)b[0], (int)b[3]);
    wmemmove(a + 1, a, 4);  /* overlap */
    printf("wmemmove %d %d %d\n", (int)a[0], (int)a[1], (int)a[4]);
    wchar_t s[5]; wmemset(s, 0x263A, 3); s[3] = 0;
    printf("wmemset %d %d\n", (int)s[0], (int)s[2]);
    /* §7.29.4.4.5 wmemcmp; §7.29.4.5.8 wmemchr */
...
void s7_28_wnum(void) {
    /* §7.29.4.1.1 wcstod; §7.29.4.1.2 wcstol/ul/ll/ull */
    wchar_t *end;
...
void s7_28_wprintf(void) {
    /* §7.29.2.3 swprintf; §7.29.2.4 swscanf */
    wchar_t buf[64];
    int n = swprintf(buf, 64, L"%d-%ls-%c", 7, L"hi", 'Z');
    printf("swprintf-ret %d\n", n);
    printf("swprintf-buf");
    for (int i = 0; buf[i]; i++) printf(" %d", (int)buf[i]);
    printf("\n");
    /* §7.29.2.3p2 width via * */
...
void s7_28_wctypef(void) {
    /* §7.30.2.1 classification in C locale, ASCII */
    ...
    /* §7.30.2.2 wctype/iswctype */
    printf("wctype %d %d\n",
        iswctype(L'q', wctype("alpha")) != 0,
        iswctype(L'5', wctype("digit")) != 0);
    printf("wctype-bad %d\n", wctype("nosuchclass") == 0);
    /* §7.30.3.1 towlower/towupper; §7.30.3.2 wctrans/towctrans */
    printf("towlower %d\n", (int)towlower(L'Z'));
    printf("towupper %d\n", (int)towupper(L'a'));
...
void s7_28_wtok(void) {
    /* §7.29.4.5.7 wcstok reentrant 3-arg */
    wchar_t s[] = L"a,bb,,ccc";
    wchar_t *save = NULL, *tok;
    tok = wcstok(s, L",", &save);
    printf("wcstok");
    while (tok) { printf(" %d", (int)wcslen(tok)); tok = wcstok(NULL, L",", &save); }
    printf("\n");
}
