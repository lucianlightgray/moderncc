/* C9911 §4 Conformance & §5 Environment (s04) — main-free test unit.
   No #includes: the includer provides the environment (full_language.c
   via mcclib.h; parts/run_s04.c via <std_env.h>). Compiled 3-way
   (gcc/clang/mcc) as a unit by the parts-suite, and aggregated into
   full_language.c. */
/* §5.2.2p2/p3, §5.2.1p2/p3: escape-sequence values, null char, digit contiguity */
void s04_charset_test(void)
{
    ...
    s04_vals[0]=s04_a; s04_vals[1]=s04_b; s04_vals[2]=s04_f; s04_vals[3]=s04_n;
    s04_vals[4]=s04_r; s04_vals[5]=s04_t; s04_vals[6]=s04_v;
    for (s04_i=0;s04_i<7;s04_i++)
        for (s04_j=s04_i+1;s04_j<7;s04_j++)
            if (s04_vals[s04_i]==s04_vals[s04_j]) s04_distinct=0;
    printf("esc distinct: %d\n", s04_distinct);
    printf("nul=%d strterm=%d\n", '\0', s04_str[2]=='\0');
    printf("digits=%d %d\n", '9'-'0'==9, ('0'<'1') && ('1'<'2') && ('8'<'9'));
}

/* §5.2.4.1p1: >=63 significant initial characters in an internal identifier */
...
/* §5.2.4.2.1: <limits.h> range invariants + #if-usability of the macros */
...
/* §5.2.4.2.2: <float.h> model characteristics incl. C11 additions */
