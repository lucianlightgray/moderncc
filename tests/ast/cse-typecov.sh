...
#
# The subject repeats a double expr, a float expr, and an int expr -- 3 CSE
# folds. Anti-vacuity: an integer-only CSE would fold only 1 (icse); with the
# float type extension it folds 3. Correctness: -O2 output must equal -O0.
# (long double is deliberately excluded -- >8B vs the shared width-8 ltemp slot;
# pointer CSE is a separate residual, blocked by a non-type gate.)
set -e
...
echo "cse folds (double+float+int) = $cse"

if [ -z "$cse" ] || [ "$cse" -lt 3 ]; then
	echo "FAIL: type-complete CSE did not fire on the double+float expressions (cse=$cse, want >=3)"
	exit 1
fi
...
echo "ast/cse-typecov OK: CSE type-complete (cse=$cse over double+float+int), result-invariant ($r2)"
