...
function(build_and_run out_exe expect_rc env_on out_all)
    # One knob, one flag. The off spelling is the on spelling with -fno-, so
    # each knob is a single append and there is no else arm to keep in step.
    macro(_knob _var _flag)
        set(_pfx "no-")
        if(${_var})
            set(_pfx "")
        endif()
        list(APPEND _flags "-f${_pfx}${_flag}")
    endmacro()
    ...
    if(env_on)
        set(_opt "-O1")
        list(APPEND _flags -fdump-replay)
        _knob(TEMPLATES reemit-templates)
        _knob(PROMOTE promote-locals)
        _knob(INLINE inline)
        if(COLOR)
            list(APPEND _flags -freg-color -fspill-share)
        endif()
    endif()
    execute_process(
        COMMAND "${MCC}" "-B${BDIR}" "${_opt}" ${_flags} "${SRC}" -o "${out_exe}"
