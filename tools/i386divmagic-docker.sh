...
dg_docker run --rm --platform linux/amd64 \
	...
	command -v gcc >/dev/null && command -v objdump >/dev/null && command -v bash >/dev/null || {
	...
	cd /b
	echo "-- building mcc-i386-opt --"
	gcc -O1 -w -DMCC_CONFIG_OPTIMIZER=1 -DMCC_TARGET_I386=1 \
	...
	nmulh=$(objdump -dr /w/soak.o | grep -Ec "__mcc_[su]mulh64" || true)
	echo "   idiv=$nidiv imul=$nimul __mcc_?mulh64_calls=$nmulh"
	if [ "$nimul" -lt 20 ] || [ "$nmulh" -lt 10 ]; then
		echo "FAIL  divmagic did not fire (expected imul>=20 and __mcc_?mulh64 calls>=10)"
		exit 2
	fi
