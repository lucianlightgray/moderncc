/*
 *  mccharness.c - one conformance / differential test runner (PLAN 0.7)
 *
 *  The cmake -P drivers in the duplication catalog (#3) all share one skeleton:
 *  glob or fixed program list -> per-program compile -> (link) -> run (maybe
 *  under a launcher prefix) -> judge by exit code or stdout byte-compare ->
 *  first-meaningful-stderr-line on failure.  This binary is that skeleton,
 *  selected per suite by the first argument, reusing the shared tool layer
 *  (host spawn/glob/file-compare, ts_first_error_line).
 *
 *  Suites migrated so far:
 *    parts    3-way (gcc/clang/mcc) stdout-identical over a glob of wrappers
 *             (replaces tests/diff/run_parts.cmake)
 *    mcctest  ref-cc vs mcc stdout-identical for one source
 *             (replaces the generated run_mcctest.cmake)
 *
 *  exit: 0 all pass, 1 a failure, 2 usage.
 */
#include "toolsupport.h"

/* first-meaningful-stderr-line needles/skips for a failed compile/link */
...
static int compile(const char *const *argv, char **err) { return compile_l(argv, NULL, err); }

/* run argv under optional launcher, redirecting stdout to outfile */
...
static int run_to(const char *const *argv, const char *outfile) { return run_to_l(argv, NULL, outfile); }
...
static void split_append(Argv *v, const char *s)
{
    ...
    for (tok = strtok_r(dup, " \t", &save); tok; tok = strtok_r(NULL, " \t", &save))
        A(v, tok);   /* dup's storage persists for the process lifetime */
}

/* current environ plus the given NULL-terminated extra "K=V" entries, as a
   fresh NULL-terminated array (leaked; short-lived tool) */
static const char *const *make_env_plus(const char *const *extra)
{
    ...
    for (i = 0; i < n; i++) out[i] = env[i];
    for (i = 0; i < e; i++) out[n + i] = extra[i];
    out[n + e] = NULL;
    return out;
}

/* build a NULL-terminated launcher argv from an emulator string, or NULL */
...
/* capture both streams of a run under optional launcher */
...
/* run capturing+discarding both streams; return exit code */
...
/* write a whole string to a file; 0 ok */
...
/* a driver-level skip: print the ctest "SKIP:" marker and return 0 (these
   suites are registered with SKIP_REGULAR_EXPRESSION "SKIP:") */
...
/* normalize -E output like the cmake pp_norm: drop column-0 '#' lines, collapse
   runs of space/tab/CR to one space, strip, drop blank lines.  malloc'd. */
static char *pp_norm(const char *raw)
{
    ...
    while (*p) {
        ...
        int inws = 0;
        if (len == 0 || p[0] == '#') { if (!e) break; p = e + 1; continue; }
        for (i = 0; i < len; i++) {
            char c = p[i];
            if (c == ' ' || c == '\t' || c == '\r') { inws = 1; continue; }
            if (inws && ti > 0) out[oi + ti++] = ' ';
            inws = 0;
            out[oi + ti++] = c;
        }
        if (ti > 0) { oi += ti; out[oi++] = '\n'; }
        if (!e) break;
        p = e + 1;
    }
    out[oi] = 0;
...
/* ------------------------------------------------------------------- */
/* parts: gcc/clang/mcc must agree byte-for-byte on stdout, over a glob */
...
static int suite_parts(int argc, char **argv)
{
    ...
    for (i = 0; i < nw; i++) {
        ...
        const char *base = strrchr(w, '/'); base = base ? base + 1 : w;
        ...
        { Argv v = {{0},0};
          A(&v, gcc); A(&v, w); A(&v, Ipart); A(&v,"-w"); A(&v,"-O0"); A(&v,"-std=gnu11");
          A(&v,"-lm"); A(&v,"-o"); A(&v, eg);
          if (compile(Z(&v), &err)) { report_err("gcc build", name, err); free(err); fail++; continue; }
          free(err); err = NULL; }
        { const char *r[] = { eg, 0 }; run_to(r, og); }

        { Argv v = {{0},0};
          A(&v, clang); A(&v, w); A(&v, Ipart); A(&v,"-w"); A(&v,"-O0"); A(&v,"-std=gnu11");
          A(&v,"-lm"); A(&v,"-o"); A(&v, ec);
          if (compile(Z(&v), &err)) { report_err("clang build", name, err); free(err); fail++; continue; }
          free(err); err = NULL; }
        { const char *r[] = { ec, 0 }; run_to(r, oc); }

        { Argv v = {{0},0};
          A(&v, mcc); A(&v, Bflag); A(&v, Iinc); A(&v, Ipart); A(&v, w);
          A(&v,"-lm"); A(&v,"-o"); A(&v, em);
          if (compile(Z(&v), &err)) { report_err("mcc build", name, err); free(err); fail++; continue; }
          free(err); err = NULL; }
        { const char *r[] = { em, 0 }; run_to(r, om); }

        if (ts_file_equal(og, oc) != 1) { fprintf(stderr, "[%s] gcc vs clang stdout diverged\n", name); bad = 1; }
        if (ts_file_equal(og, om) != 1) { fprintf(stderr, "[%s] gcc vs mcc stdout diverged\n", name); bad = 1; }
        if (bad) fail++; else ok++;
...
/* ------------------------------------------------------------------- */
/* mcctest: reference cc vs mcc must agree on stdout for one source     */
...
static int suite_mcctest(int argc, char **argv)
{
    ...
    const char *work = opt(argc, argv, "--work", NULL);
    const char *testdefs = opt(argc, argv, "--testdefs", NULL);  /* both legs */
    const char *refflags = opt(argc, argv, "--refflags", NULL);  /* ref only */
    const char *mccargs  = opt(argc, argv, "--mccargs", NULL);   /* mcc only */
    ...
    { Argv v = {{0},0};
      A(&v, cc); A(&v, src); A(&v,"-o"); A(&v, refexe); A(&v, Iinc); A(&v, Isrc); A(&v, Ibld);
      split_append(&v, testdefs);
      A(&v,"-w"); A(&v,"-O0"); A(&v,"-std=gnu11"); A(&v,"-fno-omit-frame-pointer");
      split_append(&v, refflags);
      A(&v,"-lm");
      if (compile(Z(&v), &err)) { report_err("cc build", "mcctest", err); free(err); return 1; }
      free(err); err = NULL; }
    { const char *r[] = { refexe, 0 }; run_to(r, refout); }

    /* mcc build (optionally under the emulator launcher) */
    { Argv v = {{0},0};
      A(&v, mcc); A(&v, Bflag); A(&v, Iinc); A(&v, Isrc); A(&v, Ibld);
      split_append(&v, testdefs);
      split_append(&v, mccargs);
      A(&v, src); A(&v,"-o"); A(&v, mccexe); A(&v,"-lm");
      if (compile_l(Z(&v), emu, &err)) { report_err("mcc build", "mcctest", err); free(err); return 1; }
      free(err); err = NULL; }
    { const char *r[] = { mccexe, 0 }; run_to_l(r, emu, mccout); }
...
/* ------------------------------------------------------------------- */
/* mccexe: compile source(s) with mcc and require the program to exit 0  */
...
static int suite_mccexe(int argc, char **argv)
{
    ...
    { Argv v = {{0},0};
      A(&v, mcc); A(&v, Bflag); A(&v, Iinc);
      split_append(&v, srcs);
      A(&v,"-o"); A(&v, out);
      if (compile_l(Z(&v), emu, &err)) {
          report_err("mcc compile", srcs, err); free(err); return 1;
      }
      free(err); }

    { Argv v = {{0},0};
      HostSpawnOpts o; memset(&o, 0, sizeof o); o.launcher = emu;
      A(&v, out);
      split_append(&v, runargs);
      rc = host_spawn_ex(Z(&v), &o);
      if (rc != 0) { fprintf(stderr, "%s exited with %d\n", out, rc); return 1; }
    }
...
/* ------------------------------------------------------------------- */
/* asmconnect: combined vs separately-linked build must agree on stdout  */
...
static int suite_asmconnect(int argc, char **argv)
{
    ...
    { const char *r[]={single,0}; run_to_l(r,emu,o1); }
    { Argv v={{0},0}; A(&v,mcc);A(&v,Bflag);A(&v,Iinc);A(&v,"-c");A(&v,p1);A(&v,"-o");A(&v,a1); if(compile_l(Z(&v),emu,&err)){report_err("compile part1","asm-c-connect",err);free(err);return 1;} free(err);err=NULL; }
    { Argv v={{0},0}; A(&v,mcc);A(&v,Bflag);A(&v,Iinc);A(&v,"-c");A(&v,p2);A(&v,"-o");A(&v,a2); if(compile_l(Z(&v),emu,&err)){report_err("compile part2","asm-c-connect",err);free(err);return 1;} free(err);err=NULL; }
    { Argv v={{0},0}; A(&v,mcc);A(&v,Bflag);A(&v,Iinc);A(&v,a1);A(&v,a2);A(&v,"-o");A(&v,sep);
      if (compile_l(Z(&v),emu,&err)){report_err("separate link","asm-c-connect",err);free(err);return 1;} free(err);err=NULL; }
    { const char *r[]={sep,0}; run_to_l(r,emu,o2); }
...
/* ------------------------------------------------------------------- */
/* dashs: mcc -S listing must re-assemble to a behaviourally identical exe */
...
static int suite_dashs(int argc, char **argv)
{
    ...
    { const char *r[]={direct,0}; run_to_l(r,emu,out1); }

    { Argv v={{0},0}; A(&v,mcc);A(&v,Bflag);A(&v,Iinc);A(&v,"-S");A(&v,src);A(&v,"-o");A(&v,sfile);
      if (compile_l(Z(&v),emu,&err)){report_err("-S emit","dash-S",err);free(err);return 1;} free(err);err=NULL; }

    /* sanity: the listing must carry code directives, a function label, a size */
    asmtext = ts_read_file(sfile, NULL);
    if (!asmtext || !strstr(asmtext, "\n\t.text") || !strstr(asmtext, "\nmain:")
        || !strstr(asmtext, ".size\tmain")) {
    ...
    { Argv v={{0},0}; A(&v,mcc);A(&v,Bflag);A(&v,Iinc);A(&v,sfile);A(&v,"-o");A(&v,via);
      if (compile_l(Z(&v),emu,&err)){report_err("re-assemble","dash-S",err);free(err);return 1;} free(err);err=NULL; }
    { const char *r[]={via,0}; run_to_l(r,emu,out2); }
...
/* ------------------------------------------------------------------- */
/* preprocess: mcc -E must match the gcc==clang consensus (impl-defined     */
/* divergences skipped), plus .S -E smoke and diagnostics/ diagnosis checks */
...
static int suite_preprocess(int argc, char **argv)
{
    ...
    /* gcc and clang identical (same binary or same --version) => no independent
       consensus: mcc divergences on impl-defined output are skipped, not failed */
    { char *gc = host_path_canonical(gpath), *cc = host_path_canonical(cpath);
      if (gc && cc && !strcmp(gc, cc)) pp_same = 1;
      free(gc); free(cc);
      if (!pp_same) {
          char *gv = NULL, *cv = NULL, *e = NULL;
          const char *av1[] = { gpath, "--version", 0 }, *av2[] = { cpath, "--version", 0 };
          run_cap(av1, NULL, &gv, &e); free(e); e = NULL;
          run_cap(av2, NULL, &cv, &e); free(e);
          if (gv && cv && *gv && !strcmp(gv, cv)) pp_same = 1;
          free(gv); free(cv);
      }
    }
    if (pp_same)
    ...
    for (i = 0; i < nf; i++) {
        ...
        if (rl >= 2 && !strcmp(f + rl - 2, ".S")) {
            ...
            free(o); free(e);
            if (rc == 0) pass++;
            else { fail++; fprintf(stderr, "FAIL %s: mcc -E crashed/hung on asm input\n", rel); }
        } else if (!strncmp(rel, "diagnostics/", 12)) {
            char *ge = NULL, *ce = NULL, *me = NULL, *o = NULL;
            const char *ag[] = { gpath, "-E", f, 0 }, *ac[] = { cpath, "-E", f, 0 };
            int gd, cd;
            run_cap(ag, NULL, &o, &ge); free(o); o = NULL;
            run_cap(ac, NULL, &o, &ce); free(o); o = NULL;
            ...
            if (gd || cd) {
                ...
                run_cap(am, NULL, &o, &me); free(o);
                if (ci_contains(me, "error") || ci_contains(me, "warning")) pass++;
                else { fail++; fprintf(stderr, "FAIL %s: mcc emits no diagnostic (gcc/clang do)\n", rel); }
                free(me);
            } else skip++;
        } else {
            char *go = NULL, *co = NULL, *mo = NULL, *gn, *cn, *mn;
            const char *ag[] = { gpath, "-E", f, 0 }, *ac[] = { cpath, "-E", f, 0 };
            run_cap(ag, NULL, &go, &e); free(e); e = NULL;
            run_cap(ac, NULL, &co, &e); free(e);
            gn = pp_norm(go); cn = pp_norm(co);
            free(go); free(co);
            if (strcmp(gn, cn)) { skip++; }
            else {
                const char *am[] = { mcc, Bflag, Iinc, "-E", f, 0 };
                run_cap(am, NULL, &mo, &e); free(e);
                mn = pp_norm(mo); free(mo);
                if (!strcmp(mn, gn)) pass++;
                else if (pp_same) skip++;
                else { fail++; fprintf(stderr, "FAIL %s: mcc -E diverges from gcc==clang\n", rel); }
                free(mn);
            }
            free(gn); free(cn);
        }
...
/* ------------------------------------------------------------------- */
/* i386fastcall: __fastcall ABI interop between mcc-i386 and gcc -m32       */
/* (ports run_i386_fastcall.cmake; generated sources are string tables)    */
...
#undef P
    ...
    { char fp[4096], fm[4096], fmo[4096], fgo[4096], fpe[4096]; Argv a = {{0},0}, b = {{0},0}, c = {{0},0};
      ...
      A(&b, gpath); split_append(&b, m32s); A(&b, "-c"); A(&b, fm); A(&b, "-o"); A(&b, fgo);
      A(&c, gpath); split_append(&c, m32s); A(&c, fgo); A(&c, fmo); A(&c, "-o"); A(&c, fpe);
      if (run_quiet(Z(&a)) || run_quiet(Z(&b)) || run_quiet(Z(&c)))
          return skip0("cannot link mcc + reference-cc objects (object-format mismatch)"); }

    write_file(cC, FC_CALLEE); write_file(cR, FC_CALLER); write_file(cU, FC_UNSUP);
    /* compile the ABI test both ways */
    { Argv v = {{0},0}; A(&v, imcc); A(&v, "-c"); A(&v, cC); A(&v, "-o"); A(&v, et); if (run_quiet(Z(&v))) fail = 1; }
    { Argv v = {{0},0}; A(&v, imcc); A(&v, "-c"); A(&v, cR); A(&v, "-o"); A(&v, lt); if (run_quiet(Z(&v))) fail = 1; }
    { Argv v = {{0},0}; A(&v, gpath); split_append(&v, m32s); A(&v,"-O0"); A(&v,"-c"); A(&v, cC); A(&v,"-o"); A(&v, eg); if (run_quiet(Z(&v))) fail = 1; }
    { Argv v = {{0},0}; A(&v, gpath); split_append(&v, m32s); A(&v,"-O0"); A(&v,"-c"); A(&v, cR); A(&v,"-o"); A(&v, lg); if (run_quiet(Z(&v))) fail = 1; }

    /* three cross-link/run combinations must all exit 0 */
    { struct { const char *name, *o1, *o2; } chk[] = {
      ...
      for (i = 0; i < 3; i++) {
          Argv v = {{0},0};
          A(&v, gpath); split_append(&v, m32s); A(&v, chk[i].o1); A(&v, chk[i].o2); A(&v,"-o"); A(&v, run);
          if (run_quiet(Z(&v))) { printf("FAIL  %s (link)\n", chk[i].name); fail = 1; continue; }
          { const char *r[] = { run, 0 }; int rc = host_spawn_ex(r, NULL);
            if (rc == 0) printf("PASS  %s\n", chk[i].name);
            else { printf("FAIL  %s (exit %d)\n", chk[i].name, rc); fail = 1; } }
      } }
    /* unsupported float-before-reg fastcall must be rejected by mcc */
    { Argv v = {{0},0}; A(&v, imcc); A(&v,"-c"); A(&v, cU); A(&v,"-o"); A(&v, uo);
      if (run_quiet(Z(&v)) == 0) { printf("FAIL  unsupported float-before-reg case should error\n"); fail = 1; }
      else printf("PASS  unsupported float-before-reg case errors cleanly\n"); }

    if (fail) { fprintf(stderr, "i386-fastcall-abi: FAILURES\n"); return 1; }
    printf("ALL OK\n");
    return 0;
}

/* ------------------------------------------------------------------- */
/* gcctestsuite: run mcc over the gcc c-torture set, writing mcc.sum       */
/* (ports run_gcctestsuite.cmake; dev custom-target, skips without path)   */
...
static int gccts_skiplisted(const char *base, const char *content)
{
    ...
    for (i = 0; GCCTS_FIXED_SKIP[i]; i++)
        if (!strcmp(base, GCCTS_FIXED_SKIP[i])) return 1;
...
static int suite_gcctestsuite(int argc, char **argv)
{
    ...
    char *files[8192]; int nf = 0, i, ok = 0, sk = 0, fa = 0, xf = 0, isd;
    FILE *sum;

    if (!mcc || !builddir) { fprintf(stderr, "usage: mccharness gcctestsuite --mcc --builddir [--bdir --idir --path]\n"); return 2; }
    ...
    for (i = 0; i < (nf < 0 ? 0 : nf); i++) {
        ...
        char *content = ts_read_file(s, NULL), *o = NULL, *e = NULL; const char *r;
        Argv v = {{0},0};
        A(&v, mcc); A(&v, Bflag); A(&v, "-DNO_TRAMPOLINES");
        if (idir) { A(&v, Iinc); A(&v, Iinc2); }
        A(&v, "-o"); A(&v, tsto); A(&v, "-c"); A(&v, s);
        int rc = run_cap(Z(&v), NULL, &o, &e);
        if ((o && strstr(o, "cannot use local functions")) || (e && strstr(e, "cannot use local functions"))) { r = "SKIP"; sk++; }
        else if (rc == 0) { r = "PASS"; ok++; }
        else if (gccts_skiplisted(base, content)) { r = "SKIP"; sk++; }
        else { r = "FAIL"; fa++; }
        fprintf(sum, "%s: %s\n", r, s);
        free(content); free(o); free(e); free(files[i]);
    }
    /* execute tests (+ ieee) */
    { const char *subs[] = { "execute", "execute/ieee", 0 }; int si;
      for (si = 0; subs[si]; si++) {
        ...
        for (i = 0; i < (nf < 0 ? 0 : nf); i++) {
            ...
            A(&v, mcc); A(&v, Bflag); A(&v, "-DNO_TRAMPOLINES");
            if (idir) { A(&v, Iinc); A(&v, Iinc2); }
            A(&v, s); A(&v, "-o"); A(&v, tstx); A(&v, "-lm");
            int rc = run_cap(Z(&v), NULL, &o, &e);
            if ((o && strstr(o, "cannot use local functions")) || (e && strstr(e, "cannot use local functions"))) { r = "SKIP"; sk++; }
            else if (rc == 0) {
                const char *rr[] = { tstx, 0 };
                if (run_quiet(rr) == 0) { r = "PASS"; ok++; }
                else { r = "FAILEXE"; xf++; }
            }
            else if (gccts_skiplisted(base, content)) { r = "SKIP"; sk++; }
            else { r = "FAIL"; fa++; }
...
/* ------------------------------------------------------------------- */
/* penative: native PE conformance - compile + run each program (exit 0)   */
/* (ports run_pe_native.cmake; SKIP_RETURN_CODE 77)                        */
...
static int suite_penative(int argc, char **argv)
{
    ...
    char *files[4096]; int nf, i, status = 0, isd;
    ...
    for (i = 0; i < (nf < 0 ? 0 : nf); i++) {
        const char *f = files[i], *base = strrchr(f, '/'); char n[256], exe[4200], *err = NULL;
        base = base ? base + 1 : f;
        snprintf(n, sizeof n, "%.*s", (int)(strlen(base) - 2), base);   /* strip .c */
        ...
        { Argv v = {{0},0};
          ...
          if (compile(Z(&v), &err)) {
              ...
              printf("FAIL %s (compile): %s\n", n, l ? l : ""); free(l); free(err);
              status = 1; free(files[i]); continue;
          }
          free(err); }
        { const char *r[] = { exe, 0 }; HostSpawnOpts o; memset(&o,0,sizeof o);
          o.stdout_file = runout; o.stderr_file = runout;
          int rc = host_spawn_ex(r, &o);
          if (rc == 0) printf("PASS %s\n", n);
          else { printf("FAIL %s (run, rc=%d)\n", n, rc); status = 1; } }
        remove(exe);
        free(files[i]);
    }
    return status ? 1 : 0;
...
static int suite_qemurun(int argc, char **argv)
{
    ...
    const char *launcher[4]; char *files[4096]; int nf, i, fails = 0;
    ...
    launcher[0] = qemu; launcher[1] = "-L"; launcher[2] = sysroot; launcher[3] = 0;

    nf = ts_glob(srcdir, "*.c", 0, files, 4096);
    for (i = 0; i < (nf < 0 ? 0 : nf); i++) {
        const char *s = files[i], *base = strrchr(s, '/'); char n[256]; int m;
        base = base ? base + 1 : s;
        ...
        for (m = 0; m < 2; m++) {              /* 0=default, 1=pic */
            ...
            A(&v, s); A(&v, "-o"); A(&v, out);
            if (compile(Z(&v), &err)) { fprintf(stderr, "  %s [%s]: compile failed\n%s\n", n, m?"pic":"default", err?err:""); fails++; free(err); continue; }
            free(err);
            { const char *r[] = { out, 0 }; int rc = run_to_l(r, launcher, "/dev/null");
              if (rc != 0) { fprintf(stderr, "  %s [%s]: run exited %d\n", n, m?"pic":"default", rc); fails++; } }
        }
        free(files[i]);
    }
    if (fails) { fprintf(stderr, "qemu conformance failures: %d\n", fails); return 1; }
    printf("qemu conformance: all programs passed\n");
    return 0;
}

/* ------------------------------------------------------------------- */
/* pewine: PE conformance under wine, per-target staging + env            */
/* (ports tests/qemu/run_pe_wine.cmake)                                    */

/* copy every file matching pat in dir into dstdir (basename preserved) */
static void copy_glob(const char *dir, const char *pat, const char *dstdir)
{
    char *files[512]; int n = ts_glob(dir, pat, 0, files, 512), i;
    for (i = 0; i < (n < 0 ? 0 : n); i++) {
...
static int suite_pewine(int argc, char **argv)
{
    ...
    if (!src || !xb || !work) { fprintf(stderr, "usage: mccharness pewine --src --xb --work\n"); return 2; }
        ...
        for (i = 0; i < (nf < 0 ? 0 : nf); i++) {
            ...
            { Argv v = {{0},0}; A(&v, mcc); A(&v, Bf); A(&v, Iw32); A(&v, Irt); A(&v, f); A(&v,"-o"); A(&v, exe);
              if (compile(Z(&v), &err)) { char *l = ts_first_error_line(err, NULL, NULL);
                  printf("FAIL %s/%s (compile): %s\n", tgt, n, l ? l : ""); free(l); free(err); status = 1; free(files[i]); continue; }
              free(err); }
            { const char *r[] = { exe, 0 }; const char *lp[] = { wine, 0 };
              HostSpawnOpts o; memset(&o, 0, sizeof o);
              o.launcher = lp; o.env = env; o.stdout_file = runout; o.stderr_file = runout;
              int rc = host_spawn_ex(r, &o);
              if (rc == 0) printf("PASS %s/%s\n", tgt, n);
              else { printf("FAIL %s/%s (run, rc=%d)\n", tgt, n, rc); status = 1; } }
            remove(exe);
            free(files[i]);
        }
    }
    if (!any) ts_skip("no win32 cross-mcc for any target");
    return status ? 1 : 0;
}

/* ------------------------------------------------------------------- */
/* machonative: native macOS Mach-O conformance (Darwin host only)         */
/* (ports tests/qemu/run_macho_native.cmake)                               */
...
static int suite_machonative(int argc, char **argv)
{
    ...
    { Argv v = {{0},0}; char *err = NULL; A(&v, mcc); A(&v, Bf); A(&v, probe); A(&v,"-o"); A(&v, probeexe);
      ...
      free(err); }
    if (!obj_is_macho(objcheck, probeexe)) ts_skip("native mcc does not target Mach-O");
    ...
    for (i = 0; MACHO_PROGS_NATIVE[i]; i++) {
        ...
        { Argv v = {{0},0}; A(&v, mcc); A(&v, Bf); A(&v, Iinc); A(&v, f); A(&v,"-o"); A(&v, out);
          ...
          free(err); }
        if (!obj_is_macho(objcheck, out)) { printf("FAIL osx/%s: not a Mach-O image\n", t); status = 1; continue; }
        { const char *r[] = { out, 0 }; int rc = run_quiet(r);
          if (rc == 0) printf("PASS osx/%s (native Mach-O executed)\n", t);
...
/* ------------------------------------------------------------------- */
/* machoimage: Mach-O images run through the in-tree loader (x86_64 host)  */
/* (ports tests/qemu/run_macho_image.cmake; wrap.c is a string table)      */

static int host_is_x86_64(void)
{
#if defined(__x86_64__) || defined(__amd64__)
    return 1;   /* host CPU (target axis) - not a host-gate OS macro */
#else
    return 0;
#endif
}
...
static int suite_machoimage(int argc, char **argv)
{
    ...
    char wrapc[4200], wrapo[4200], Iinc[4200]; char probe[4300]; int i, status = 0, isd;
    ...
    for (i = 0; PROGS[i]; i++) {
        ...
        { Argv v = {{0},0}; A(&v, mcc); A(&v,"-nostdlib"); A(&v, co); A(&v, wrapo); A(&v, o1); A(&v, o2); A(&v, o3); A(&v, o4); A(&v,"-o"); A(&v, macho);
          if (compile(Z(&v), &err)) { char *l = ts_first_error_line(err, NULL, SKIPS); printf("FAIL osx/%s (link): %s\n", t, l?l:""); free(l); free(err); status = 1; continue; } free(err); }
        if (!obj_is_macho(objcheck, macho)) { printf("FAIL osx/%s: not a Mach-O\n", t); status = 1; continue; }
        { const char *r[] = { loader, macho, 0 }; int rc = run_quiet(r);
          if (rc == 0) printf("PASS osx/%s (Mach-O image loaded + executed)\n", t);
          else { printf("FAIL osx/%s (run, rc=%d)\n", t, rc); status = 1; } }
        remove(macho);
    }
    return status ? 1 : 0;
...
/* ------------------------------------------------------------------- */
/* machoapplelibc: run Apple's vendored libc as Mach-O images              */
/* (ports tests/qemu/run_macho_apple_libc.cmake)                           */
                        ...
                        const char *work, const char *src, const char *label)
{
    ...
    a[n++] = mcc; a[n++] = "-nostdlib"; a[n++] = testo;
    for (i = 0; i < nobjs; i++) a[n++] = objs[i];
    a[n++] = wrapo;
    for (i = 0; i < nrt; i++) a[n++] = rt[i];
    a[n++] = "-o"; a[n++] = macho; a[n] = 0;
    { char *o = NULL; int rc = run_cap(a, NULL, &o, &err); free(o); free(a);
    ...
    { const char *ldargv[3]; char loader[8192]; ts_path(loader, sizeof loader, work, "machoload");
      ...
      { int rc = run_quiet(ldargv);
        ...
        printf("FAIL %s (run, rc=%d)\n", label, rc); remove(macho); return 1; } }
}
...
static int suite_machoapplelibc(int argc, char **argv)
{
    ...
    char *objs[1024], *rt[4]; int nobjs = 0, nrt = 0, status = 0, isd, di;
    ...
    for (di = 0; SUBDIRS[di] && !status; di++) {
        ...
        for (i = 0; i < (nf < 0 ? 0 : nf); i++) {
            ...
            { Argv v = {{0},0}; A(&v, mcc); A(&v,"-nostdlib"); A(&v, shiminc); A(&v,"-c"); A(&v, f); A(&v,"-o"); A(&v, oo);
              ...
              free(err); }
            if (nobjs < 1024) objs[nobjs++] = strdup(oo);
            free(files[i]);
        }
    }
    if (status) return 1;
    { Argv v = {{0},0}; char *err = NULL; A(&v, mcc); A(&v,"-nostdlib"); A(&v, shiminc); A(&v,"-c"); A(&v, wrapc); A(&v,"-o"); A(&v, wrapo);
...
/* ------------------------------------------------------------------- */
/* machocodegen: osx-cross codegen linked as an ELF via a _sym->sym shim   */
/* and run natively (x86_64) or under qemu (arm64)                         */
/* (ports tests/qemu/run_macho_codegen.cmake)                              */
...
{
    int i; for (i = 0; set[i]; i++) if (!strcmp(s, set[i])) return 1; return 0;
}
...
static int suite_machocodegen(int argc, char **argv)
{
    ...
    char *rt[8]; int nrt = 0, status = 0, isd, i;
    char shim[8192]; int sl = 0;
    char *progs[4096]; int np;
    ...
    if (is_arm) {
        char *tg = NULL, *e = NULL;
        if (!host_find_tool("clang", NULL, clang, sizeof clang)) ts_skip("no clang for the aarch64 link");
        { const char *a[] = { clang, "-print-targets", 0 }; run_cap(a, NULL, &tg, &e); }
        if (!tg || !strstr(tg, "aarch64")) { free(tg); free(e); ts_skip("clang lacks the aarch64 target"); }
        free(tg); free(e);
        if (!host_find_tool("qemu-aarch64", NULL, qemu, sizeof qemu)) ts_skip("no qemu-aarch64");
        if (!*sysroot || host_stat(sysroot, &isd, NULL, NULL) || !isd) ts_skip("no arm64 glibc sysroot (%s)", sysroot);
    }
    /* runtime object set (per arch) */
    ...
    for (i = 0; CG_LIBCNAMES[i]; i++) sl += snprintf(shim+sl, sizeof shim-sl, "tramp _%s, %s\n", CG_LIBCNAMES[i], CG_LIBCNAMES[i]);
    ...
    for (i = 0; i < (np < 0 ? 0 : np); i++) {
        const char *f = progs[i], *base = strrchr(f, '/'); char n[256], oo[4300], run[4300], *err = NULL;
        base = base ? base + 1 : f;
        snprintf(n, sizeof n, "%.*s", (int)(strlen(base) - 2), base);
        ...
        { Argv v = {{0},0}; A(&v, mcc); A(&v, Iinc); A(&v,"-c"); A(&v, f); A(&v,"-o"); A(&v, oo);
          ...
          free(err); err = NULL; }
        { const char **a = malloc((nrt + 32) * sizeof *a); int m = 0, k; char sysf[4300], isysf[4300], Ls[4][4300];
          if (!is_arm) {
              a[m++] = gcc; a[m++] = harness; a[m++] = oo; a[m++] = shimS;
              for (k = 0; k < nrt; k++) a[m++] = rt[k];
              a[m++] = "-o"; a[m++] = run;
          } else {
              snprintf(sysf, sizeof sysf, "--sysroot=%s", sysroot);
              ...
              a[m++] = "-isystem"; a[m++] = isysf; a[m++] = harness; a[m++] = shimS; a[m++] = oo;
              for (k = 0; k < nrt; k++) a[m++] = rt[k];
              a[m++] = Ls[0]; a[m++] = Ls[1]; a[m++] = Ls[2]; a[m++] = Ls[3]; a[m++] = "-o"; a[m++] = run;
          }
          a[m] = 0;
          { char *o = NULL; int rc = run_cap(a, NULL, &o, &err); free(o); free(a);
            if (rc) { char *l = ts_first_error_line(err, CG_LINK_NEEDLES, NULL); printf("FAIL osx-%s/%s (link): %s\n", arch, n, l?l:""); free(l); free(err); status = 1; free(progs[i]); continue; }
            free(err); } }
        { int rc; if (is_arm) { const char *r[] = { run, 0 }; const char *lp[] = { qemu, "-L", sysroot, 0 }; rc = run_to_l(r, lp, "/dev/null"); }
          else { const char *r[] = { run, 0 }; rc = run_quiet(r); }
          if (rc == 0) printf("PASS osx-%s/%s (%s-osx codegen executed)\n", arch, n, arch);
          else { printf("FAIL osx-%s/%s (run, rc=%d)\n", arch, n, rc); status = 1; } }
        remove(run);
        free(progs[i]);
    }
    for (i = 0; i < nrt; i++) free(rt[i]);
    return status ? 1 : 0;
}

/* ------------------------------------------------------------------- */
/* armasm: assemble each mnemonic x operand-combo with mcc and GNU as,     */
/* diff the objdump output (ports run_arm_asm.cmake; dev custom-target)    */
...
static int arm_isreg(const char *s)
{
    ...
    int i;
    for (i = 0; ex[i]; i++) if (!strcmp(s, ex[i])) return 1;
...
/* objdump -S obj, keeping only lines whose first token is "0:" */
static char *arm_dump(const char *objdump, const char *obj)
{
    const char *a[] = { objdump, "-S", obj, 0 };
    char *out = NULL, *e = NULL, *keep, *p;
    size_t kl = 0;
    run_cap(a, NULL, &out, &e); free(e);
    if (!out) return NULL;
    keep = malloc(strlen(out) + 1); keep[0] = 0;
    for (p = out; *p; ) {
        char *nl = strchr(p, '\n'); size_t len = nl ? (size_t)(nl - p) : strlen(p);
        const char *q = p; while (q < p + len && (*q == ' ' || *q == '\t')) q++;
        if (q + 2 <= p + len && q[0] == '0' && q[1] == ':') { memcpy(keep + kl, p, len); kl += len; keep[kl++] = '\n'; }
        if (!nl) break;
        p = nl + 1;
    }
    keep[kl] = 0; free(out);
...
static int suite_armasm(int argc, char **argv)
{
    ...
    char *mnem[8192]; int nm = 0, i, c, total = 0, okc = 0, st = 0, isd;
    char *tok, *text; char *failed[8192]; int nf = 0;

    if (!mcc || !tokh || !builddir) { fprintf(stderr, "usage: mccharness armasm --mcc --tokh --builddir [--cross-compile --vfp]\n"); return 2; }
    ...
    for (tok = strtok(text, "\n"); tok; tok = strtok(NULL, "\n")) {
        char *op, *cp, args[256]; const char *vfp64;
        if (!strstr(tok, "DEF_ASM")) continue;
        if (strstr(tok, "not useful") || strstr(tok, "#define") || strstr(tok, "/*") ||
            strstr(tok, "DEF_ASM_CONDED_WITH_SUFFIX(x")) continue;
        op = strchr(tok, '('); cp = strrchr(tok, ')');
        if (!op || !cp || cp <= op) continue;
        snprintf(args, sizeof args, "%.*s", (int)(cp - op - 1), op + 1);
        vfp64 = strstr(tok, "DEF_ASM_CONDED_VFP_F32_F64");
        if (vfp64) {
            char b[300];
            snprintf(b, sizeof b, "%s.f32", args); if (nm < 8192) mnem[nm++] = strdup(b);
            snprintf(b, sizeof b, "%s.f64", args); if (nm < 8192) mnem[nm++] = strdup(b);
        } else {
            char name[300]; int j = 0, k;
            for (k = 0; args[k] && j < 298; k++) {
                if (args[k] == ',' && args[k+1] == ' ') { name[j++] = '.'; k++; }
                else name[j++] = args[k];
            }
            name[j] = 0;
            if (!arm_isreg(name) && nm < 8192) mnem[nm++] = strdup(name);
        }
    }
    free(text);

    for (i = 0; i < nm; i++) {
        const char *s = mnem[i]; int is_v = s[0] == 'v';
        if (is_v && !vfp) continue;
        for (c = 0; ARM_COMBOS[c]; c++) {
            const char *args = ARM_COMBOS[c];
            char t[512]; char *exp, *got;
            snprintf(t, sizeof t, "%s %s", s, args);
            { char line[520]; snprintf(line, sizeof line, "%s\n", t); write_file(ins, line); }
            { Argv v = {{0},0}; A(&v, as); A(&v,"-mlittle-endian"); if (is_v) A(&v,"-mfpu=vfp"); A(&v,"-o"); A(&v, aso); A(&v, ins);
              if (run_quiet(Z(&v))) continue; }
            total++;
            exp = arm_dump(objdump, aso);
            { char line[560]; snprintf(line, sizeof line, "__asm__(\"%s\");\n", t); write_file(inc, line); }
            { Argv v = {{0},0}; A(&v, mcc); A(&v,"-o"); A(&v, mo); A(&v,"-c"); A(&v, inc);
              if (run_quiet(Z(&v))) { printf("warning: '%s' did not work in mcc\n", t); free(exp); continue; } }
            got = arm_dump(objdump, mo);
            if (exp && got && !strcmp(exp, got)) okc++;
            else { printf("warning: '%s' did not match GNU as\n", t); if (nf < 8192) failed[nf++] = strdup(t); }
            free(exp); free(got);
        }
        free(mnem[i]);
    }
    ...
    for (i = 0; i < nf; i++) {
        if (in_list(failed[i], ARM_KNOWN_FAIL)) printf("Failed test: %s (known failure)\n", failed[i]);
        else { printf("Failed test: %s\n", failed[i]); st = 1; }
        free(failed[i]);
    }
    if (st) { fprintf(stderr, "arm-asm-testsuite: FAILURES\n"); return 1; }
    return 0;
}

/* ------------------------------------------------------------------- */
/* machostructural: link each conformance program for the osx targets and */
/* validate the Mach-O structure + LC_BUILD_VERSION via objcheck           */
/* (ports tests/qemu/validate_macho.cmake)                                 */
...
static int suite_machostructural(int argc, char **argv)
{
    ...
    for (t = 0; TGT[t]; t++) {
        ...
        nf = ts_glob(conf, "*.c", 0, files, 4096);
        for (i = 0; i < (nf < 0 ? 0 : nf); i++) {
            const char *f = files[i], *base = strrchr(f, '/'); char n[256], exe[4400];
            base = base ? base + 1 : f;
            snprintf(n, sizeof n, "%.*s", (int)(strlen(base) - 2), base);
            ...
            { Argv v = {{0},0}; A(&v, mcc); A(&v, Irt); A(&v, f); A(&v,"-o"); A(&v, exe);
              if (compile(Z(&v), &err)) {
                  ...
                  else { char *l = ts_first_error_line(err, NULL, NULL); printf("FAIL %s/%s (link): %s\n", tgt, n, l?l:""); free(l); status = 1; }
                  free(err); free(files[i]); continue;
              }
              free(err); }
            if (obj_is_macho(objcheck, exe)) printf("PASS %s/%s (valid Mach-O)\n", tgt, n);
            else { printf("FAIL %s/%s: objcheck rejected the Mach-O structure\n", tgt, n); status = 1; }
            remove(exe);
            free(files[i]);
        }
        ...
        { Argv v = {{0},0}; A(&v, mcc); A(&v, Irt); A(&v,"-mmacosx-version-min=12.3.1"); A(&v, vsrc); A(&v,"-o"); A(&v, vexe);
          if (compile(Z(&v), &err)) {
              ...
              else { char *l = ts_first_error_line(err, NULL, NULL); printf("FAIL %s/versionmin (link): %s\n", tgt, l?l:""); free(l); status = 1; }
              free(err);
          } else {
              free(err);
              if (obj_minos(objcheck, vexe, "12.3.1")) printf("PASS %s/versionmin (LC_BUILD_VERSION minos 12.3.1)\n", tgt);
              else { printf("FAIL %s/versionmin: minos 12.3.1 not in load commands\n", tgt); status = 1; }
          } }
    }
    if (!any) ts_skip("no osx cross compilers (mcc-<tgt>) for any target");
    return status ? 1 : 0;
}
...
int main(int argc, char **argv)
{
    ...
    if (!strcmp(argv[1], "i386fastcall")) return suite_i386fastcall(argc, argv);
    if (!strcmp(argv[1], "gcctestsuite")) return suite_gcctestsuite(argc, argv);
    if (!strcmp(argv[1], "penative"))     return suite_penative(argc, argv);
    if (!strcmp(argv[1], "qemurun"))      return suite_qemurun(argc, argv);
    if (!strcmp(argv[1], "pewine"))       return suite_pewine(argc, argv);
    if (!strcmp(argv[1], "parts"))      return suite_parts(argc, argv);
    if (!strcmp(argv[1], "mcctest"))    return suite_mcctest(argc, argv);
    if (!strcmp(argv[1], "mccexe"))     return suite_mccexe(argc, argv);
    if (!strcmp(argv[1], "asmconnect")) return suite_asmconnect(argc, argv);
    if (!strcmp(argv[1], "dashs"))      return suite_dashs(argc, argv);
    if (!strcmp(argv[1], "preprocess")) return suite_preprocess(argc, argv);
