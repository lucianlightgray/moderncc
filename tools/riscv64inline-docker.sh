...
# its ABSENCE for the >16B struct that must stay retained. objdump -dr then
...
# memcpy), which the Tier-4 capture does not model, so sumbigf must stay
# retained-only, mirroring arm64 sumbig.
marker_graft sumbigf  no  "(32-byte struct-by-value: indirect ABI, not modeled)"
if grep -q "\[ast-inline\] candidate sumbigf .*retained-only" /w/gatedump.txt; then
  echo "   OK   sumbigf : retained-only candidate marker present -- matches arm64 sumbig"
else
  echo "   MISS sumbigf : expected retained-only candidate marker"; GRAFT_OK=0
fi
echo "-- objdump: residual R_RISCV_CALL reloc to each grafted callee (must be 0) --"
for pair in "c_scalar addf" "c_scalar scalef" "c_float areaf" "c_smallstruct sumpair" "c_pointer derefsum"; do
  set -- $pair
  rc=$(call_reloc "$1" "$2")
...
$OD -d /w/tested.o | grep -Eo "<(addf|scalef|areaf|sumpair|derefsum)>" | sort | uniq -c || echo "   (none: fully grafted)"
