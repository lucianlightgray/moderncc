...
# repo sources inside a linux/amd64 container (an amd64 host binary that EMITS
...
# --- single linux/amd64 container: build the optimizer-enabled riscv64 cross mcc
#     (amd64 host binary emitting riscv64), compile the soak with divmagic, confirm
#     it fired, link with the riscv64 cross gcc, and run under qemu-riscv64. ---
echo "== docker linux/amd64: build optimizer-enabled riscv64 cross mcc + soak + qemu run =="
dg_docker run --rm --platform linux/amd64 \
	...
	cd /b
	echo "-- building mcc-riscv64-opt --"
	gcc -O1 -w -DMCC_CONFIG_OPTIMIZER=1 -DMCC_TARGET_RISCV64=1 \
		...
		src/mcc.c -o /w/mcc-riscv64-opt
	echo "-- generating soak --"
	bash /w/gen.sh /w/soak.c
	echo "-- compiling soak with divmagic (MCC_AST_DIVMAGIC=1, -O2) --"
	MCC_AST_DIVMAGIC=1 /w/mcc-riscv64-opt -O2 -I /b/runtime/include -c /w/soak.c -o /w/soak.o
	echo "-- confirm divmagic fired (native riscv64 high-multiply) --"
	nmulhi=$(riscv64-linux-gnu-objdump -d /w/soak.o | grep -Ec "mulh|mulhu|mulhsu" || true)
	ndiv=$(riscv64-linux-gnu-objdump -d /w/soak.o | grep -Ecw "div|divu|divw|divuw|rem|remu|remw|remuw" || true)
	echo "   riscv64 mulhi(mulh/mulhu/mulhsu)=$nmulhi residual(div/rem)=$ndiv"
	if [ "$nmulhi" -lt 20 ]; then
		echo "FAIL  divmagic did not fire (expected riscv64 mulhi >= 20)"
		exit 2
	fi
	echo "-- link (riscv64 static) + run under qemu-riscv64 --"
	riscv64-linux-gnu-gcc -static /w/soak.o -o /w/soak
	rc=0
	qemu-riscv64-static /w/soak || rc=$?
	echo "soak exit=$rc"
	exit $rc
'
