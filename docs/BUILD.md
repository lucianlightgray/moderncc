...
plain-Release test run) and `ast` (the only `MCC_CONFIG_OPTIMIZER=OFF` build) — plus `matrix`
...
rest (`debug`, `sanitize`, `diagnostics`, `cross`) are **aliases**: they
...
(`local-ci` likewise — it *is* the orchestrator; `cst` is the parity-exempt
`MCC_CONFIG_LSP=OFF` feature-off axis):
...
| `cst` | Debug | `MCC_CONFIG_LSP=OFF`: the feature-off axis for `--lsp` capture — no CST subsystem, no `tests/cst` suite; `--lsp` errors out |
| `ast` | Debug | `MCC_CONFIG_OPTIMIZER=OFF`: the feature-off axis for the optimizer — no AST intention-IR subsystem, no `asttool` pure-lib suite or `tests/exec` `-O1-replay` column; `-O1+` degrades to `-O0`-equivalent output |
...
| `MCC_CONFIG_LSP` | BOOL | **ON** | | always (`GROUP "Advanced"`) | Enable `--lsp` capture: builds the CST database subsystem (side-recorded concrete syntax tree; LSP/`-g`/opt substrate) → `MCC_CONFIG_LSP=1`. On by default; codegen is byte-identical either way (guarded by the codegen-identity gate); OFF makes `--lsp` error out. The `cst` preset is the OFF axis. |
| `MCC_CONFIG_OPTIMIZER` | BOOL | **ON** | | always (`GROUP "Advanced"`) | Enable the optimizer: builds the AST intention-IR subsystem (MCC.md §9 / EXCESS.md) → `MCC_CONFIG_OPTIMIZER=1`. A pure side-channel like the CST: `-O0` never builds it and stays byte-identical; `-O1` lowers it and replays through the vstack API; OFF degrades `-O1+` to `-O0`-equivalent output. The `ast` preset is the OFF axis. |
