# ARCHIVED — TODO.md history and landmines, pending validation

**Snapshot of `docs/TODO.md` taken 2026-08-05 at `9b83dc05`, before TODO.md was trimmed
to instructions + present-tense open items.**

This file is a validation backlog, not living instructions. It contains, intermixed:

- **Completed / historical / superseded content** — landed-fix write-ups, dated status
  snapshots, past measurement boards, and internal contradictions (e.g. `MCC_RIR_ONLY`
  and `RIRC2TREE` are described here as live but were deleted by the RIR cut's P5; many
  `src/*.c:NNNN` anchors have drifted ~1000–1900 lines after merges). This is to be
  **validated and then purged**.
- **"Landmines"** — `do not re-diagnose` / `do not re-bank` / `why this is not a bug` /
  harness-and-measurement traps / deliberate-red warnings. These are to be **validated
  and then either promoted back into TODO.md or kept here** as durable cautions.

The still-open, actionable subset was distilled into `docs/TODO.md`'s
"Present-tense open items" section (validated against the tree at 9b83dc05). When
validating an entry here, cross-check it against the code by *symbol*, not by the line
...
| W8 | **`selfhost-jit` faults `0xC0000374`/`0xC0000005` on x86_64 Windows** — **root-caused 2026-08-05; fix still open** | The faulting allocation is now named, and it is not a JIT/KGC codegen bug at all — it is a **heap-use-after-free of a `Sym`** in the AST forward-inline re-emit path. ASan (the `mcc_s` MSVC-ASan binary from `cmake-sanitize-msvc`, which embeds the JIT) turns the nondeterministic `0xC0000374`/`0xC0000005` into a deterministic report every run: **UAF READ of 4 bytes at `src/mccgen.c:2133` in `gaddrof`**. The 176-byte region is a `Sym` allocated by `sym_malloc`←`sym_push2`←`macro_subst_tok` (`src/mccpp.c:4865`), **freed by `ast_func_end` (`src/mccast.c:17011`, the deferred-sym retirement at 16998–17014)**, then **read after free by `ast_reemit_forward_inlines` (`src/mccast.c:17140`) → `ast_reemit` → `ast_replay_body` → `ast_replay_bb` → `ast_inline_graft` (`:2939`) → `ast_replay_value` → `gaddrof`**. So the Windows-only manifestation was a red herring: in a non-DIAG release build the retired `Sym` goes onto `sym_free_first` and is *recycled* rather than freed, so the corruption only shows once the slot is overwritten — hence `0xC0000374` vs `0xC0000005` by luck of heap reuse, and hence green on Linux where the reuse timing differs. **Fix direction:** a `Sym` referenced by a *forward-inline* re-emit target must be retained (added to `ast_sym_retained`), not dropped, when `ast_func_end` decides `!keep_inline && !keep_reemit && !keep_baseline` — i.e. `ast_reemit_retain`/`ast_inline_retain` are under-counting cross-function graft references. Left unfixed here because it is a codegen-lifetime change that needs the full suite as a regression gate, not just the ASan oracle; but the deterministic ASan repro (`mcc_s` + `tools/selfhost-jit.py`) is now the verification harness for whoever takes it. `tools/selfhost-jit.py` still SKIPs both exit codes. |
