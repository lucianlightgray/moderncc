...
The live task tracker moved to **[TODO.md](TODO.md)** — the single home for all
remaining, incomplete work (optimizer-ladder residue, sanitizer follow-ups,
CI/hardware-blocked validations, conformance test-depth, CST slice-G / `H_e`,
and the parked/revisit-trigger and ACHTUNG lists). The `-O1` soundness backlog
and `-O1` wiring that lived here are both closed (see EXCESS.md's AST notes).
