...
| x86_64-osx | **NO** | reader done. NEW BLOCKER FOUND on real Apple-Silicon hardware (2026-07-29, IN PROGRESS): the JIT engine DOES build here (`cmake-debug` bakes `mccjit_blob.c` + ASan `mcc_s`), but macOS mcc cannot self-compile at all at -O2+ — see the arm64-osx detail row + the sym-pool-corruption item below |
