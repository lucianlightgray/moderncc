...
| lin-x64   | Linux    | x64   | 10000–29999 | 10384   | 2026-08-15T15:46Z |
=======
...
- [ ] T-lin-10041 [X] mac-arm64 — the native MSL region arm
      OWNER: mac-arm64 | STATE: CLAIMED | SHA: b3da6a4a | TS: 2026-08-15T15:25Z
      REF: DETAILS.md#t-lin-10042-slice-7-msl-regions-complete-m4-m5 | DEPS: — | NOTE: subject delivered under T-lin-10042 slices 5-7 (same M-TODO-0051 migration, per anti-redundancy); claiming to verify against ITS spec and close on the shared full-native-suite run now in flight
...
- [x] T-lin-10383 [S] Run the self-hosted JIT always-GPU with the refusal trace, enumerate every AST/RIR GPU coverage hole, and mint a fix task per hole
      OWNER: lin-x64 | STATE: DONE | SHA: dc1e52a8 | TS: 2026-08-15T16:12Z
      REF: DETAILS.md#t-lin-10383-census-results-the-spir-v-arm-histograms | DEPS: — | NOTE: DONE, both halves, SPIR-V arm. Census non-vacuous everywhere (forced=1, dispatches>0, device RTX 5070 Ti idle): subject.c + full_language.c (-O1 and -O4) + src/mcc.c self-host (--embed-jit -O1, 244,838 rungs / 489,677 dispatches / 4.73e9 lanes) + ALL 312 tests/exec files. THE EMITTER HISTOGRAM HAS EXACTLY ONE NON-ZERO ENTRY across every subject: by-node Unary=81 / by-op member=81 (same 81, subject.c only) → taskified as T-lin-10384. Ladder layer refused=0 on everything except subject.c's emit-lhs=66/emit-rhs=15 (the same 81). MSL arm structurally uncensusable (no refuse counters compiled on MCC_GPU_LANG_MSL) → T-lin-10385 [X] mac. En-passant: MCC_DEV=ON cleanup.c compile cost → T-lin-10386. Honesty qualifiers (129/311 exec files fire rungs=0; pre-fit rejects invisible to both layers; by-op caps at 24) at the REF. No code commits — suite state stands at T-lin-10092/lin's clean number
...
- [ ] T-lin-10384 [S] The one emitter hole the census found: the SPIR-V ladder emitter refuses struct member select (`Unary` + `AST_OP_MEMBER`)
      ...
      REF: DETAILS.md#t-lin-10383-census-results-the-spir-v-arm-histograms | DEPS: — | NOTE: the census's single non-zero by-node/by-op entry pair, filed once per the T-lin-10383 rule. Evidence: 81 refusals on tests/smoke/subject.c -O4 (the only subject that fires it; ladder-layer view emit-lhs=66/emit-rhs=15), zero on src/mcc.c self-host / full_language / all 312 tests/exec files. WHY REFUSED: spv_expr has no arm for AST_OP_MEMBER — falls to spv_refuse (mccgpu.h:3625). LOWERING NEEDED: struct-typed livein layout to the device + byte-offset member load; candidate reuse is the slice/region byte-addressed load/store machinery (backend_has_regions now unconditional on both arms after T-lin-10042 slice 7). WHAT IT MOVES: closes the only emitter hole in the pair population; per T-lin-10040 the break-even table is this hole's own pricing argument — price before building. First slice = confirm the node/operand shape at the 81 sites and whether struct liveins even pass the ladder's livein-type check
