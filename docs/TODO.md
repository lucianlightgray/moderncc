...
| win-x64   | Windows  | x64   | 50000–69999 | 50022   | 2026-08-15T21:21Z |
...
- [ ] T-lin-10374 [S] Two builds of identical mcc source do not produce identical binaries (win-x64 via WSL Ubuntu)
      OWNER: win-x64 | STATE: IN_PROGRESS | SHA: 8dd00e11 | TS: 2026-08-15T20:30Z
      REF: DETAILS.md#t-lin-10012-the-binary-diff-instrument-that-did-not-work | DEPS: — | NOTE: CLAIMED (win-x64 working Linux tasks via WSL Ubuntu 26.04, native gcc build in ~/mcc). FIRST-SLICE progress: mcc's x86_64 codegen is BYTE-DETERMINISTIC — proven: `mcc -c src/mcc*.c` identical at -O0/-O1/-O2/-O2-finline; FULL self-host compile+link `mcc src/mcc.c -o mcc` identical at -O0/-O2/-O2-finline/-O4 (reemit optimizer active). The native gcc-built mcc differs by only 25 bytes @921 (gcc `.note.gnu.build-id`, benign). So the 1,104,183-byte mcc-arm64 diff is NOT mcc's core x86_64 codegen — it is cross/arm64-target-specific. REMAINING: build mcc-arm64 in WSL, diff two builds, pin whether it's the arm64 backend, DWARF (paths/timestamps), or the cross build config; then fix or bank
