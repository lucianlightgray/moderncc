...
| x86_64-osx | **NO** | the Mach-O READER is no longer the blocker — plain objects, `llvm-ar` archives and relocations (incl. GOT + SUBTRACTOR) all landed on Linux and are covered by 7 ctest cells. What is left is the RUNTIME JIT itself: it is unavailable in every local build, and building the JIT engine for an apple triple needs the macOS SDK. See P0 step 1 |
| arm64-osx | **NO** | same as x86_64-osx: reader done, runtime JIT unavailable in every local build |
