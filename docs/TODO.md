...
| arm64-osx | **NO — IN PROGRESS on native Apple-Silicon 2026-07-29** | The stale "runtime JIT unavailable / needs macOS SDK" claim is WRONG on this Mac: `cmake-debug` builds the embedded JIT blob AND an ASan `mcc_s`. The live blocker is more fundamental: **plain `mcc -O2 -c src/mcc.c` (no --jit) heap-corrupts** (`selfhost-jit`'s crash is just downstream of it). -O1 clean; -O2/-O3/-O4 SIGSEGV. Linux self-compile is fine (selfhost-fixpoint passes) — macOS-only, layout-dependent. Root localized: an errant write puts 0x30001F into `mcc_state->nb_sym_pools` (MCCState+2400) with `sym_pools`/`sym_free_first` nulled, inside `ast_func_end` while processing `gjmp_append` (between ast_func_end-IN clean and fe-A-before-retain corrupt — i.e. the replay / mccjit_embed_note+get_sym_ref / ast_arena_clone specialization path). Bisecting that range to the exact writer. NOT double-defer (idempotent ast_sym_defer did not fix it). |
