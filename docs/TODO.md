...
- **Trunk-based: no branches, no PRs.** Commit a verified fix and `git push origin main` directly, without asking — CI on main is the gate and the iteration loop is push-then-read-the-run. Split unrelated fixes into separate logical commits. If the push is rejected, `git fetch` + `git rebase origin/main`, then push again; main advances frequently from other sessions.
- **Never pre-judge an optimization out of scope.** No technique is dismissed a priori ("micro-opt", "the tables are tiny", "won't pay off") — implement it and measure it. The recorded pattern is that a pass looks inert only because an *enabling* gate is off: every candidate measured `desync` until `MCC_AST_OPASSIGN` unblocked the whole hot-loop story. Frame measurement as an **ever-fires ledger**, not a payoff gate: report whether a technique EVER changes the AST / finds a promoted match, at function, TU and whole-corpus scope. "Fired but small here" and "never fired anywhere" are different verdicts, and only the second is a reason to change course — toward fixing the *enabler*, not deleting the technique. A never-firing technique is a named, surfaced gap to refine (better query/reconciliation, wider vocabulary, a missing gate), never silently pruned. Encoded in the ever-fires bullet under Strategy scheduler and the handle-reduction bullet under JIT runtime.
- Standing reference — recipes, per-subsystem state, and the gotchas that cost time — lives in **"Consolidated reference"** at the end of this file (42 formerly-separate notes merged in 2026-07-30). Read it before re-deriving a gate recipe, a self-host/cross-arch procedure, or the state of the JIT, the recorder gates, or the search substrate.
...
| x86_64-osx | **NO — self-compile bugs FIXED, JIT verify HW/Rosetta-blocked** | The two macOS self-compile miscompiles (storeval-arg vstack underflow b6af0f7f, RANGE landor-invert 2c88e797) were arch-independent AST bugs, so they fix x86_64-osx too; validated on x86_64-linux (CI) + arm64-osx (selfhost-jit passes). But `selfhost-jit` for x86_64-osx can't be verified on THIS arm64 host: an x86_64-Darwin mcc runs under Rosetta 2, but Rosetta hangs executing dynamically-generated x86_64 code (JIT / `-run`), and even AOT self-compile is impractically slow. Needs real x86_64-macOS HW to close. See arm64-osx row. |
| arm64-osx | **selfhost-jit PASSES 2026-07-29 (in-process JIT self-host at parity)** | Two separate self-compile bugs found on real Apple Silicon, both ROOT-CAUSED + FIXED; `selfhost-jit` (`--jit -O4 -run src/mcc.c`) now OK, JIT output byte-identical to AOT. (1) **the -O2+ self-compile heap corruption is FIXED.** It was NOT a JIT bug — plain `mcc -O2/-O3/-O4 -c src/mcc.c` SIGSEGV'd. Root: the `MCC_AST_STOREVAL_CALL` replay (`ast_replay_value` AST_Invoke live-arg) does `vrotb(3)`/`vswap()`, but the storeval-arg admission (`ast_finalize_storevals`) is a structural AST walk that cannot see the replay value-stack, so for `gjmp_append` a `vrotb(3)` ran at value-stack depth 1 and underflowed — and `vstack` (`gen_vstack+1`) sits right after `nb_sym_pools`/`sym_pools` in MCCState, so `vtop[-2]` silently overwrote them (→ crash in `__sym_malloc`). Fix: guard the rotation on actual depth and bail to byte-faithful baseline (setjmp) if short. Verified: macOS -O2/-O3/-O4 self-compile now clean; Linux `selfhost-fixpoint` byte-identical at -O2/-O3/-Os/-O1/gates (guard never fires there). The stale "runtime JIT unavailable / needs macOS SDK" claim was also wrong — `cmake-debug` builds the embedded JIT blob + ASan `mcc_s`. (2) **FIXED — the `intr()` assertion was an `MCC_AST_RANGE` miscompile, NOT a JIT bug.** `--jit -O4 -run` (also `MCC_JIT=0`) aborts in `intr` (`arm64-gen.c:71`) because the -O4 self-compiled mcc mis-emits `intr`'s `assert(0<=r && r<=19)`. Root: `ast_range_try` folds `lo<=x && x<=hi` to `(unsigned)(x-lo) <= span` (TOK_ULE) but wiped the `AST_FB_LANDOR_INVERT` bit the parser sets for the assert's `!(...)`; the folded plain-comparison replay path (unlike the landor path at ast_replay_value ~5870) does not honor that bit, so the branch inverted — asserting when IN range (r=0 → abort). Fix: bake the negation into the op, folding to `TOK_UGT` (=ULE^1) when the invert bit is set. This class was invisible to selfhost-fixpoint (byte-identity passes a STABLE miscompile) and only `selfhost-jit` (which RUNS the compiled mcc) caught it. Verified: selfhost-jit rc=0 (JIT≡AOT), macOS exec 100%, Linux fixpoint byte-identical -O2/-O3/-O1/-Os, differential vs gcc on inverted+plain int/long range folds = OK. The OR/out-of-range fold form (`ast_range_try_lor`) had the same dropped-invert bug — fixed too (commit 311767c7). **Test-gap lesson:** selfhost-fixpoint's byte-identity can never catch a STABLE miscompile, and it can't add a -O4 leg (the -O4 search is legitimately non-reproducible, o1!=o2). `selfhost-jit` (`--jit -O4 -run`) is the only thing that RUNS a self-compiled mcc, and it caught this — but only at -O4; a stable -O2/-O3-only self-compile miscompile would still slip both. A non-JIT `-O2 -run` correctness leg (compile+run a small workload with the mcc-built mcc, compare to the stage0 mcc) would close that. |

**Ordered plan — each step unblocks the next, and the first two are the widest wins:**
1. **Mach-O `.o`/archive/relocation READER — landed on Linux; the only remainder genuinely needs macOS:** validation against a real `libmcc_jitengine.a` and a real-world `clang -c` corpus, because building the JIT engine for an apple triple needs the macOS SDK headers. That is also the only place `--embed-jit` can be exercised for the osx triples. `TLV` and `POINTER_TO_GOT` stay deliberate refusals (no sound ELF image), and scattered relocations do not exist in these two ABIs, so refusing them is the whole correct implementation, not a gap. The reader traps worth re-reading before touching it are in the guards section at the end of this file.

   *(Steps 2–4 are DONE and pruned — riscv64 + arm-linux JIT run/verify across the whole ELF family, JIT/AOT arena-normalization alignment so both paths agree on `ast_intention_hash`, and the i386 stub-tail flip making `MCC_JIT_I386_STUBS` default-ON with native-ELF backing. Detail in git history; the numbering below is kept so cross-references to "P0 step 5" still resolve.)*
...
## Latent self-miscompile — `.data.rel.local` (RELRO) synthesis in the ELF **executable** linker (OPEN, not root-caused)

A gcc-built mcc and an mcc-built mcc, given byte-identical inputs, produce DIFFERENT linked
executables. The gcc-built mcc emits a separate `.data.rel.local` section (8 bytes = one
pointer); the mcc-built mcc folds those bytes into `.data` (`.data` grows by exactly 0x8). Every
other byte diff cascades from that one section shifting all addresses.

Codegen itself is fine — `tools/selfhost-fixpoint.py` compares `.o` objects and passes. The
divergence exists only in the linked *executable*, which that script never compares;
`tools/fixpointgate.c` (ctest `fixpoint-invariant`) compares executables and is what catches it.
mcc's source contains no `.data.rel.local` string: the section is SYNTHESIZED by the linker from
data carrying relocations to local symbols, and that synthesis behaves differently when the
linker itself was compiled by mcc rather than gcc. So this is mcc miscompiling mcc's own linker,
not a linker design bug.

It surfaced only when linking the larger cmake-cross `<arch>-libmccrt.a` runtime (never the
native `libmccrt.a`), found 2026-07-27 while fixing the `linux-gcc-cross` `fixpoint-invariant`
break caused by the archive-selection change below. **That CI break was worked around at the
archive-selection layer; this miscompile was NOT fixed and remains latent.** Root-causing which C
construct in the linker mcc miscompiles is the correct-but-deep fix. Same family as the win32
ratchet nondeterminism under Tests / infra (an mcc-built-by-mcc binary that differs run-to-run),
and the same theme as the FP/compute section: mcc's own codegen is the frontier.

### Runtime-archive selection (`mcc_add_support`) — the rule, and why it is shaped that way
`mcc_add_support` (`src/libmcc.c`) resolves the runtime support archive when two candidates sit
side by side in a `-B` dir: the plain `[<crossprefix>]libmccrt.a` and the cmake-cross-staged
`<arch>[-<os>]-libmccrt.a`. **The rule (as of 2026-07-27):** prefer the PLAIN name WHEN IT IS
ARCH-CORRECT — decided by `mcc_support_arch_match` (`mccelf.c`), a pure read-only ELF `e_machine`
probe, never a load attempt, because loading a wrong-arch member trips `mcc_error_noabort` from
inside and the resulting `nb_errors != 0` fails the whole link. Then add the arch-tagged archive
as an à-la-carte SUPPLEMENT (it pulls only still-undefined symbols, so a complete plain archive
makes it inert). If plain is absent or wrong-arch, use the arch-tagged one; plain-with-an-error is
the last resort.

Two opposing scenarios force that dance, both with an empty `MCC_CONFIG_CROSSPREFIX` and the same
target arch:
- **Native self-host fixpoint** (`fixpoint-invariant`, cmake's own build dir): the plain native
  `libmccrt.a` is authoritative and complete; linking the arch-tagged cross build instead diverges
  from self-host byte-identity and exposes the `.data.rel.local` miscompile above. Wants PLAIN.
- **Stage-built cross mcc** (`tools/selfhost-cross-native.sh`, empty crossprefix, `-B` pointed at
  a multi-target staging dir): on an x86_64 host the plain `libmccrt.a` is the HOST arch, wrong for
  an arm64/riscv64 target — the "invalid object file" + unresolved `__ashldi3`/`__clear_cache`/
  `__floatunsitf` trap. Wants ARCH-TAGGED.

`fd108bd4` first tried "arch-tagged FIRST" and broke the native fixpoint; plain-first-when-arch-correct
plus the supplement satisfies both. Note for future readers: where host==target (e.g. arm64 mac),
`selfhost-cross-native.sh` with STALE cmake-cross archives segfaults at stage2 — that reproduces on
pristine main too, so it is stale-archive rot, not this logic. That test is SKIP (77) on CI (no
`cmake-cross` dir, no qemu, no sysroot on the runners).
...
2. **Ternary-statement replay residue.** `MCC_AST_TERNARY_DISCARD` records a discarded ternary and re-tags it op 7, so the statement dispatcher replays value+`vpop` and int-arm ternaries are faithful (the op-5 collision with a condless for-loop was the original degenerate-replay cause). **Open:** VOID-arm ternaries take a depth-guarded discard emitter and verify honest `unfaithful`, pending a byte-mirror of the parser's void join. `(void)(c ? 1 : 2)` stays `empty` by design (it constant-folds, no `AST_If` is built).
...
### 1. D1a — closed
The verdict count is trustworthy; the 1856 → 1841 drop does not reproduce (re-confirmed 2026-07-29 on a much-changed gate set). Do not re-add "still open" duplicates of it.

### 2. B1a — closed, but this is the region that miscompiled
All three assignment-as-value shapes are faithful (`MCC_AST_CHAINSTORE` staged at `-O2`; `ast_hook_vpop` attaching the `AST_If` a ternary pushes; `MCC_AST_STOREVAL_CALL` for the call-argument path). Two alternatives are NOT viable and should not be re-tried: emitting nothing at the marker leaves `gfunc_call` short an operand (its own comment says so), and re-emitting the RHS duplicates the call — measured, and it is how the shape became `unfaithful` in the first place. Reloading from the lvalue is semantically right but emits a load where the parser reuses a register, trading one unfaithful form for another.

**`emit-at-marker` here was correct at `-O0`/`-O1` and MISCOMPILED at `-O2`/`-O3`, caught only by `assign_value_effects.c` counting evaluations.** Gate any further change in this region on that file plus the full bar.

### 3. A2a — the SYNC site is small but NOT closed
`ast_hook_vpush`'s `ast_vn != rel - 1` guard holds **5** events as of 2026-07-29 — `parse_number`, `gen_opif`, `combine_types`, `set_symbol`, `layout_sections` — and they report **`desync`, not `bail`**, so they are not free: a desync means replay cannot be used for that function. The site is a **DETECTOR, not the culprit** — the drift happens earlier and the next push is merely the first thing to check the invariant. Capacity is not the problem (`rel > AST_VS_MAX` was 0 of 43, so do NOT raise `AST_VS_MAX`), and the value being pushed when it fires is overwhelmingly a CALLEE symbol.

**The rule every fixed pop-imbalance violated: a hook that models a codegen pop must account for it on EVERY exit, bail included.** `ast_bail` does not stop the recorder's stack bookkeeping — it only marks the function un-replayable — so an early return that skips the decrement silently converts a clean `bail` into a `desync`, attributed to whatever pushes next.

**Empirically NOT the cause, so do not re-test:** early `return` in a branch, a `noreturn` call in a branch, a noreturn call followed by more statements, deref-after-check, `c ? g() : 0`, and discarded `(void)x` casts.

**The `&&`/`||` identity-constant region is the remaining model gap.** For the IDENTITY constant (`1 && X`, `0 || X`) the parser drops the constant and evaluates every remaining operand, so each middle operand IS consumed and `expr_landor`'s const path decrements for none of them; only the SHORT-CIRCUITING constant (`0 && X`, `1 || X`) genuinely skips the rest. What landed is `ast_lor_consumed[]` — desync at the SECOND consumption inside a const region, the first moment a middle operand is dropped (SYNC-site desyncs 23 → 10, totals unchanged, only attribution moved). **Three attempts are recorded so nobody repeats them:** (a) *decrement the middle operand and continue* — counts balance and `ast_replay_ok` passes, so replay RUNS on a model missing an operand → immediate SEGV in `ast_replay_bb`→`gfunc_return`→`gvtst`; a count fix is not a model fix, the operand has to become a child of the region's Binary rather than vanish. (b) *desync the whole identity-constant case at the first operand* — costs 17 previously-faithful functions; the 2-operand shape (`1 && x`) models correctly today and must keep working. (c) *model the identity constant as an ABSENT operand, or decide the region's kind (Binary vs Literal) at the END* — both tried: every isolated shape becomes faithful (including `1 && x`, which no other attempt fixed) but mcc's own TU nets +2 faithful with 15 NEW desyncs inside the deferred operand path itself (`ast_vn < 1`), and individual functions move both ways. **So the region's KIND was never the blocker — stop optimising the region representation and go find out why an operand arrives with `ast_vn < 1`.**

Two sub-cases stay open under any representation: a later SHORT-CIRCUITING constant (`x && 0`, 16 events — the parser folds the whole expression from that point, `nocode_wanted++`, so the region stops being a Binary and the model has no way back) and a materialized ending (20 events — `expr_landor` finishes with `if (cc || f) { vpop(); vpushi(i ^ f); }` while `ast_in_call` is still 1, so those pushes are invisible to the model and `ast_hook_landor_end` would have to synthesize the Literal itself).
...
## `-O1`'s `atoi` out-of-line copy — open (the `-O1` restaging itself is done)
`-O1` was `-O0` plus twelve inert gates; `MCC_AST_REGDISP` and `MCC_AST_OPASSIGN` were restaged to `optimize >= 1` (nbody 4.71G → 3.885G instructions, −17.5%, every other kernel unchanged), and the missing `exec-O1` and `-O1` 3-stage-fixpoint lanes were added (314 cells passing; o1==o2==o3 at 6381471 B). **Do not re-run the per-gate `-O1` sweep** — every `-O2`-staged candidate was forced on individually and only those two measure non-negative everywhere; `PROMOTE` and `DIVMAGIC` each cost spectral real instructions in isolation, and nothing recovers matmul's or spectral's `-O2` win at `-O1` because the gates interact. Making `-O1` reach `-O2` would mean making it `-O2`.

**Still open, and a separate change: the `atoi` out-of-line copy.** mcc defines `__OPTIMIZE__` from `-O1` up, which turns on glibc's `__USE_EXTERN_INLINES`, and `<stdlib.h>` then supplies an `extern __inline __attribute__((__gnu_inline__)) atoi` wrapping `strtol`. mcc turns that into `VT_STATIC | VT_INLINE` (`mccgen.c`, the `(VT_EXTERN | VT_INLINE)` branch of `decl`) and emits it OUT-OF-LINE into `.text`, so an un-inlined call reaches a LOCAL copy; gcc emits no out-of-line definition at all and lets the call bind to libc. Proof it is `atoi` rather than optimizer output: `-O1 -D__NO_INLINE__` gives `.text` **425 on nsieve, exactly `-O0`'s 425**, and the `-O0`/`-O1` disassembly diff is one added 16-instruction `atoi` body plus `call atoi@plt` becoming a local call — three unrelated programs growing by the same 53 bytes was the tell. Fixing it removes ~52 bytes of dead `.text` from every `-O1`+ TU that includes glibc headers, but it changes identifier binding and can turn a today-working link into an unresolved reference wherever the external definition does not exist, so it wants its own change and its own validation run. It is also a `__gnu_inline__` semantic deviation (gcc emits NO out-of-line copy).

## The seven `o4` math-inline gates that measure as no-ops
**Do not re-run the `o4`-gate mis-staging sweep** (done 2026-07-29 — `REGDISP` was the only mis-staged gate and it is landed); re-run it only if the gate list changes. `IVSR_PTR` is correctly `o4`-only (matmul +17% WORSE), and `PROMO_LEAF_CALLEE`/`PROMO_LEAF_XMM` measured not worth restaging. **The interesting residue is the seven gates that changed NOTHING**, on five kernels and on mcc's own TU. A direct probe (`sqrt`/`copysign`/`fmin`/`fmax`/`fma`/`fabs`/`floor`/`ceil`, one call each) shows `COPYSIGN_INLINE=1` DOES fire — 7 calls drop to 6 — while `MATH_INLINE`, `MINMAX_INLINE`, `FMA_INLINE`, `MATH_INLINE_PREPASS`, `FMOV_IMM` and `XMM_HI` change nothing even with `-fno-math-errno`, and `-O4` itself only ever removes that same one call. gcc at plain `-O2` inlines `sqrt`, `copysign` AND `fma`, and with `-ffast-math` all eight. So mcc emits libcalls for `fmin`/`fmax`/`fma`/`ceil`/`floor` that gcc does not, and the gates named after those transforms are either unreachable or guarded by a condition no ordinary call meets. Find out which before anyone counts them as implemented features.

## `MCC_AST_REGDISP` — open follow-up
Staged at `-O2` and it fires on exactly one of five kernels. `ast_regdisp_env`'s guard (`!(vtop->r & (VT_SYM | VT_LVAL | VT_MUSTBOUND | VT_BOUNDED))`) is a wide exclusion — a transform this cheap probably should apply more often, so find out why it misses the other four hot loops. x86_64-only by construction; `MCC_AST_REGDISP=0` restores the old staging.
...
**Work.**
1. **REMAINING — and it is the blocked half.** Have the JIT enumerate candidates from
...
**Constraint on every fix in this section — the compile-speed moat.** mcc compiles its own ~100k-line amalgamation about **24× faster than clang -O2** (2.2 s vs 52 s), and flat across `-O0..-O3` (0.47 s CPU). That is the project's actual advantage over clang/gcc, so a runtime fix must not spend it: favour a **linear-scan** promotion pass over AST-arena vregs, not clang-style graph colouring. Also worth keeping from the arm64-macOS profiling that opened this section (2026-07-26, vendor/plb kernels, min-of-5 vs Apple clang `-O2`): mcc's `-O0`/`-O1`/`-O2`/`-O3` were all within noise **of each other**, i.e. the AST optimizer gave ~0 hot-loop speedup and the gap is the backend — nbody's inner loop was 221 loads / 58 stores against clang's 29 / 9. Enabling hardware `sqrt` inlining there emitted `fsqrt` but did NOT speed nbody up, which is the cleanest proof that register allocation, not instruction selection, is #1.
...
**Prerequisite status:** `-ffunction-sections` and `-fdata-sections` are implemented (2026-07-29), so an external `--gc-sections` already drops unreferenced statics (demonstrated on `unused_helper`) and an internal collector becomes a section-drop rather than byte surgery inside one `.text`.
...
| how `sq` is written | relocs | |
|---|---:|---|
| `static int sq(int)` | 0 | **inlined** |
| `static inline int sq(int)` | 2 | not inlined |
| `inline int sq(int)` | 2 | not inlined |
| `extern __inline__ __attribute__((__gnu_inline__)) int sq(int)` | 2 | not inlined |

`static inline` is the ordinary way to write an inlinable helper in C and the dominant form in headers, so this is
not a corner: it is the common case that fails while the less-idiomatic one works. Confirmed to be the parking
mechanism, not a cost model — `nm -n` puts `use` at 0 and `sq` at 0x3d, i.e. the body is generated at END of TU,
long after its callers, so `ast_inline_capture` never has it in the pool. Same root cause the `-fc99-inline-body`
note already records for plain `inline`.

**The fix mechanism already exists and is proven — it is just not applied to this case.** `-fc99-inline-body`
generates a plain-`inline` body in place instead of parking it, and with it `inline int sq(int)` goes to **0
relocs, inlined**. The same flag leaves `static inline` at 2 relocs, because its guard is
...
## Backend intrinsic lowering — two open items
...
- Shifts — **CONSTANT-count case DONE + DEFAULT-ON 2026-07-30 (`ac997715` emitter, flip in a
  ...
  identity early-out (in the commit). **(a) the default-ON flip is done and CI-green.** **(b) VARIABLE-count case — STILL A HELPER
...
- **THE RATCHET IS NOT SELF-HOST COVERAGE — run `selfhost-fixpoint` before flipping any gate default-on.** The AST recorder-fidelity ratchet (`ast-verify`/`verify-baseline`) and the byte-mirror `fixpoint-invariant` test do NOT run a 3-stage self-host, so flipping an AST gate default-on at `-O2+` can bank the ratchet green while the gate miscompiles mcc's OWN TU: the mcc-built mcc (stage2 `mcc1`) then aborts compiling `src/mcc.c` (arm64 SIGABRT / x86_64 SIGSEGV), reddening every Linux `selfhost-fixpoint{,-O3,-Os,-gates}` cell while `-O1` and the ratchet stay green. Different coverage — the ratchet measures record/replay fidelity on a corpus, not that mcc can still build itself. Concrete case (2026-07-29): `MCC_AST_WHILE_COMMA`'s loop-condition *prefix-store* admission (`c614afc4` + `d30c5b99`) — NOT the base comma recorder — was the culprit, re-gated behind default-off `MCC_AST_LOOPCOND_STORE` (`fa6c2db5`). **How to apply:** before flipping ANY AST gate default-on at `-O2+`, run `tools/selfhost-fixpoint.py <build> --opt=-O2` (and `-O3`, `-Os`) FIRST. To bisect a self-host break, disable suspects with `MCC_AST_<NAME>=0` in the env (it is forwarded to both stages). Reproduce on Linux (macOS self-host was JIT-blocked when this was written) via a native-arm64 container + the `linux-gcc` preset, forcing `-DMCC_TARGET_ARCH=arm64` because CMake can misdetect the processor as x86_64 under colima. And note the dual gap from the arm64-osx parity row: byte-identity `o1==o2==o3` passes a STABLE miscompile and never RUNS the result, so a `-O2 -run` correctness leg is the missing complement.
- **`ast-verify-ratchet` on the mingw / x86_64 (win32) cell is NONDETERMINISTIC — a red cell there is often not a real regression.** The set of non-faithful (`desync`/`unfaithful`/`stackresidue`) functions the win32 mcc reports over `tests/exec/*.c` CHANGES run-to-run on identical source. Observed 2026-07-28: 7 functions (`bounds/bound_setjmp.c check`, `expressions/lshift_type.c main`, `features_c99_c11/builtins.c main`, `lexical/utf8_string_literal.c main`, `preprocessor/conditional_inclusion.c main`, `structs_unions/struct_init.c foo`, `vla/label.c f2`) recorded `desync` at `c47a5d17` but `faithful` at `6a0e1f47`, with nothing in between touching their codegen. Cause: `tests/ast/verify_ratchet.cmake` demands an EXACT `(file,func)` set match (it FATAL_ERRORs on wins as well as regressions, ~line 116), and the win32 baseline is regenerated by the mingw build — an `mcc`-hosted-by-mingw-gcc two-stage self-build that is itself nondeterministic, the same class as the `.data.rel.local` self-miscompile section above, so the fidelity verdicts of the mcc it produces flicker. `x86_64-linux.txt` does NOT flake (verified in a linux/amd64 container), so it is win32-self-build-specific. It cannot be reliably greened by editing `x86_64-win32.txt` from a non-mingw host — the delta can only be reverse-engineered from CI logs and it moves. Real fixes: address the win32 self-build nondeterminism, or relax the ratchet to fail only on NEW gaps, not on banked wins. Baselines are per-target at `tests/ast/verify-baseline/<cpu>-<os>.txt`; only `x86_64-linux` and `x86_64-win32` exist.
- **FP differential fuzzing vs clang — recipe that has NO false positives, and it found a real bug.** The integer fuzzer (`tests/fuzz/gen.h`) emits no floating-point at all, so FP codegen was un-fuzzed. FP differential fuzzing works once the two sources of *legitimate* cross-compiler FP divergence are controlled: (1) **x87 excess precision** — fuzz on **arm64** (no excess precision) or x86_64 with SSE; i386 is UNRELIABLE for FP differential; (2) **FMA contraction** — build the reference with **`-ffp-contract=off`** (plus `-fno-fast-math`) so `a*b+c` stays a discrete mul+add, matching mcc, which does not fuse. With those, IEEE `+ - * /` and `sqrt` are correctly rounded and **bit-identical** between mcc and clang, so a 1-ULP diff is a real bug. Recipe (native arm64-macOS, no docker): a generator (the working one lives at an untracked `_fp/fpgen.py` — hence the "commit it" item below) emitting provably-finite, NaN-free programs (guarded `/`, `sqrt(fabs())`, clamp non-finite through a `fin()` helper) that bit-folds every local into an exit-code checksum; then `cmake-release/mcc -O2 p.c` vs `clang -ffp-contract=off -fno-fast-math -O2 p.c`, run both natively, compare exit status. One real reference is enough — the runner's 2-ref consensus is optional. Note this macOS host has no real gcc (`/usr/bin/gcc` is a clang shim). Remaining: commit the generator into `tests/fuzz/` and wire a ctest cell so this runs in CI rather than ad hoc; extend beyond `+ - * / sqrt` to `long double` and the x86_64-with-SSE leg.
...
- **Never forward-declare another translation unit's `static` helper.** `mccgen.c` declared arm64 `intr`/riscv64 `ireg` as `static ...; /* defined later in the same TU */`, which holds only in the amalgamated build; the multisource cells (`cmake-linux-gcc-multisource`) compile `mccgen.c` separately and `libmcc.so` got an undefined `intr`, breaking every exe that links it. Cross-file helpers must be `ST_FUNC` (= `static` when `MCC_AMALGAMATED`, external otherwise) and declared that way at the use site. Local amalgamated builds and `cmake-debug` cannot catch this — reproduce with `gcc -c -DMCC_AMALGAMATED=0 ... src/mccgen.c` and check `nm -u` for the symbol.
  ...
  **CAVEAT RESOLVED 2026-07-29 — the mutation exists and this preset catches it.** A live call to another TU's `static` is the trigger, exactly as the caveat below suspected: `mccgen.c` called `pp_in_system_header` (defined `static` in `mccpp.c`) from a reachable path, and `cmake-debug` plus all five amalgamated targets built clean while `linux-gcc-multisource` failed with `libmcc.so: undefined reference to pp_in_system_header`. CI found it first (`10_linux-gcc-multisource`), and it reproduced locally in one `cmake --build cmake-linux-gcc-multisource`. The same build surfaced two PRE-EXISTING instances the amalgamated builds had always hidden — `pointed_type` and `gen_cast_s`, both called by `x86_64-gen.c`. So the preset IS a sufficient gate for this bug class, and the dead-branch experiment below failed only because a dead call is never emitted and so never needs the symbol. Original text kept for the method:
  **CAVEAT (superseded), stated because the claim above it is stronger than what I could verify: I could NOT construct a mutation that this build catches and `cmake-debug` misses.** Declaring a nonexistent helper `static` fails in BOTH builds (compile error either way, so it proves nothing about multisource). Declaring a real cross-file static (`ast_treechk_on`, defined `static` in mccast.c) and calling it under a runtime-false guard was accepted by BOTH — amalgamated rc=0 as expected, but multisource rc=0 too, with mccgen.c genuinely recompiled at `MCC_AMALGAMATED=0`. So the guard's effectiveness against the exact recorded shape is UNVERIFIED here; the historical failure was a link error on `libmcc.so`, which suggests the trigger needs the symbol to be reached by a live call rather than a dead branch. Do not assume this preset is a sufficient gate for the bug class until a mutation reproduces it.
- **A debug-info table keyed on the wrong `VT_` code returns 0, and 0 is a LEGAL-LOOKING DWARF value.** `mccdbg.c`'s
  `default_debug[]` keyed its two 128-bit rows on `VT_QLONG` (13, the two-register ABI return class) while a declared
  `__int128` carries `VT_INT128` (12). Every `__int128` lookup missed and fell through to `return 0`, which the caller
  wrote out as `DW_AT_type`. gdb rejects the ENTIRE compilation unit on a null DIE reference — "Cannot find DIE at
  0x0" — so `-gdwarf` produced no usable debug info at all, for any program, and nothing in the suite noticed except
  `dwarf-gdb-docker`, which is docker-gated and SKIPS locally. Fixed 2026-07-29 (`34fec309`) by keying on `VT_INT128`
  with `VT_QLONG` normalised to it. Two things generalise: a type-table miss must not be representable as a valid
  output value, and three unused typedefs in `mccdefs.h` were enough to void every `-gdwarf` build — nothing in the
  failing test source mentions `__int128`. Also worth knowing that the harness hid its own diagnosis: `set -e` killed
  the script the moment `grep` found no frames, so the CI log showed a bare header and no error.
- **An AST fold that rewrites a `TOK_LAND`/`TOK_LOR` node DROPS `AST_FB_LANDOR_INVERT` and inverts the branch — a recurring `-O2+` miscompile class.** `ast_set_fbits(a,n,0)` clears every flag bit, including the `AST_FB_LANDOR_INVERT` the parser sets for `!(...)`. Only the LAND/LOR replay path (`ast_replay_value` ~5870, which swaps jtrue/jfalse via `cmp_op ^= 1`) honors that bit; the folded plain node does not, so a negated condition comes out with its branch inverted. Two instances, both FIXED, both fixed the same way — bake the negation into the emitted op instead of relying on the bit:
  - **`MCC_AST_RANGE`** (`ast_range_try`/`_try_lor`), `lo<=x && x<=hi` → unsigned compare: fixed `2c88e797`/`311767c7` by folding to `ULE ^ invert` / `UGT ^ invert`. This was the macOS `selfhost-jit` blocker (arm64-osx parity row).
  - **`MCC_AST_BITFLAG`** (`ast_bf_try_if`/`_ifne`/`_lor`/`_land`), 5+-value equality chains → bitmask test: fixed `bcd68443`. The culprit was `ast_bf_try_if` (folds `if(x==a||…)` → `if(member)`), which ignored the invert bit on the parsed LOR — `objdump` showed `cbnz member` where `cbz` was needed. `try_if`/`_ifne` now bake the negation in (member ↔ member^1 per the bit); `try_lor`/`_land` (value context) skip when inverted.
  **Method to find and verify one of these:** write a self-verifying differential program (checker functions vs the direct expression, printing OK/FAIL n), compile it with mcc `-O2` and with gcc, then gate-bisect via `MCC_AST_<GATE>=0` — `MCC_AST_LANDOR_INVERT=0` changing the result is the tell for this class. **Then use `tools/tracediff.sh <mcc> r.c MCC_AST_BITFLAG=5 MCC_AST_BITFLAG=100` (fold on vs off, LANDOR held on): it pointed straight at `ast_bf_try_if`, which four rounds of manual reasoning had missed.** Disassemble a single static function with `objdump -d --disassemble=<fn>` (expect a leading `_` on a macOS-config mcc); a `cset ne`/`cbnz` where `cset eq`/`cbz` belongs is the inverted branch. **Audit lead:** other `ast_set_fbits(a,n,0)` sites that rewrite LAND/LOR nodes may carry the same bug — grep for `ast_op(a,n)==TOK_LAND||TOK_LOR` near fold rewrites.
- **`x < 0 ? -x : x` on a FLOAT type mishandles `-0.0`** (integer abs is fine): `-0.0 < 0` is FALSE, so the sign bit survives and the "absolute value" is negative zero. Found 2026-07-30 (`3249304a`): `__builtin_fabs`/`fabsf`/`fabsl` (`runtime/include/mccdefs.h`) and `foldm_fabs` (`mccgen.c`) both lowered fabs that way, so `__builtin_fabs(-0.0)` returned `-0.0`. Fixed by writing the macros as `<= 0 ? 0.0 - (x) : (x)` (`0.0 - x` canonicalizes both zeros to `+0.0`) and clearing the sign bit via a union mask in `foldm_fabs`. `mccdefs.h` is baked into mcc through `c2str`, so a rebuild propagates a header fix to every target. **AUDIT DONE 2026-07-30 — this class is CLOSED:** fabs was the only buggy instance. Verified bit-exact vs clang that `trunc`/`floor`/`ceil`/`round`/`fmin`/`fmax`/`copysign` all give correct signed-zero results, `signbit(-0.0)==1`, and `isnan`/`isinf`/`isfinite`/`isnormal`/`fpclassify`/`fmin`/`fmax`/`fdim` are correct on Inf/NaN/subnormals; FP arithmetic (~2444 double+float seeds) and FP↔int conversions (`fcvtzs`/`scvtf`, ~220 seeds) fuzz clean. Re-check any NEW float helper written in this shape.
- **Method for layout-dependent heap heisenbugs in mcc itself** (earned on the `MCC_AST_STOREVAL_CALL` vstack underflow, `b6af0f7f`, where lldb, ASan and gmalloc all failed): the corruption was a value-stack rotation at depth 1 writing `vtop[-2]`, and `vstack` (`gen_vstack+1`) sits immediately after `nb_sym_pools`/`sym_pools` in `MCCState`, so the symptom was a crash in `__sym_malloc` far from the cause. What worked was a `dynarray_add` sanity guard plus a `mcc_dbg_check_syms(where)` that reads `MCCState` at fixed offsets, sprinkled as checkpoints and gated to one function name — ~10 debug cycles at native speed. Reach for that before another sanitizer run.
...
- **A reserved replay slot must be excluded by EVERY allocator, not just the one that reserved it.** `ast_divmagic_materialize` reserves an `ast_ltemp` slot and only the register-promotion pass honoured the reservation (`ast_ltemp_off` poison, mccast.c:5353); the value-stack spill allocator did not, so under higher local pressure `get_temp_local_var` reused the slot and clobbered a live dividend (the i386 `-O2` `d2` miscompile, fixed `1a980d5b` by `ast_ltemp_overlaps()`). Any new reserved-slot mechanism needs the same check in both places, and the failure mode is within-frame so ASan cannot see it.
- **Mach-O reader traps, each of which once produced a passing test over broken output:** `reloff`/`nreloc` read and ignored linked SUCCESSFULLY and left `e8 00 00 00 00` (assert resolved ARITHMETIC targets, not link success); mcc's appliers ADD into the field (`add32le`/`add64le`, REL semantics inherited from tcc) so passing the field's existing value as a RELA addend counts it twice, and a relocation test whose addend is ZERO cannot tell "applied once" from "applied twice"; `llvm-objdump -s` prints each section twice and the second dump is numeric 32-bit words, so parsing both silently reverses every group of four; clang folds small addends into the instruction (`msg[5]` gets no `ARM64_RELOC_ADDEND`), so a LO12 handler that OVERWRITES the immediate silently reads `msg[0]`.
- **`-fdata-sections` trap: mcc already owns a section literally named `.data.ro`, so a global called `ro` was silently MERGED into it** — wrong flags, no diagnostic. Guard by scanning the existing section list and falling back to the base section, and assert **by symbol index**, because comparing section-name lists cannot distinguish "`ro` got its own section" from "`ro` was absorbed". The guard is an inline scan rather than a call to `have_section()`, which is `static` to `mccelf.c` — calling it from `mccgen.c` reproduces the multisource break `pp_in_system_header` caused.
- **Durable `-march` rules:** `-O4` means "run every optimizer", NOT "raise the required ISA" — an optimizer that changes the ISA floor never belongs in a blanket keyed on optimization effort. Do not stage a gate whose lowering the backend cannot emit. A consumed-feature counter cannot key the cache (the key is taken from the PRISTINE arena before transforms run, so a consumption accumulator is always one function late — use arena-relevance scans). Observing the slice-cache key needs a WARM cache and `XDG_CACHE_HOME`, not `HOME`. Nothing in the suite compares object bytes to a checked-in artifact across hosts — expectations are behavioural. Cross-compilation keeps the triple baseline by construction (`mcc_isa_native` reads CPUID only under `__x86_64__`/`__i386__`). Predefined macros following `-march` is a REGRESSION — do not implement it. The static `.ARM.attributes` blob stays keyed on the build config by design.

### Method rules harvested from pruned items
- **Prefer `cdelta` to `differ` for any gate whose transform is real but whose byte-level effect is target-dependent.** An object diff asks "did the bytes change", which is only a proxy for "did the pass fire"; `cdelta` compiles with the gate forced to 0 and to 1 and requires the named `--stats` counter to be 0 then nonzero, which is attributable to the pass and portable across targets. An arch guard is the wrong fix — it records a pass as absent where it demonstrably fires.
- **Write optfire/regression cases in `long long`, not `long`.** `long` is 64-bit on x86_64 and 32-bit on i386, so a "64→32 narrowing" case poses no question at all on i386 and reads as a pass with nothing to do.
- **`rc=$?` after a pipeline reports the LAST command's status** (usually `grep`'s), not `mcc`'s. A measured "pass" was misread on exactly that.
- **Anchor trace greps.** `grep -o 'SYNC .*'` also matches the tail of `DESYNC ...` state lines — anchor with `\bSYNC vn=`. Substring matches have produced false signals three times in this file (`faithful`/`unfaithful`, `verified` inside a refusal message, `SYNC`).
- **Respect function boundaries when correlating trace events.** A line-window correlation over a 1.4M-line trace produced a clean-looking split that was pure artifact; re-running with the window reset at each `ast_func_begin` scattered the counts 0..9 and two conclusions had to be withdrawn.
- **Do NOT byte-compare two self-compiled `src/mcc.c` objects across a compiler-source edit.** `src/mcc.c` is the amalgamation and INCLUDES `mccast.c`, so editing the recorder also edits the TU being compiled — two variables, not one (a never-called dummy function in `mccast.c` reproduces the effect alone). Compile a FIXED corpus with both binaries, and put both binaries in the same directory: `-B` alone does not equalise them, because auto-mccdir also probes the exe directory and picks up different `threads.h`/`stdatomic.h`.
...
## Reference — recipes, invariants, and gotchas (42 per-topic notes folded in 2026-07-30)

Standing reference, not a task list: how to run the gates, what each subsystem's rules are, and the traps that have cost time. Landed-work narrative has been pruned — git history has the chronology. Open items live in the sections above; the few open landmines that belong with their subsystem are marked **OPEN** here.

**Missing planning docs:** these notes reference `docs/NOTES.md`, `docs/JIT-PLAN.md`, `docs/AST.md`, `docs/SUBSTRATE-PLAN.md`, `SPLIT.md` and `GATED.md`. **None exist in the repo or its git history** (`git ls-files` shows `docs/TODO.md` as the only tracked file under `docs/`); the `TARGET_DEFS_ONLY` split was executed from a code-derived reconstruction. Treat every "detail lives in docs/X.md" pointer as dangling.

### Working agreements

- **Reply style.** Succinct, least flowery language possible. Short sentences, no filler or praise, plain words over ornate ones; state results and next steps directly, drop hedging.
- **Summaries are counts, not adjectives.** Identify the important nouns, break them into subcategories reported as **numbers**, describe macro features in plain English. "CI: 6 job groups, 28 preset configs, 5 qemu arches" beats "an elaborate CI with lots of jobs".
- **Never write code comments.** The non-vendor tree (`src/`, `runtime/`, `tools/`, `tests/`, `examples/`) is deliberately comment-free and stays that way: no `//` or `/* */` on new or edited code. Rationale goes in a NOTES entry, open work in this file. Preserve only non-authorial markers — license/SPDX/attribution headers, tooling-semantic comments (`FALLTHROUGH` is a macro here, not a comment; `clang-format off/on`, `NOLINT`), and comments that are test payload (`tests/cst/**` CST-trivia fixtures, `tests/exec/preprocessor/comment.c`).
- **Git: never create a branch.** Always commit and push directly to `main` — no `git checkout -b`, no PR. Keep the required commit trailers (Co-Authored-By + Claude-Session). Commits are often deferred to the next all-green checkpoint; reach a coherent tree first.
- **The differential FUZZER is the ONLY pre-commit gate.** Do **not** run full ctest, self-host, cross-arch, shadow, or object-diff before committing — CI catches all of that. Run fuzz (register-pressure mode, gcc+clang consensus, 100+ seeds, forced sub-gates for gated changes), confirm 0 miscompiles, commit, push.

### Build dirs, presets, and the standing gates

- **Build dirs.** CMakePresets `binaryDir` is `${sourceDir}/cmake-${presetName}` (debug → `cmake-debug/`). A leftover `cmake-build-debug/` also exists and its ctest `MCC_BUILT` fixture rebuilds binaries from current sources on `ctest`, so its timestamps look fresh while `cmake --build --preset` output actually went to `cmake-debug/` — this cost a debug loop to a stale `mcchv`. Run tests against the preset's own `cmake-<name>/`; `cmake --build --preset <p>` must run from the repo root.
- **x86_64 3-stage self-host fixpoint** (the standing gate for `src/` changes): `cmake-debug/mcc` compiles `src/mcc.c` → stage2, stage2 → stage3, stage3 → stage4, compared with `cmp`. No in-repo script. Includes: `-I<build> -I src, src/arch/{i386,x86_64,arm,arm64,riscv64}, src/objfmt, src/formats, include, repo root`. Defines: `-DMCC_AMALGAMATED=1 -DCC_NAME=CC_mcc -DMCC_CONFIG_OPTIMIZER=1 -DMCC_CONFIG_LSP=1 -DMCC_CONFIG_MCCDIR="<repo>/dist/lib64/mcc" -DMCC_CONFIG_AUTO_MCCDIR=1 -DMCC_CONFIG_PREDEFS=1 -DMCC_CONFIG_OS_RELEASE="..." -DMCC_CONFIG_TRIPLET="x86_64-pc-linux-gnu" -DMCC_TARGET_X86_64=1 -DMCC_VERSION=<from compile_commands.json>` plus `-B<build>`. `MCC_CONFIG_TRIPLET` is required on this Gentoo host or the linker picks 32-bit `/usr/lib/crt1.o`. Cheapest source of truth for current defines: the `mcc` entry in `<build>/compile_commands.json`.
  - **Never pass any `-DMCC_EMBED_*` define when LINKING stage compilers:** `MCC_EMBED_MCCRT` → "unresolved reference to `mccrt_blob`"; `MCC_EMBED_JIT`/`MCC_EMBED_JIT_BLOB` → "unresolved reference to `mccjit_blob`". Filter every token containing `MCC_EMBED` out of the derived arg list.
  - **`s2 != s3` is EXPECTED and is NOT a failure** once those defines are dropped — the build-dir mcc was compiled *with* them, so it is a differently-configured compiler than stage2 (they differ at byte 6289 on an unmodified tree). **The invariant that gates a change is `s3 == s4`.** Always run the same 3 stages on a stashed/unmodified tree as a control before believing a fixpoint failure.
  - Other gates: `cmake --preset cross` builds all 12 cross compilers (per-target `mcc -c` byte-identity checks run against these); `linux-gcc-multisource` covers the non-amalgamated build; the invariant ctests are host-gate / config-drift / preset-parity / retired-macro.
- **arm64 3-stage self-host fixpoint (qemu + musl)** — the gate that catches a fresh-temp/arm-insertion optimizer bug (a general PRE prototype passed x86_64 + adversarial exec and miscompiled here). GOTCHA: `cmake-cross/mcc-arm64` is built **without** the optimizer (`-O2`==`-O0`, `MCC_AST_*` inert — no `-DMCC_CONFIG_OPTIMIZER=1` in its compile command), so build your own optimizer-enabled arm64-targeting host cross mcc:
  ```
  gcc -O2 -w $INCS -DCC_NAME=CC_mcc -DMCC_TARGET_ARM64 -DMCC_CONFIG_CROSSPREFIX='"arm64-"' \
    -DMCC_CONFIG_PREDEFS=1 -DMCC_CONFIG_AUTO_MCCDIR=1 -DMCC_CONFIG_OPTIMIZER=1 -DMCC_CONFIG_LSP=1 \
    -DMCC_CONFIG_DIAG_RT=2 -DMCC_VERSION=20260707094756 -o xmcc src/mcc.c
  ```
  `$INCS` = the x86_64 recipe's 10 `-I` dirs with `-I<repo>/cmake-cross` first (not the debug build dir). Link with sysroot `vendor/gentoo-stage3-arm64-musl`: `LINK="--sysroot=$SR -Bcmake-cross -L$SR/usr/lib -Wl,-dynamic-linker,/lib/ld-musl-aarch64.so.1"`, `export QEMU_LD_PREFIX=$SR`; `-Bcmake-cross` supplies `arm64-libmccrt.a` (CROSSPREFIX makes mcc look for the `arm64-` prefixed runtime). Stages: `xmcc … -o s2 src/mcc.c`, `qemu-aarch64 s2 … -o s3`, `qemu-aarch64 s3 … -o s4`, `cmp s2 s3 && cmp s3 s4`. Baseline (gate-off) converges at ~1.08 MB.
  - **CRITICAL: never pipe mcc's stdout to `head`/`grep` during a build** — mcc prints progress to stdout and SIGPIPE kills it mid-link, leaving no output file. Redirect to a file.
  - **riscv64:** the same recipe works for real-program validation (`-DMCC_TARGET_RISCV64 -DMCC_CONFIG_CROSSPREFIX='"riscv64-"'`, sysroot `vendor/gentoo-stage3-riscv64-musl`, `-Wl,-dynamic-linker,/lib/ld-musl-riscv64.so.1`, `qemu-riscv64`), but the full `mcc.c` 3-stage self-host ABORTS on riscv64 (pre-existing Tier-3 backend gap) — good for exec checks, not for the fixpoint gate.
  - **Two firing caveats:** a narrow transform may not fire on `mcc.c` at all, so a byte-identical self-host proves byte-safety, NOT firing correctness; and the differential fuzzer does not generate every shape (it never emits the PRE diamond), so `--gates MCC_AST_X=1` can likewise only prove byte-safety. For narrow transforms write a bespoke generator and run it through `xmcc` gate-ON under qemu against gcc (arch-independent checksums).
- **Self-host smoke (lighter gate).** Take the `mcc.c` `compile_commands` argv via shlex, swap compiler → `./cmake-debug/mcc`, drop the source path/`-c`/`-o`/`-g`/`-O*`, add `-O2 -c src/mcc.c -o X.o` plus the knob env → ~1.6 MB object; **link with mcc itself, not gcc ld** (mcc's `runtime/lib/mccrt.c` supplies `__floatundixf`/`__fixxfdi` x87 long-double helpers gcc-ld can't resolve): `./cmake-debug/mcc X.o cmake-debug/CMakeFiles/mcc.dir/mccrt_blob.c.o -o SH -lm -ldl`; run `SH -Iruntime/include prog.c`. Also `tools/selfhost-smoke.py cmake-debug` (no knobs; gates baked in).
- **Fast per-gate cross-arch validation** (skips the cmake/JIT build): in `debian:bookworm-slim` build a per-target mcc as a host tool straight from the amalgamation — `gcc -w -DMCC_CONFIG_OPTIMIZER=1 -DMCC_TARGET_<T> -Isrc -Iinclude -Isrc/formats -Isrc/objfmt -Isrc/arch/<a> -O0 -o mcc-<T> src/mcc.c`. **`gcc-multilib` CONFLICTS with the cross-gcc packages** — use a separate container for i386. Link `cmake-cross/<arch>-libmccrt.a` for helpers like `__mcc_signbit`. On a Windows/MSYS host every `docker run`/`exec` with a `-v` mount needs `MSYS_NO_PATHCONV=1 MSYS2_ARG_CONV_EXCL='*'`; use a persistent `docker run -d … sleep N` + `docker exec` to build once and iterate.
- **riscv64 runtime-verify (ASan / libc-needing output).** Do NOT link with the vendored stage3 `riscv64-unknown-linux-musl-gcc` under qemu — running that cross-native gcc under `qemu-riscv64 -L` fails even for a trivial hello (its cc1/as/ld subprocesses and loader path don't resolve); a non-asan control failing identically proves it is the harness, not codegen. Instead (mcc-riscv64 is a HOST x86_64 driver+linker, so no gcc-under-qemu): `ROOT=vendor/gentoo-stage3-riscv64-musl`; build the sanitizer runtime with mcc itself (`mcc-riscv64 -c runtime/lib/mccasan.c -nostdinc -isystem $ROOT/usr/include -o asan_rv.o`); stage it where the driver auto-links it (`-fasan-shadow` auto-links `<crossprefix>mccasan.o` from its mccdir, which `-print-search-dirs` shows as `install:` = `cmake-cross`) via `cp asan_rv.o cmake-cross/riscv64-mccasan.o` — the CMake `_csan` staging only runs when a sysroot exists at configure time, usually absent here; then `mcc-riscv64 -fasan-shadow --sysroot $ROOT -isystem $ROOT/usr/include -L$ROOT/usr/lib test.c -o exe` and `qemu-riscv64 -L $ROOT exe`. binfmt_misc has qemu-riscv64 registered but the stage3 gcc still won't run transparently.
- **Cross-arch conformance: i386 and 32-bit arm are the ones that catch bugs.** x86_64 + qemu-arm64 + qemu-riscv64 share the LP64 / 8-byte-double / `FLT_EVAL_METHOD==0` regime, so a new self-checking `tests/exec/**` golden can pass all three and still break i386 and 32-bit arm: i386 aligns `double` to 4 in structs (not 8) and its x87 FP under `-fexcess-precision=fast` keeps 80-bit excess precision in FP function returns (exact narrowing-equality asserts fail); mcc also under-aligns over-aligned **stack locals** at -O0 (i386/arm give 4 even for `alignas(16)`; x86_64 gives 16 but not 32/64) — statics are fine. Assert alignment via `_Alignof(T)` not `sizeof(T)`, guard excess-precision equality behind `FLT_EVAL_METHOD==0`, don't assert runtime alignment of over-aligned auto locals.
  - Before committing an exec-golden change run the `qemu-docker` target (x86_64/i386/arm/arm64/riscv64 × glibc/musl, 30 cells): `docker build -t mcc-qemu tests/qemu/docker` then `docker run --rm -v $PWD:/work -v $PWD/vendor:/vendor -v $PWD/dist:/dist mcc-qemu` (~35 s once cached; `mkdir dist/` first), or the `qemu-i386`/`qemu-arm` presets.
  - **The real cross compilers are `cmake-cross/mcc-i386` and `cmake-cross/mcc-arm64`.** `cmake-qemu-i386`/`cmake-qemu-arm64` currently EMIT x86_64 (`-dumpmachine`=x86_64, `MCC_TEST_EMU` empty), so `--target mcc` there builds the NATIVE compiler — a whole session's "cross-arch 292/292" runs were x86_64 re-runs before this was caught. i386: `cmake-cross/mcc-i386 -B cmake-cross -L"$(dirname $(gcc -m32 -print-file-name=crt1.o))" -I/usr/include f.c` then run the binary directly (i386 ELF runs on x86_64; `qemu-i386` may fail on the host libc ISA level), cross-checking the test itself with `gcc -m32`. arm64: `cmake-cross/mcc-arm64 -B cmake-cross --sysroot=vendor/gentoo-stage3-arm64-glibc -isystem $R/usr/include -L$R/usr/lib64 f.c -o x && qemu-aarch64 -L $R x`.
  - **Repro recipe for an i386 codegen bug from an arm64 host (no native i386):** build `mcc-i386` from the amalgamation (`gcc -w -DMCC_CONFIG_OPTIMIZER=1 -DMCC_TARGET_I386 -Isrc -Iinclude -Isrc/formats -Isrc/objfmt -Isrc/arch/i386 -O0 -o mcc-i386 src/mcc.c`) and compile with `-I<b>/include -B<b>/include`. **The cmake i386 preset on an arm64 host emits an ARM64 `libmccrt.a`** (EM:183, misdetected arch), which link-fails any case referencing a runtime float/int128 helper — build a real i386 runtime instead: `for c in runtime/lib/*.c; do i686-linux-gnu-gcc -m32 -O2 -ffreestanding -Iruntime/lib -Iinclude -c "$c"; done; ar rcs i386-librt.a *.o`. Link `i686-linux-gnu-gcc -m32 -static <obj> i386-librt.a -lm`, run under `qemu-i386-static`, reference `i686-linux-gnu-gcc -m32 -O<n> -static <src> -lm`.
  - Cross-arch **measurement** of recorder/optimizer coverage works on an x86_64 box: point the cross compiler at its own `vendor/gentoo-stage3-<cpu>-musl` headers and lift `-D`/`-I` from `compile_commands.json`, dropping `MCC_TARGET_*`, `MCC_GITHASH` (contains a space, splits into two argv entries) and the other arch's `-I`.
- **External GNU `ld` rejects multi-function mcc objects** — `.eh_frame_hdr refers to overlapping FDEs` → `final link failed: bad value`. Pre-existing and unrelated to any feature work (reproduces on `void f(){} int main(){}` from a stock `mcc -c`): the `.eh_frame` FDE pc_begin relocs (`mcc_eh_frame_fde`, `mccdbg.c:800` — RELA addend 0, func_start written in data) are resolved by mcc's own linker but not by GNU ld. When testing codegen that needs an external toolchain, use a single-`main` program or link with mcc itself. A multi-function program that "fails to link / rc=127" under `cc` is almost always this, not the code under test.
- **cli-test printf-into-source quoting.** `tests/cli/cases.h` builds sources with a shell `printf '...'` via `popen` → `/bin/sh`. To embed a `"` do NOT use `\\\"` (C-string) → `\"` (shell): **dash** (ubuntu/CI `/bin/sh`) leaves it a literal backslash-quote so the source is malformed and the expected diagnostic never fires, while bash and macOS `/bin/sh` convert it — green locally and on the macOS cell, broken on every ubuntu cell. Use the portable octal `\\042` → `\042`. Treat "passes locally, fails only on ubuntu CI" as a shell-portability signal.
- **cli cases register at configure time** — a new case needs `cmake <build>` (reconfigure) before `ctest` sees it.
- **Driver-action flag order.** `-ar` and `-print-search-dirs` must appear before other options; a *preceding* `-f…` makes mcc abort with `cannot parse -ar here` / `cannot parse -print-search-dirs here`. Compile/`-run`/`-E` accept a preceding `-f…`. So you cannot blanket-prefix `{MCC}` in `tests/cli/runner.c` with a flag — it breaks the `ar_create_list` / `print_search_dirs` cases; inject per-case instead.
- **MSVC `cl` supports neither `-dumpmachine` nor `--version`** — it emits `D9002` + `D8003` to stderr and exits non-zero, and under MSBuild that leaked stderr fails the whole custom-build step (`MSB8066`, exit -1) even when the tool succeeded. `tools/bench.c` `detect()` skips `ts_cc_probe` for `STYLE_CL`; **the same latent bug remains in `tools/build.c:734/748`** (harmless only because MSVC CI uses the CMake path — if build.c is ever run on MSVC, detect cl by basename and skip there too).
- **`pe-wine-conformance` is FLAKY** (wine timeout). Exclude with `-E wine` or rerun-failed before believing a failure.
- **CI long-poles:** bench target ~500 s, macOS ctest ~431 s (vs ~60 s native), sequential matrix ctest.

### Differential fuzzer (`tests/fuzz`)

Tools-only, no codegen surface. `gen.h` = seed-deterministic UB-free generator (integer-only, splitmix64; csmith/cvise/creduce are NOT installed on this host, so the custom generator + line-ddmin reducer are primary). `runner.c` = oracle (gcc+clang `-O2` consensus, compare stdout+exit) + UBSan drop gate + ddmin `--reduce` + `-O`/`MCC_AST_*` gate triage + `--replay` corpus lock; argv `<mcc> <bdir> <idir> <work> <gcc> <clang>`; env `MCC_FUZZ_SEED/COUNT/GATES`. `corpus/` = graduated repros (`repro_seed<N>.c`; `accept_<name>.c` = intentional impl-def diff). `bisect.sh` = `git bisect run` predicate over a repro. ctest (x86_64-only, gated on gcc+clang via `MCC_DIFF3_GCC/CLANG`, else SKIP): `fuzz/smoke` (seeds 1..40, -O0..-O3), `fuzz/matrix` (seeds 1..24 `--gates`), `fuzz/corpus`; registered right after the diff3 block in `CMakeLists.txt`, with the blanket at ~4085 auto-labelling non-qemu tests `native`.

- **Register-pressure + const-divisor mode:** a `pressure` cfg (~1/3 of seeds) with 12–40 LOCAL live longs (`fuzz_pexpr`/`fuzz_func_pressure`, all live to a final fold) and constant-divisor cases (`fuzz_cdiv`). Two constraints that make DIVMAGIC reachable: mcc's unsigned DIVMAGIC fires ONLY on 32-bit `unsigned int` division (`mccast.c` ~7086) — `unsigned long` never triggers the mul-high magic, so the dividend must be cast to `unsigned`; and DIVMAGIC only runs on FAITHFULLY-REPLAYED functions, so pressure funcs must use FLAT expressions (deep ternary/switch/signed-shift nesting breaks faithful replay). Even so this mode does NOT reproduce whole-TU corruption like the DIVMAGIC self-host crash — self-host remains required.
- **Per-gate obligation:** extend `GATES[]` in `tests/fuzz/runner.c` for every new `MCC_AST_*` gate; the combined `RECORDER8` entry stresses interactions.
- **Only land a transform whose firing pattern the fuzz+exec regimen actually stresses** — branches/arith are well stressed; pointer-tail-recursion, >64-bitflag and libm-fold are poorly stressed, so don't land those casually.
- Gotchas when editing the harness: `R_OK` collides with `<unistd.h>` (renamed `RES_*`); a shift operand must be cast `(unsigned long)` or a 32-bit operand shifted by up to 63 is UB.

### Tracing, logging, and the `--stats` panel

- **`-v` is a category bitmask, not an ordinal level** (`src/mcclog.h`). `MCCState.verbose` is `unsigned char`; bit → tag: 0 `[CMD]` 1 `[PATHS]` 2 `[INCL]` 3 `[INFO]` 4 `[NOTE]` 5 `[STATUS]` 6 `[DEBUG]` 7 `[TRACE]`. `-v`/`-vv`/`-vvv` set the low tier bits cumulatively via `x | (x+1)` → masks 1/3/7 (`MCC_V1`/`V2`/`V3`); `-v<N>` ORs an arbitrary mask (`-v64`=[DEBUG], `-v128`=[TRACE]). Legacy gcc-compat tier checks compare `MCC_VTIER(verbose)` (= `verbose & MCC_V3`) so TRACE can be on without silencing regular `-v`. Mask with `MCC_VTIER` for tier compares; use `verbose & MCC_LOG_<CAT>` for diagnostic categories.
- Helpers (static inline in `mcclog.h`): `mcc_logf(bit,fmt,...)`, `MCC_DEBUG(...)`, and `MCC_TRACE(...)` which prints `[TRACE] file:line func:` and is compiled out unless `MCC_CONFIG_TRACE` (CMake option, default off, registered with a `--trace` mccbuild flag so config-drift/config-defines stay green — mirror that plus `MCC_CONFIG_AST_SHADOW` when adding any `MCC_CONFIG_*` macro). `mcclog.h` is included at the end of `mcc.h`, after `mcc_state`. Instrumented points: `mcc_compile`, `mcc_preprocess`, `gen_function`, `ast_func_begin`/`ast_func_end`, and the inline/promote branch.
- **GOTCHA:** `MCC_TRACE`/`mcc_logf` read the global `mcc_state->verbose`, so a trace only fires where `mcc_state` is the current verbose-carrying state — put traces AFTER `mcc_enter_state`; driver/link points (`main`, `mcc_output_file`) don't fire. The layer is otherwise `MCCState`-free: it reads a free-standing `mcc_log_verbose` (declared in `mcclog.h`, defined in `mcchost.c`, mirrored in `mcc_enter_state`/`mcc_exit_state`), with explicit-source variants `mcc_log_enabled_v`/`mcc_logf_v`/`MCC_TRACE_V`/`MCC_DEBUG_V`. Standalone non-amalgamated compiles of `mccast.c`/`mccstats.c` (e.g. `asttool`) can trace if built with `MCC_CONFIG_TRACE=1` and `mcc_log_verbose` set directly — those targets don't parse `-v`.
- **`MCC_LOG=<mask>` is the env verbosity FLOOR, and the only way to trace the LINK phase.** `mcc_log_verbose` is zeroed in `mcc_exit_state` and linking runs outside that window, so `-v128` produces ZERO output from `mccelf.c` (754 trace sites) and `mccmacho.c` (319) — 1,073 sites that could never fire, in exactly the object/archive/relocation code where format bugs live. `MCC_LOG` is ORed in by `mcc_log_enabled`, so it survives every state transition. Default 0.
- The pre-existing `--debug=<cat>`/`-d<num>` bitmask (reloc inc pp struct tok pe ver asm sym) is a SEPARATE internal-trace axis from `-v`. The `bitflag-candidate:` diagnostic is behind `MCC_AST_BITFLAG_REPORT` (it used to leak at -O2+).
- **`tools/tracediff.sh` answers "why is this body accepted under config A and rejected under config B".** Do NOT hand-patch a temporary `fprintf` into a suspect guard. `tools/tracediff.sh ./cmake-debug/mcc repro.c MCC_AST_OPASSIGN=0 MCC_AST_OPASSIGN=1` — each side takes env assignments AND mcc flags, so `-O1 -O3` works. It prints the FIRST divergence with the deciding values attached. Env: `TD_FILE` (scope, default `mccast`), `TD_SKIP`, `TD_OPT`, `TD_CTX`. Guarded by the `ast/tracediff` cell. Three load-bearing pieces: `MCC_CONFIG_TRACE` builds carry ~11,476 trace sites (every function entry + branch, enforced by `trace-gate-invariant`/`tools/tracegate.c`; `cmake-debug` has it ON, release compiles it out); scoping via `MCC_TRACE_FILE`/`MCC_TRACE_FUNC` (substring comma-lists) + `MCC_TRACE_SKIP` (exact names) — **80% of an unscoped trace is `mccpp.c` preprocessor churn**, 127,871 lines becomes 2,467 with `MCC_TRACE_FILE=mccast`; and `MCC_TRACE_IF` at `ast_hook_vpush`/`vstore`/`vdup`/`cmp_invert` printing `r=%#x t=%#x vn=%d rel=%d`, with `AST_SET_DESYNC` printing `DESYNC vn=.. inop=.. incall=.. bail=..`. **Use `MCC_TRACE_IF`, not `MCC_TRACE`, for anything dereferencing `vtop`** — plain `MCC_TRACE` evaluates its args even when the TRACE bit is off. Limit: config-vs-config only; there is no second trace for "why is this single run wrong".
- **`mcc --stats[=N]`** renders an in-place stderr panel (cursor-home redraw, ~20 fps). N is a bitmask: 2=JIT, 4=STRATEGY, 8=COMBO, 16=SEARCH; bare `--stats` = all. Field `MCCState.stats`, global `mcc_stats_mask`; module `src/mccstats.{c,h}`, included in the amalgamation right after `mcchost.c` in `src/libmcc.c` so `mccast.c`/`mccjit_embed.c` can feed it; finished by `mcc_stats_finish()` in `mcc_delete`. Feed hooks: `ast_search_select`/`ast_search_combo_score`, the post-search strat cycle in `ast_func_end`, and KGC hit/miss/poison + recompile/promote in `mccjit_embed.c`. **To see live SEARCH/COMBO data you must** use `-O4+` (search needs `optimize_search_seconds>0`; `-O2` shows nothing) AND `MCC_AST_SEARCH=1` AND `MCC_SEARCH_WORKER=1` (skips the out-of-process superopt fork pool at `mcc.c:1338`). The persistent disk memo makes a re-run all cache hits (evaluated=0) — reset with a throwaway `XDG_CACHE_HOME`. `cmake --build cmake-debug --target mcc-stats` is the demo. The JIT section only populates under `-run`/embedded JIT. Non-TTY prints one plain final snapshot (`MCC_STATS_FORCE=1` overrides). Help line is under `-hh`.

### AST side-car substrate

Four lazy, epoch-invalidated side-cars in `src/mccast.c`, all pure accelerators with no emitted-byte change:
- `ast_du_*` — per-slot {written,escaped} table; backs `ast_cprop_escapes` (11 sites) + `ast_local_is_readonly`; originals kept as `_scan`. Covers only the 2 whole-function scanners (licm/ivsr are subtree-scoped, left alone).
- `ast_memo_*` — per-node tri-state memos for `ast_ident_pure`/`ast_cprop_safe`/`ast_sccp_has_label`/`ast_cse_regpure` (renamed `_compute`, wrapped by `AST_MEMO_QUERY`).
- `ast_hash_*` — structural subtree hash; `ast_ident_same` uses it as a collision-proof O(1) fast-reject falling through to `ast_ident_same_scan`. It is an epoch-rebuilt side-car array, not the incremental SoA `h[]` column.
- `ast_vlat_*` — the value-domain lattice (`AstVLat` = int64 interval ∧ uint64 known-bits ∧ state), slot-offset side-car like `ast_du_`, `do{}while(changed)` fixpoint over never-written int locals meeting in dominating-`AST_If` predicates via a parent-spine walk (reuses `ast_range_bound`). Projections: `ast_vlat_narrowing` (§29 residue) and `ast_vlat_context` (`context_in` memo key). Shadow assert `ast_vlat_check_sound` = cached ⊇ fresh; `tools/asttool.c suite_vlat` is the logic gate, since a lattice with no consumer cannot be caught by ctest.

**Invalidation is one `uint64_t AstArena.epoch`** bumped by every mutation: public setters, `ast_node`, `ast_add_child`, `ast_clear_children`, `ast_arena_reset`, and the only 3 functions that write columns directly (`ast_ident_adopt`, `ast_cse_setref`, `ast_ltemp_insert_before` — verified complete; no aliased/bulk column writes exist). Each side-car rebuilds when the epoch differs. Chosen over a dirty-flag / parent-chain-repatch design: simpler, trivially correct. **Any new direct column write must bump the epoch.**

**Validation is `MCC_CONFIG_AST_SHADOW`** (CMake option, default OFF; `--ast-shadow` in `tools/build.c` keeps `config-defines` in sync; also wired into asttool). Under it every side-car query also runs the legacy scan and `abort()`s on divergence. Deliberately NOT in a stock preset (release inherits debug) — use a dedicated dir: `cmake --preset debug -B cmake-shadow -DMCC_CONFIG_AST_SHADOW=ON && cmake --build cmake-shadow && (cd cmake-shadow && ctest -j$(nproc))`.

**Guard gotcha:** the side-car definitions live under `#ifdef MCC_INTERNAL` (line 506) inside the whole-file `#if MCC_CONFIG_OPTIMIZER && (defined(MCC_INTERNAL)||!defined(MCC_AMALGAMATED))`. Cross-region calls from core arena functions (e.g. `ast_arena_free`'s invalidate) must be guarded `#if defined(MCC_INTERNAL) && MCC_CONFIG_OPTIMIZER`, else "used but never defined" in the `mcc.c` TU.

### The `-O4` search — architecture and safety

Each pass is an `AstStrategy {name,gate,apply}` row in a frozen `ast_strategies[]` that `ast_func_end` always consumes; this is the sole pipeline (the legacy inline block and its `MCC_AST_ENGINE` toggle are gone). Byte-identity recipe for any change here: `fuzz_runner … --gen SEED` emits a standalone program; compile it two ways and `cmp`.

At `-O4+` (`optimize_search_seconds>0`) with `MCC_AST_SEARCH=1` — a dedicated flag kept off the default -O4+ path so it doesn't perturb the out-of-process superopt — `ast_func_end` runs `ast_search_select`: a per-function search whose candidates are measured on an `ast_arena_clone` by `ast_cost_score` (static node cost, factored from `ast_fn_cost`) or by emitted size, memoized by `ast_intention_hash`, enumerated through `combo_run` (`src/mcccombo.h`) with `ast_search_combo_score` wrapping `ast_search_score_one` (which owns the `ast_cur` + gate-global save/restore so `combo_run` stays a pure enumerator).

- **Safety architecture: the search only SELECTS gates.** The winner (or a time-truncated fallback to baseline) is emitted by the existing unmodified pipeline on the untouched captured tree, so a search bug can only pick a larger-but-correct config, never miscompile. No emit during measurement ⇒ no emit-cursor/promo/counter hazards. `-O1..-O3` have no budget, never search, stay byte-reproducible; `-O4+` is timing-bounded and non-reproducible by design.
- Budget is `-ON` seconds ABSOLUTE from the first tick, timed via portable `clock()` (CPU-time proxy; works in asttool, which lacks the mcchost timer). Tick-duration prediction uses `src/mccforecast.h`: 13 libm-free one-step predictors (rw/SES/AR1/lin/ridge/pspline/GAM/BSTS-Kalman/GP/GBM/Holt/TheilSen/movmed; online accuracy → top-3 → least-distance-to-median consensus). The same module is exposed to `-ffold-math` as `mcc_fc_*` constant-arg foldable builtins (`foldfc_try` in `mccgen.c`); asttool `forecast` covers it. Header-only, so it works in the amalgamation and standalone.
- **Emit-size scoring** (`MCC_AST_SEARCH_EMITSIZE`): `ast_search_emit_size` replays a folded candidate into the LIVE text section (in-place emit-and-rewind like `AST_PF_EMIT`) with inline+promote OFF (avoids the promo save/restore desync), scores by `ind - ast_body_ind_sv`, rewinds all cursors. It must run RUN-TO-COMPLETION per candidate — a fair tick scheduler thrashes shared ltemp/fconst emit state across interleaved candidates, so a candidate is only emit-measurable right after it folds to completion. `MCC_AST_SEARCH_EMITISO` adds scratch-`Section` TEXT isolation (`ast_scratch_init/_enter/_measure_exit`, `SHF_PRIVATE` so scratch never reaches output; **text+reloc only — NEVER truncate data/rodata**), which also fixes the in-place path's leaked-duplicate-constant anomaly.
- **Fork pool** (`MCC_AST_SEARCH_THREADS`): `ast_search_pool` forks `host_nproc()-1` score-only workers over the candidates, so each gets its own COW copy of every optimizer global — zero shared-state races and no `_Thread_local` refactor. Workers write `{idx,score}` to a pipe and `_exit`; the parent applies the winner single-threaded. `#if MCC_HOST_POSIX` (serial elsewhere); asttool excludes it.
- **KEY LESSON:** when a search item looks "blocked on a milestone-scale refactor", look for a safe sidestep — emit-size used run-to-completion + in-place emit-rewind, the pool used fork isolation. Both avoided the scary path.

**Gate plumbing and vocabulary:**
- `AstGateMask` is `uint64_t` (was `unsigned`, hard-capped at 8 bits on disk via `gates & 0xff` + `MAGIC<<8`): 64 knobs in memory, low 48 persisted (`AST_GATE_BITS=48`, MAGIC in the high 16 of the on-disk word). `AST_SG_*` are `#define ((AstGateMask)N)`, not an int enum. Old 8-bit records are skipped on load (MAGIC check fails, cache rebuilds).
- `src/mccgate.h` (header-only, dep-free) holds `AstGateMask`, all `AST_SG_*`, and the M3 vocabulary bridge: `ast_gate_from_so`/`_to_so` (superopt 4-bit gate) and `ast_gate_from_perfn`/`_to_perfn` (perfn `{1,3,7}` cfg) on one 64-bit mask, with superopt-only axes (`AST_SG_{PROMOTE,INLINE,NOCALLFUL,CPROPJOIN,CSEJOIN}`) in bits 6.. so they don't collide. Selftested by asttool `suite_gatemap` (ctest `ast/gatemap`). **GOTCHA:** the whole search subsystem in `mccast.c` is inside `#ifdef MCC_INTERNAL` (line 518), which `mcc.c` defines and asttool does not — anything asttool must test has to live OUTSIDE that guard (hence the header). This is the `mcccombo.h` "selftested before wired" pattern.
- The candidate lattice is `subsets(searchable)` where `searchable = base | opt-in-knobs`, so the search can ADD knobs that are off in every -O baseline; the memo-hit mask is `& searchable`, not `& base`.
- **Hits tie-break** (`ast_search_pack_score`): score packs `(primary<<12)+(HITMAX-clamp(hits))`, so equal-cost candidates favour the config whose algorithms fired more on this slice; base still wins its own ties. Changes -O4+ winners only.
- **GOTCHA for future knobs:** during any -O4+ compile the out-of-process superopt spawns workers that inject a MINIMAL per-fn gate config (observed `base` is always `narrow|sethi`=10, even at -O4), so **droppable** knobs — ones that need their gate present in base — are shadowed and never exercised; only **addable** knobs (which `searchable` ORs in) reliably fire. Diagnose with `-v128`: the winner TRACE prints `base=` and `searchable=`. Prefer the addable pattern until M3 unifies the two searches.
- **`MCC_AST_SEARCH_ORDERED`/`_ORDER` are correct but pay nothing yet.** Order-honoring emit exists (`ast_strat_order[]`/`_n` indirection in `ast_func_end` and both scorers, identity default = frozen order; winner order persisted in the disk memo, MAGIC 0x4644; `MCC_AST_STRAT_ORDER="i,j,k"` forces an override). **KEY FINDING: order is *byte*-non-confluent but the 16 strategies are *cost/size*-confluent** — reordering doesn't change the score, so the search keeps identity. A scoring gain needs a pass whose reordering changes cost/size (inline/promote per D6, gated on §22 scratch-`Section`; or a size-scored reassoc). Over a pure gate-bitmask vocabulary, permutation mode is a no-op by construction.
- **Pipeline cycle-to-fixpoint** (`MCC_AST_CYCLE`): `ast_run_strat_cycle` re-runs the sequence until 0 hits (cap `AST_CYCLE_MAX=8`, non-silent), in emit and both scorers. Under default gates there is NO cross-pass cascade — the passes' internal fixpoints cover it; a real cascade needs a tree-EXPANDING pass (`MCC_AST_DIVMAGIC=1` gives `iter1 h=3, iter2 h=1, iter3 h=0`).
- **Arena inliner** (`MCC_AST_INLINE_PASS`): `ast_inline_graft` is emit-time codegen, NOT an arena op, so a separate arena inliner exists (`ast_inline_run` + `ast_inline_copy_expr` cross-arena expr copy). It covers pure-expression callees only — no locals/control-flow, hence no frame/label remap, avoiding the §34b and arm64 hazards — and makes inline a reorderable tree-expanding `AST_STRAT_INLINE` row so `fold→inline→fold` works. **Promote is confirmed NOT decouplable** (register-binding emission is irreducibly late), and promote-as-a-search-axis corrupts reg-alloc/spill state that `AstScratchSave` does not isolate (caught by a stage3 self-host miscompile).
- **Settled runner architecture (D1–D6), not yet built past step 3:** candidates become ordered sequences with repetition, with a correctness mask guarding only insane/illegal combinations (nearly empty — the safety model already makes any order correct); DFS + BFS as selectable root runners AND runners as rows in `ast_strategies[]`, with runner identity in the memo key; repetition matters only across **interleaving** (`fold→inline→fold`), so the runner is a fixpoint over the sequence and per-strategy 0-hits is the convergence signal (no explicit SKIP protocol); unify the SUBSTRATE not the ROLES — one enumerator, one memo, one **estimator interface** where score = predict quality and forecast = predict eval-cost (the M7 item); ALL opts become strategies, which needs §22 scratch-`Section` isolation for inline/promote. Remaining build order: sequence+repetition vocab + cycle-to-fixpoint runner, DFS/BFS generators, runner-as-strategy + memo identity, unified estimator, then D6.
- **Tiered validation (velocity rule):** the full bar — especially self-host — is the phase-BOUNDARY gate, not a per-edit gate. Inner loop = cmake-trace build + `-v128` TRACE micro-test + native ctest + a short fuzz burst.

### Const-data analysis (M5/M6)

- `ast_hook_data(sec,off,size,is_ro)` in `mccast.c` (mirrors `ast_du_*`) records per-object {section,offset,size,rodata?}, hooked at `decl_initializer_alloc`'s `put_extern_sym`/`vpush_ref` in `mccgen.c` under `#if MCC_CONFIG_OPTIMIZER`, after the `mccast.c`-include-at-EOF so the hook is declared in `mccast.h` and defined non-static. Queryable via `MCC_AST_DATA_REPORT` + `-v128` TRACE.
- `ast_data_estimate` runs `combo_pipeline_search` — the same `combo_run` permutation engine the gate search rides — over each object's emitted bytes to find the best codec CHAIN (depth ≤3, RLE/LZSS/LZW), flagging >50%-shrink objects as datacomp candidates with the winning chain (32..8192 B window, ping-pong buffers). A chain beats a single codec: `const int[256]={0}` → `rle+lzss` 1024→5 B vs 16 B single. `ast_data_roundtrips` runs `combo_pipe_apply`→`unapply` + memcmp before counting a candidate — only bit-exact chains are accepted (a datacomp rewrite trusting a lossy chain would silently miscompile) and it doubles as a decoder bug-hunt. **Needs a 3rd ping-pong buffer `ast_data_pipe_c`** to hold the compressed blob (apply and unapply share a/b). The ctor+runtime half (M6 C/D) is not built.
- **`MCC_ZERO_BSS`** (all-zero `static T x[N]={0}` → `.bss`): in `decl_initializer_alloc` right after `decl_initializer`, truncate `data_section->data_offset` back to `addr`, `section_add(bss_section,…)`, re-bind via `put_extern_sym(sym,bss_section,new_addr,size)`. Relocs are symbol-keyed (`greloca` stores `sym->c`) so re-homing the symbol auto-fixes all refs. Guards: `v&&sym`, `sec==data_section`, `!bcheck&&!asan_g&&!flexible_array&&!TLS`, **last-alloc** (`addr+size==data_section->data_offset`, which auto-excludes the bcheck byte / asan redzone), and **no reloc emitted by the initializer** (`data_section->reloc->data_offset` unchanged across `decl_initializer`). **THE RELOC GUARD IS CRITICAL:** a pointer init (`={0,&g}`) leaves zero *bytes* but carries a reloc, and moving it to NOBITS silently drops the pointer — byte-scan analysis alone flags such objects. Deferred: TLS tdata→tbss, asan/bcheck cases.
- **`MCC_MERGE_STRINGS`** (rodata pooling, `-fmerge-constants`-style): SOUND because C11 6.4.5p7 leaves identical literals' distinctness unspecified — **unlike const-array dedup, which C11 6.5.9 FORBIDS: distinct named objects need distinct addresses, so do NOT implement array dedup.** Value-use literals (`str_init`, `mccgen.c:8593`, `v==0`) become anon symbols via `vpush_ref`/`get_sym_ref` with symbol-keyed refs (`greloca(sec,vtop->sym,…)`, `mccgen.c:11518`), so the same symbol-rebind as `MCC_ZERO_BSS` applies. `ast_strpool_find_or_add` is a hash + exact-memcmp content pool keyed on bytes+size+align, per-TU (reset in `ast_configure`). On hit: **`memset(rodata+addr,0,size)`**, then truncate `data_offset`, then re-home the symbol. Guards: `v==0&&sym`, `sec==rodata_section`, last-alloc, `size>0`; `char arr[]="x"` is excluded (`v!=0`). **The memset is load-bearing:** truncate-reuse left dirty bytes and the next literal's memcpy relies on a pre-zeroed trailing NUL — a format string lost its NUL and read a stray character. It restores the "bytes at/after `data_offset` are always zero" invariant. Suffix/tail sharing also works (`"bar"` folds into `"foobar"`): refs are `symbol+addend`, so re-homing to `A_addr+(A_size-size)` lands every ref in A's interior (C11 6.5.2.5 footnote allows overlapping-representation sharing), guarded by `interior_off % align == 0`.
- **Pattern proven repeatedly: research → symbol-rebind guarded implementation → full bar.**

### Codegen folds — what exists, and where the boundary is

All matched on the recorded AST and spliced in place. **Reusable recipe:** model on `ast_bf_run`, reuse `ast_bf_cmpconst`/`keyexpr`/`lit` builders + `ast_ident_pure`/`cval`/`same`/`etype`, splice like `ast_bf_try_land` (`set_op`/`type`/`clear_children`/`add_child`), then wire the strategy table + `ast_env_gate` + the `exec-narrowfix` env + fuzz `GATES[]`. A cross-file hook called from `mccgen.c` (which includes `mccast.c` at EOF) must be declared in `mccast.h` and defined non-static, and env vars set in `ast_configure` must be declared BEFORE it (~line 560, not in a later block) — plus `extern int ast_<name>_env;` in `mccast.h` and a non-static def in `mccast.c`.

- **Range predicates** (`ast_range_run`, `MCC_AST_RANGE`, `AST_SG_RANGE`): `lo<=x && x<=hi` → `(unsigned)(x-lo) <= (hi-lo)`, and `ast_range_try_lor` for `x<lo || x>hi` → `(unsigned)(x-lo) > (hi-lo)` (TOK_UGT). Matches two SIGNED relationals over the same pure key, one lower + one upper, `lo<=hi`; `<`/`>` normalized inclusive, INT64_MIN/MAX rejected, unsigned keys skipped (they use TOK_ULE). Only the `&&`-as-condition `TOK_LAND` form is visible — value-context `&&` desyncs. 64-bit capable.
- **Constant div/mod strength reduction** (`ast_divmagic_run`, `MCC_AST_DIVMAGIC`, `AST_SG_DIVMAGIC`) is COMPLETE for 32-bit: unsigned `mag.a==0` and `a==1`, signed pow2 (both signs), signed non-pow2, div AND rem. Magic constants come from `src/mccmagic.h` (`mcc_magicu`/`mcc_magics`, Granlund-Montgomery / Hacker's Delight, plus `mcc_div*_apply` reference sequences), proven EXHAUSTIVELY by the `ast/magic` ctest (divisors ±2..20000 × boundary-heavy dividends). Duplicated mul-high costs 2–3× `imul`, which still beats `idiv`; the optimal 1× form needs temp materialization (a Store to a fresh local + Loads) — **reordering DIVMAGIC before CSE does NOT merge the mul-high subtrees; that was tried and reverted.** 64-bit is genuinely blocked: it needs the high 64 of a 64×64→128 product, a `mulh` op the AST cannot express, so it is a per-backend primitive (x86_64 `mulq`→rdx, arm64 `umulh`, riscv `mulhu`, i386 helper).
- **Branchless abs / clamp** (`ast_abs_run`, `MCC_AST_ABS`, `AST_SG_ABS`), modes 0/1/2/3 = abs/-abs/max0/min0 via `ast_abs_signmask`: matches ternary `AST_If` op5 `[cond,tval,fval]` with `cond = x REL 0` signed pure. abs → `(x^(x>>31))-(x>>31)`; ReLU `x<0?0:x` → `x-(x&s)`; `x<0?x:0` → `x&s`. 32- and 64-bit (`ast_abs_signmask` is parameterized with (ty,sh)). Note `-x` is `AST_Binary('-',[0,x])`, not `AST_Unary` (mccgen does `vpushi(0); gen_op('-')`).
- **Reassociation family** (`ast_reassoc_run`, `MCC_AST_REASSOC`, `AST_SG_REASSOC`): `(x OP c1) OP c2` → `x OP combine(c1,c2)` for `&`,`|`,`^` (exact), `+`,`*` (modular, machine-equivalent) and shifts (only when `c1+c2<width`); mixed additive (`(x±c1)±c2` → `x + net`, backend turns `+neg` into sub); commutative-aware (for `+,*,&,|,^` the const may be on either side of inner AND outer; `-`/shifts keep const-on-right, so `3-(x+5)` correctly does NOT fold); `(x<<c)>>c` unsigned → `x & (~0>>c)` (`ast_reassoc_shlshr`; signed excluded, arithmetic `>>` is sign extension); `(x>>c)<<c` → `x & ~((1<<c)-1)` (`ast_reassoc_shrshl`, valid for both signednesses); multiply distribution `(x*C1) ± (x*C2)` → `x*(C1±C2)` incl. bare+mul (`x + x*C`, treating a matching bare `x` as `x*1`, requiring ≥1 real multiply so `x+x` stays add). `combined ∈ {0,1}` is skipped — the "become x" node-copy is fragile child-reparenting.
- **CRITICAL TOKEN GOTCHA:** the PARSER emits `TOK_SAR` (`'>'` = 62) for `>>` in BOTH signed and unsigned code — logical vs arithmetic is decided by the OPERAND TYPE (VT_UNSIGNED), not the token. `TOK_SHR` (0x8b=139) is a different value used for CREATED nodes. So to MATCH a parsed `>>` check `op==TOK_SAR` + type; to BUILD a logical shift use `TOK_SHR`. `TOK_SHL` = `'<'` (60). Tokens at `mcc.h:1099-1101`.
- **Do NOT re-implement these as AST folds — the backend already does them:** `x*2^k`→shl, unsigned `x/2^k`→shr, unsigned `x%2^k`→and, `!(a<b)`→single setge, `!!x`→single setne, and the algebraic identities `x&0`, `x|-1`, `x*0`, `x*1`, `x+0`, `x&-1`, `x|0`. Signed `x/2^k` is NOT backend-reduced (rounding correction). The only non-redundant peepholes found are the default-on V-ident folds: `x OP x`→const (all 10 relational ops, int-only so the float NaN idiom stays safe; result typed VT_INT because relational nodes have `type_t==0`) and unsigned `u>=0`→1 / `u<0`→0 (the op is signed TOK_GE/LT with VT_UNSIGNED on the operand type, NOT TOK_UGE).
- **`cond?1:0` is a FRONTEND fix, not an AST fold.** `expr_cond` (`mccgen.c:9932`) has an `is_cond_bool && is_cond_bool` fast path that lowers `cond?bool:bool` via gvtst/gjmp AND returns early BEFORE `ast_hook_ternary_end`, so those ternaries DESYNC and the optimizer never sees them. Routing that path through the setCC/VT_CMP materialization the backend already has for `!(a<b)→setge` is core default codegen, target-sensitive and golden-churning. **General rule: when a ternary/cond fold "doesn't fire", check whether the FRONTEND materialized or desynced it (grep `ast_hook_ternary_end` early-returns) before assuming a guard bug.**
- **min/max stays cmov-territory:** `b^((a^b)&-(a<b))` is ~5-6 dependent ops vs a well-predicted `cmp;jl` (~1-2 cyc) and risks regressing predictable branches — which is why gcc uses cmov. abs and clamp-to-0 are different: a constant arm makes them a clear 3-op win.
- **LESSON, reinforced five times: suboptimal-but-correct-and-winning is NOT a blocker.** abs, signed div, signed rem, unsigned `a==1` and negative pow2 were each deferred on a "blocker" that was either a one-minute check or merely an extra `imul` that still beats `idiv`. "Check don't assume" also applies to already-landed folds — reassoc had a commutativity coverage gap found only by re-checking. The boundaries that held: setCC-bool, cmov, 64-bit mulh, CFG-lattice (SCCP/GVN/jt/global-DSE), and the structural M5/M6/M3 items.
- **Template for landing a risky pass:** find the invariant that makes a bounded slice provably correct (DSE: non-escaping locals can't be call-written; TCO: pointer store == int store), gate it default-off, then validate exec + fuzz + self-host + cross-arch.
- Validation infra to reuse: exec-corpus ctest variants (`exec-narrowfix` = knobs forced on at -O2; `exec-search`/`-emitsize`/`-threads` = the 3 search scoring paths at -O4, with `MCC_SEARCH_WORKER=1` to suppress the superopt spawn that otherwise collides with the exec runner's `-o`) plus fuzz `GATES[]`.
- Tooling gotcha: `sed` with `|` in the replacement breaks (unknown-option) — use an editor for lines containing `|`.

### Recorder fidelity — the ceiling on every optimizer number

An optimizer pass cannot run in a function the recorder did not model faithfully, so recorder fidelity bounds every optimization figure in this file, and fidelity work has repeatedly been worth more than new passes. mcc's own TU sits at ~44 unfaithful functions with the ratchet baseline banked at 159.

- **`MCC_AST_MEMBER_AGG`, `MCC_AST_MEMBER_CONST`, `MCC_AST_CMP_INVERT` are default-on and do NOT go through `ast_env_gate`** — each is a raw `getenv(...)` with a hand-rolled `e && e[0] ? (strcmp(e,"0") ? 1 : 0) : 1` idiom in `src/mccast.c`, so grepping for `ast_env_gate("MCC_AST_MEMBER…")` finds nothing. `CMP_INVERT` is a deliberate no-op on arm64 (it desyncs rather than mismodels; there the arm64 baseline, not the model, is what is wrong).
- Default-on at `o4 || optimize >= 2`: STOREVAL_CONSTL, CONVERT_GV (cast-of-lvalue `gv` — the largest single producer), CALL_NORETURN, STOREVAL_CALLSTORE, FNEG, CMP_MAT, CHAINSTORE_LIVE, CHAINSTORE_MEMBER, WHILE_COMMA (all three loop forms), TERNARY_DISCARD. The constructs behind them: parse-side `gv` in `unary()` after casts, `gen_opif(TOK_NEG)` with no hook, comma side effects before the loop node, the noreturn `CODE_OFF`, and replay-order mismatches (`vsetc`'s `vcheck_cmp` vs replay's raw `vpushv`; chained stores re-materializing).
- **To measure a gate's coverage delta use the FULL `src/mcc.c` TU**, never `tests/exec` or the `optfire` corpus — those give ±0–2 functions because they contain almost no nested-struct member access. Older figures derived against the "48% unoptimizable" ceiling are stale (it is ~41%); re-measure before quoting.
- The full-dump instrument is `MCC_AST_VERIFY_DIFF=<name>,full` (or `full`); decode deltas with objdump on the dumped hex. `gv`/`gv2` carry `MCC_TRACE_IF` value-traces.
- **Verify-faithfulness does NOT cover the promoted re-emit path:** a replay can be byte-faithful in the verify run and WRONG under promotion when a recorded register-finalized copy goes stale. Always run the runtime differential (-O1/2/3 vs gcc) and the fuzzer per gate.
- **Ternary `AST_If` records op 5 — the SAME op as a condless for-loop at statement level;** discarded ternaries are re-tagged op 7. Watch for this collision in any new statement-level `AST_If` consumer.
- Remaining tail: mov-only register-order deltas, the void-arm ternary byte-mirror, 3+-member chains, `store_packed_bf`, and A1a `nocode_wanted`.
- **`git show --stat` before trusting a commit message about landed behavior** — `8e867f40` describes a code fix that only landed as a TODO edit.

### Default-on policy

16 optimizer gates are default-on at `optimize >= 2` — `MCC_AST_RANGE`, `MCC_ZERO_BSS`, `MCC_MERGE_STRINGS`, `MCC_AST_NARROW_FIX`, `MCC_AST_SCCP_FIX`, `MCC_AST_DSE_CALL`, `MCC_AST_TCO_PTR`, `MCC_AST_CSE_COMM` (in `ast_func_end`'s config block, `src/mccast.c` ~867-879) and `MCC_AST_NARROW_ELIM`, `MCC_AST_VLAT`, `MCC_AST_ARGFWD`, `MCC_AST_SETHI_NARY`, `MCC_AST_SPILL_SHARE`, `MCC_AST_INLINE_PASS`, `MCC_AST_CYCLE`, `MCC_AST_COLOR` (in `ast_configure`, ~1168-1283). `s1->optimize >= 2` is the convention for a default-on transform. VLAT default-on enables only the landed path-sensitive narrowing; PR-C IV-widening is a separate held concern.

- **Held opt-in:** `MCC_AST_DIVMAGIC` (see below) and `MCC_AST_ABS` (branchless bit-trick vs branch tradeoff — gcc prefers cmov, which mcc lacks), both waiting on backend cmov/csel parity.
- **`MCC_AST_DIVMAGIC` must NOT ship default-on at -O2.** With it on, mcc's stage2 SIGSEGVs recompiling `src/mcc.c` — a deterministic stack overflow via runaway self-recursion in a function containing no mul/div. The quotients are correct (differential fuzz is clean); the fault is a **non-local x86_64 mul-high (`imul; shr $0x20`) register-allocation bug that only manifests under the register pressure of a heavy TU.** Masks: i386 and arm64 self-host both compile `mcc.c` cleanly with it on, the whole exec corpus and ctest pass, and `-g` builds pass (regalloc changes hide it). **The rule: before flipping ANY codegen knob default-on, run the x86_64 3-stage self-host at -O2 explicitly** — fuzz + exec + cross-arch never build a TU heavy enough. `MCC_AST_X=0` force-disables even a default-on knob (`ast_env_gate` treats "0" as off), which is how you bisect a flip.
- There are **no static byte-goldens** in this repo (mcc-vs-mcc `dash-s` byte tests + gcc/clang `diff3` differentials + runtime-stdout `exec/*` goldens), so a semantics-preserving byte change churns ~0 goldens. Forced-on ctest variants that already exist: `exec-zerobss`, `exec-mergestrings`, `exec-narrowfix`.

### §32c PRE (`MCC_AST_PRE`) — hoist-only by design

`ast_pre_run` hoists a pure integer Binary E out of an if/else into a fresh carved temp when the post-join reuse already evaluates E on every path. Coverage: both arms' single Store holding E; multi-statement arms (`ast_pre_arm_store` = the arm's LAST statement is a Store of E); Convert-wrapped RHS (`ast_pre_binary_of` peels `AST_Convert` to the inner Binary, the temp takes E's own type and the Converts re-wrap the temp read); reuse-distance scan (the reuse may be a later sibling — stop when an intervening statement writes an operand of E or carries a label); single-arm anticipability (only ONE arm's last Store need be E, sound because the reuse already evaluates E on every path).

Soundness rests entirely on `ast_licm_operands_ok(if_node, E)` + `…(reuse_store, E)` + `ast_cse_regpure(E)` — no div/mod/volatile, so nothing trap-speculating is hoisted. **Do NOT add speculative arm insertion** (computing E on a path where it is not guaranteed to reach a post-join use): arm-insertion is what killed an earlier general prototype on the arm64 backend, and it can pessimize cold paths. It needs a cost model, the arm64 self-host gate, and a diamond fuzzer before anyone tries again. The fuzzer does not emit the diamond shape, so gate-on fuzz only proves byte-safety — use a bespoke generator (`dgen*.py`-style: mixed-width targets, multi-statement/intervening/negative cases) through both `./cmake-debug/mcc` and `xmcc` under qemu against gcc. Permanent lock: `cli/pre_diamond`.

### §27 loop transforms

Interchange (`MCC_AST_INTERCHANGE`), fusion (`MCC_AST_FUSION`) and tile-and-interchange (`MCC_AST_TILE`/`MCC_AST_TILE_SIZE`) run in `ast_func_end` on the faithful path before the -O4 search, with the fired flag OR'd into the `AST_PF_EMIT` re-emit. Tiling strip-mines the inner loop of a 2-deep perfect nest and hoists the strip loop outermost, gated on `ast_loop_interchange_legal` (≡ full permutability for a 2-deep nest — both canonicalize dependence vectors and reject the lone `(<,>)`); it mints the strip IV as a fresh replay-time stack slot the `ast_ltemp` way (`off=(ast_ltemp_cur-8)&-8`, frame reserved via `loc=ast_ltemp_cur`) and synthesizes the strip cond/incr/`&&`-guard with `ast_bf_lit`/`ast_bf_bin`.

- **Gotcha #1 (cost a miscompile):** the affine-dependence test (`ast_dep_collect`) models ONLY array loads/stores through a computed `AST_Load` address. It is BLIND to function calls (`AST_Invoke`), scalar-local carried deps (a Store to a bare `AST_Ref`, e.g. a non-associative accumulator), early exits, and nested control. **Any iteration-reordering transform MUST gate on `ast_interchange_body_ok`** or it silently miscompiles.
- **Gotcha #2 (crashed the compiler with a tree cycle):** when grafting an EXISTING linked node under a new parent via `ast_add_child` (e.g. `newcond = LAND(old_cond, guard)`), `ast_add_child` clobbers the old node's `next_sib` and severs its former siblings. ALWAYS `ast_li_list_remove` from the old parent FIRST, then graft, then re-insert. Fresh nodes are safe; only reused linked nodes bite.
- Machinery: `ast_li_list_{remove,insert_before}`, `ast_li_append_children`, `ast_li_prev_sib`, `ast_li_refs_off`, `ast_interchange_is_init`, `ast_interchange_body_ok`, `ast_tile_local_ref`. A `for` loop is `AST_If` op 3 with children `[cond, incrBB, bodyBB]`, incr and body being BasicBlocks.
- Per-transform validation: default ctest byte-identical; an arch-neutral `exec/optimizer/loop_*.c` golden (`long long`/`%lld`, verify gcc `-m32`==`-m64`); an `exec-<name>/` variant forcing the gate on; gate-on differential -O0..-O6 vs gcc; the gate in fuzz `GATES[]` + ~120 seeds; shadow 0-divergence; sanitizer clean. Build with `cmake-debug` (optimizer ON) — `cmake-ast` has it OFF. The §27 analyses live inside `#ifdef MCC_INTERNAL`, which `tools/asttool.c` excludes, so they cannot be unit-tested there without cross-guard code motion.
- Remaining: tiling is v1 (one tile per function, const inner bound, unit stride, outer loop not also strip-mined ⇒ not true 2-D cache tiling); dependence-test symbolic bounds and precision.

### Runtime JIT (§26) — mechanics and rules

**Settled model.** The recompile trigger is both an `.init_array` startup ctor AND a `jit-profile` hot counter; guarded deopt ships first, with `eval_slice` as later hardening; ELF x86_64 is the reference and arm64/riscv64 a mechanical port; the worker pool is pthreads via `runtime/include/threads.h` (a real C11-over-pthreads shim, `thrd_create:59` — the old "threads don't exist, use fork" note is stale), not fork. **The baseline is the AOT-optimized shipped code (final emit), NOT the pre-fold parse-time body:** the JIT produces a further-optimized variant specialized to an observed context, keyed by a hash of that context (§21 epoch/cache key ⊕ §25 hot-value key, carrying `ast_search_key_salt`), the cache maps `key → best-known variant`, and the dispatcher deopts to the AOT baseline on guard-fail or key-miss. A key-miss never inline-recompiles; there is one best variant per sym (overwrite); the guard re-checks on entry so a colliding hit self-corrects.

- **Dispatch modes** (`MCC_AST_JIT_DISPATCH`): 1 = never-deopt, 2 = always-deopt (both arms the AOT baseline, synthetic guard), 3 = non-null speculative specialization (real per-ptr-param `cmp [rbp+off],0; jz deopt`; spec arm = arena CLONE + `ast_nonnull_fold` + sccp; deopt arm = AOT splice; sound only for `ast_local_is_readonly` param slots, and the clone keeps `ast_cur` pristine so inlining is preserved), 4 = const-param, 5 = value-range (bound from STATIC `ast_vlat_context`), **6 = the indirect variant-slot entry, and the only mode wired to the runtime engine**. Modes 1–5 are dead in production but their selftests still exercise them.
- **Mode-6 mechanics:** `jmp *SLOT(%rip)` into an 8-byte writable `.data` slot, `R_X86_64_64` slot→text-local-sym at **`aot_base+6`, NOT `func+6`** (that lands mid-prologue), splice `src_base=aot_base`, publish via `mcc_jit_publish` (plain aligned 8-byte `__ATOMIC_RELEASE` store — x86-TSO) with never-free reclamation (bounded leak, correct; QSBR/epoch via `tss_*` is the later upgrade). Reloc idioms to copy: `gen_addrpc32`/`gen_bounds_epilog` (`x86_64-gen.c:114/637`), `get_sym_ref`+`section_ptr_add` (`mccgen.c:308`). The splice primitive `ast_baseline_splice` copies bytes, rebases reloc `r_offset` only, and rethreads the return chain.
  - On **arm64**, keep the address-of-slot computation PC-relative and self-contained (`adrp/add`, `ADR_PREL_PG_HI21`/`ADD_ABS_LO12_NC`) — a GOT-based load against the local slot symbol corrupts the fn symbol when the search re-emits candidates to MEMORY.
- **KGC stubs are DISPATCH-ONLY — never direct-call one.** The hand-emitted differential-verify stubs (`mccjit_make_kgc_stub_n`/`_fp` ~:3145, `mccjit_make_trampoline` ~:1107) open with a `0xC9` LEAVE and assume the AOT function's prologue supplied the frame. A direct `call` runs that LEAVE against the C caller's frame and the trailing `ret` returns to garbage — the signature is the process "exiting" with the result in eax (`exit_group(11) = f(5)`), no crash signal. This burned two separate debugging sessions. Enter via `jmp *slot`, or use `mccjit_dispatch_entry(void **slot, fallback)` (x86_64: mmap a `push rbp; mov rbp,rsp; movabs rax,&slot; mov rax,[rax]; jmp rax` trampoline; otherwise the raw ptr). To test a stub end-to-end, compile a program that calls the JIT'd function with `MCC_AST_JIT_DISPATCH=6 --embed-jit` and `mcc_run` in-process so the `.init_array` ctor swaps the slot; to test marshalling in isolation, call `mccjit_kgc_calln[_fp]` directly.
- **Impure functions are never swapped.** The KGC stub double-executes (baseline + variant) to differentially verify, so an impure fn doubles its side effect (observed: `double free or corruption`). `mccjit_last_kgc_ok = scalar_ok && purity != AST_PURITY_IMPURE` (`mccjit_embed.c:200`) routes them to the refused-unverified path. Purity comes from `ast_fn_purity`: the minimal sound whole-function classifier rejects any `AST_Store`/`AST_Invoke`/`VT_VOLATILE`; **TIER-0** (also Load-free) is memoizable, **TIER-1** (Load present, e.g. a pointer-param reader) is safe only for IMMEDIATE re-run because the result depends on memory that can change between calls — so cross-call memoization is sound only for Tier-0 unless the key includes the loaded values.
- **Engine-internal names must be excluded from stashing.** `mccjit_engine_internal()` filters `mccjit_`/`mcc_jit_` plus `mcc_log_enabled[_st]`/`mcc_trace_at`/`mcc_logf`/`mcc_log_tag` out of `mccjit_embed_note`, because the JIT swaps `mcc_log_enabled` and any `MCC_TRACE` inside the swap machinery re-enters the stub → stack-overflow recursion in TRACE builds. **A re-entrancy guard inside the stub cannot work — its own MCC_TRACE fires before any guard.** No-op for ordinary programs; only self-host carries those names.
- **`ast_fconst_reuse` must be disabled on recompile.** During `ast_replaying` it returns a recorded `.rodata` offset to REUSE, skipping the `init_putv` data write in `gv()` (`mccgen.c` ~1922) — true for in-process AOT replay (same section), but a JIT recompile replays into a FRESH MCCState with an empty `rodata_section` while the global `ast_fconst[]` still holds the original offsets, so FP constants referenced never-written slots and came out `0.0`. `mccjit_recompile_common` wraps `ast_reemit_extern` with `ast_fconst_reuse_disable(1)/(0)`; AOT replay is unaffected. Locks: `jit/selftest-fparg`, `jit/selftest-mixed`. **Debug lesson: when a recompiled fn returns wrong, disasm it and inspect the referenced rodata bytes before assuming codegen.**
- **Recompile must not leak state into the host compiler.** `mcc_enter_state`/`mcc_exit_state` (`libmcc.c:76/83`) is a FLAT non-saving swap while mcc's codegen working state (`global_stack`, token/sym tables, `vtop`, `cur_text_section`, …) are process-GLOBALS, so `mccjit_recompile_common` must snapshot `global_stack`/`local_stack` after `mccgen_init` and `sym_pop(...,0)` back to the snapshot before `mcc_exit_state` (mirroring `mccgen_finish`; the variant is already relocated via js's ELF symtab, so the transient Syms are safe to drain). Without it the host's own `mccgen_finish`→`sym_pop`→`sym_link` unlinks a Sym whose token's `table_ident` slot is gone → SEGV, and it needs ≥2 JIT'd functions to appear.
- **Serialization constraints** (`mccjit_intent.c`, format `MCCJIT_INTENT_FORMAT`): a leaf integer function's AST-node closure is empty cross-session, but `ast_reemit(sym, arena)` (`mccast.c:11128`) NEEDS `sym->type.ref` = the FUNCTION SIGNATURE (FUNC type: `.type`=return, `.f`=attrs, `.next`=param chain), rebuilt by `mccjit_rebuild_sym`. `MCCJIT_ROLE_NAMED`/`ROLE_PTR` must serialize `s->c` and rebuild with `sym_push(...,(int)r->c)` — for an array that field is the element COUNT, and hardcoding `-1` made `type_size()` abort "unknown type size" on any fn subscripting a global array. Pointer params and CALLS are the classes needing the full CType graph. Names: `get_tok_str(v)` serialize-side, `tok_alloc(name,len)` runtime-side (user ids are not cross-session stable; builtins/keywords are, same binary). **The serializer must bail on anonymous NAMED handles** (string literals intern as NAMED with no name → `mccjit_build_rec` returns NULL → node keeps `VT_SYM` with a null sym → codegen deref): return -1 so the caller keeps the AOT baseline. Recompile flow: `mcc_new`→nostdlib→MEMORY→`enter_state`→`mccpp_new` (**inits the token table — CRITICAL**)→`mccgen_init`→deserialize→rebuild sym→`ast_reemit_extern`→relocate→`get_symbol`. Runtime symbol binding is free: `mcc_relocate` auto-resolves `SHN_UNDEF` via `host_dlsym_process=dlsym(RTLD_DEFAULT)` against the running program (so it must not set nostdlib), and a leaf needs zero resolution.
  - **OPEN landmine:** the serializer still crashes on VLA functions under `MCC_JIT=1` (VLA-local functions pass `ast_jit_eligible`) — harden it or reject VLA in eligibility. Relatedly `mccjit_embed_stash_leaf` (`mccast.c` ~13048) must stay gated `(ast_jit_dispatch_env || ast_jit_fns_n > 0)`, or with `MCC_EMBED_JIT` default-ON it serializes every faithful function even with the JIT off and SIGSEGVs on VLA functions.
- **Trigger precedence:** env `MCC_JIT` > `--jit`/`--no-jit` (for `-run`) > CMake `MCC_CONFIG_JIT` (BOOL, default ON → `MCC_JIT_DEFAULT`). `s->jit` is a tri-state `signed char` (default -1); the generated `.init_array` ctor in `mccjit_embed_finalize` gates at runtime (`getenv("MCC_JIT") ? e[0]!='0' : <def>`); `ast_jit_env = s1 && (s1->embed_jit || output_type==MCC_OUTPUT_MEMORY)`. **The transform lives in the AST-replay optimizer, so it needs `-O1+`** — a plain `-O0` embed exe is 4 KB with NO engine.
  - **OPEN (committed WIP):** that `ast_jit_env` definition is also true INSIDE `mccjit_recompile_common` (it builds its state with `output_type=MEMORY`), so the recompile's own `ast_reemit_extern` re-applies the mode-6 stub → `unresolved reference to __mccjit_slot_f` → NULL recompile, breaking 7 `jit/selftest-*` (lazy/pool/fork/observability/vrange/fparg/mixed). Fix: a `mccjit_recompiling` flag `&&`-ed out of `ast_jit_env` around `ast_reemit_extern`, or gate on `s1->optimize>=1` (recompile sets optimize=0).
- **Two slices, decoupled.** (1) **embed-less** — the in-process JIT for `-run`/`MCC_OUTPUT_MEMORY`, engine in mcc's own process, nothing baked into output: works, robust on simple programs, and the first-class path. (2) **embed** — baking the engine into a file output: immature (KGC variants crash on real programs like `tests/exec/vla/basic.c`), behind explicit `--embed-jit` with `s->embed_jit` default 0 (`libmcc.c` ~957). Gating: mode-6 emission (`mccast.c` ~12741) requires `ast_jit_dispatch_env != 6 || embed_jit || output_type==MEMORY`; the embed_note (~12765), `mccjit_embed_finalize` (`mccjit_embed.c` ~1875) and `mcc_add_runtime`'s finalize call (`mccelf.c` ~1604) gate on `embed_jit || output_type==MEMORY`. Net: `MCC_JIT=1` is safe on every output.
- **Standalone `--embed-jit` exe:** `MCC_AST_JIT_DISPATCH=6 MCC_EMBED_JIT_LIB=<builddir>/libmcc-static.a mcc -B<builddir> -I runtime/include -O1 --embed-jit --jit-functions f prog.c -o prog -lm -lpthread -ldl`, then `env -u LD_LIBRARY_PATH ./prog`. The engine link is `AFF_WHOLE_ARCHIVE` because the generic ctor (`__mccjit_boot_all` → `mccjit_boot_swap`) is synthesized *during output*, after the archive is linked, and **archive resolution is single-pass** so on-demand extraction can never resolve it. Under `MCC_BUILD_STATIC_LIB=ON` the archive ships as **`libmcc-static.a`** (shared as `libmcc-dynamic.so`), NOT `libmcc.a`, so plain `-lmcc` won't prefer it — the `MCC_EMBED_JIT_LIB` full path is the wiring. `cmake-embedjit` needs `-DMCC_BUILD_STATIC_LIB=ON -DMCC_BUILD_DYNAMIC_LIB=ON` for `jit/standalone-static` to run. `bin2c` bakes the archive into `mcc` as `mccjit_blob` (mirroring `mccrt_blob`); `mcc_add_jit_engine_embedded` writes it to a temp fd and whole-archives it; the embed link skips it when `find_elf_sym(s->symtab,"mccjit_boot_swap")` is already defined internally (self-host). `libmcc_jitengine` must be in the ~line 2260 `_mccdefs` `foreach` or it lacks `MCC_EMBED_JIT` and has no `boot_swap`. Test-infra: `tools/fixpointgate.c` strips `MCC_EMBED_JIT_BLOB` so the single-file self-host compile doesn't reference an unlinked blob, and `tools/build.c` emits `MCC_EMBED_JIT` under `--embed-jit` for the `config-defines` drift test. Deferred: the ~800 KB Tier-B slice needs a parser-less re-emit-only engine + `--gc-sections`; whole-archive currently pulls ~2 MB.
- **Embedded programs need only glibc, not `libmccrt.a`.** The boot recompile resolves runtime helpers (`__divdi3`, `__moddi3`, `memcpy`, `__mcc_c*`) in-process via `dlsym(RTLD_DEFAULT)` against the program image. Two lines make it work: `mccelf.c` ~970 `if (!s1->nostdlib || s1->output_type == MCC_OUTPUT_MEMORY)` (the resolver was gated on `!nostdlib`, but the embedded recompile needs nostdlib behavior to avoid pulling disk libs — one flag drove both), and `mccelf.c` ~1657 `if (MCC_MCCRT[0] && s1->output_type != MCC_OUTPUT_MEMORY)`. **The disk-libmccrt skip must be an ATTEMPT, not an unconditional skip:** `mcc_add_dll(s1, MCC_MCCRT, 0)` for MEMORY (silent if absent) — in-process compilers that link libmcc but not libmccrt (`abitest`, ctest `abitest-cc`) genuinely need the disk lib for helpers like `__va_arg`. Shared path: `mcc_relocate`→`mcc_relocate_ex`→`relocate_syms(...,do_resolve=1)` (`src/mccrun.c:314`). Residual: a helper the AOT program never linked won't dlsym-resolve, and that recompile falls back to the baseline.
- **PC32-to-external-data must not route through the PLT** (`mccelf.c:1255`). Codegen for a `VT_STATIC` global emits a direct `R_X86_64_PC32` load, but in the JIT the symbol is UNDEF (dlsym-resolved) and `code_reloc(PC32)=1`, so `build_got_entries` built a **PLT stub even for DATA** and the load read the stub bytes (`ff 25…`) as the value. Fix: `if (s1->output_type == MCC_OUTPUT_MEMORY) continue;` in the `AUTO_GOTPLT_ENTRY` block. **Output-type constants are VALUES, not flags** (MEMORY=1, EXE=2), which is why `output & EXE` is 0 for MEMORY and the existing EXE direct-rewrite at :1293 did not cover it.
- **Exporting internals for a self-host JIT is opt-in** (`MCC_JIT_EXPORT_INTERNALS=1`, default OFF). Recompiled fns reference mcc's own internals as UNDEF externs resolved via `dlsym`, which only sees `.dynsym`; mcc's symtab hashes only GLOBALs, so `ast_*` accessors just need rdynamic but `mcc_state`/`mcc_log_tags` are true STB_LOCAL and `find_elf_sym` misses them. `mcc_jit_export_local` (`mccelf.c`) LINEAR-scans `symtab_section` — **not `find_elf_sym`, locals aren't hashed** — and flips the first defined LOCAL match to STB_GLOBAL so `bind_libs_dynsyms` exports it. Kept opt-in because resolution removes the old "unresolved → keep AOT baseline" safety net, so a resolved-but-miscompiled variant actually RUNS.
- **`-g` (or `-ftest-coverage`) silently disables JIT baking.** `ast_try_active = ast_replay_env && !debug_modes` (`mccast.c:11439`), `debug_modes = do_debug | test_coverage<<1` (`mccgen.c:575`), so the whole AST-replay optimizer — where dispatch slots and `mccjit_embed_note` baking live — is off, `mccjit_embed_fns` is NULL at finalize, and you get an empty `.init_array` with no self-recompile. A `mcc_warning` now surfaces that early return (with a benign false positive under `-b --embed-jit`). **Reliable bake metric: `.init_array` present, or a `-c -r` object slot count** — `readelf -sW` on a linked EXE shows 0 because slot syms are LOCAL/stripped.
- **Debug technique for mcc-built binaries with empty symtabs:** temporarily relax the symtab-retention gate (`mccelf.c:2985`, `embed_jit && do_debug` → add `|| getenv("MCC_KEEP_SYMTAB")`) so a baked non-`-g` build keeps `.symtab` and gdb gives real frames; add `backtrace_symbols_fd` at the failing site. `-g` `.symtab` retention for `--embed-jit` exes is otherwise gated on `embed_jit && do_debug && !do_strip` (non-alloc, so no loaded-image change).
- **Validating hand-emitted AArch64 bytes from an x86_64 host.** Static: `llvm-mc` (`/usr/lib/llvm/22/bin/llvm-mc`), e.g. `printf 'ldr x16, #8\nbr x16\n' | llvm-mc -arch=aarch64 --show-encoding` → `[0x50,0x00,0x00,0x58]` / `[0x00,0x02,0x1f,0xd6]`; confirm every hand-written word. Execution: the host `ld` has no aarch64 support but `lld` cross-links — `clang --target=aarch64-linux-gnu -fuse-ld=lld -nostdlib -static -O1 -o out src.c` then `qemu-aarch64 ./out`, with the test written freestanding (own `_start`, raw `svc 0`: mmap=222, exit=93) to avoid needing an arm64 libc. **Arm64 needs icache maintenance after writing code** (x86 does not): `dc cvau,addr; dsb ish; ic ivau,addr; dsb ish; isb` per cache line, else the CPU may execute stale I-cache bytes — which is why the dispatch/stub tail is a per-arch hand-emitted primitive (`host_icache_flush`). Validator: `tests/qemu/jit_arm64_dispatch.c` + `.sh`, ctest `jit/arm64-dispatch` (self-skips without the toolchain).
- **SOC split of `mccjit_embed.c`** (for isolation and the embed size-slice). Done: `mccjit_intent.c` (serialize/deserialize/rebuild_sym) + `src/mccjit_internal.h`. **The pattern:** types used across TUs go in `mccjit_internal.h` (`MccjitBuf`/`MccjitTypeRec`/`MccjitIntent`; `MccjitReader`/`MccjitHandles` stay private); shared helpers become `MCCJIT_LOCAL` (non-static) with a header decl; the `MCCJIT_LOCAL` macro + `MCCJIT_KGC_MAXARG` + `MCCJIT_INTENT_*`/`MCCJIT_ROLE_*` live in the header; the amalgamation `#include`s the module .c before `mccjit_embed.c` under `#ifdef MCC_EMBED_JIT`; CMake ~1795 `list(REMOVE_ITEM MCC_CORE_SRC …/mccjit_intent.c)` then appends it inside `if(MCC_EMBED_JIT)`. Remaining modules: `mccjit_stub.c` (KGC/counter/dispatch/trampoline hand-emit, arch-conditional — trickiest), `mccjit_pool.c` (pool+QSBR), `mccjit_recompile.c`, `mccjit_boot.c`, `mccjit_selftest.c` (biggest line win). Build both configurations and `ctest -R jit/selftest` after each. **Always verify multisource (`cmake --preset linux-gcc-multisource`) when adding cross-file calls** — an earlier `--stats` change `#include`d `mccstats.c` only in the amalgamation block, leaving `mccast.c`/`mccjit_embed.c`/`libmcc.c` calling `mcc_stats_*` undeclared there.
- **Design not yet built:** M5b's known-good value cache (per-variant sorted mmap'd file of live-in tuples proven optimized==baseline; entry guard = membership test; a miss runs the baseline AND verifies the optimized form, inserting on match or permanently deopting on mismatch — **baseline execution IS the runtime oracle**) and M5c's pure/impure slicing (pure kernels get a bespoke off-C-ABI convention with register live-ins/outs, freely re-runnable; impure ops stay C-ABI "bound" calls once in order). The custom ABI is net-new backend work: `gfunc_prolog` spills all params to the frame and the dispatcher reads `[rbp+off]`, so no non-C-ABI register convention exists.
- **OPEN — nondeterministic UAF:** `MCC_JIT=1 --stats` crashes even on hello.c; `boot_swap→mcc_jit_recompile_blob→mccjit_salt_witness→mcc_trace_at→crash` (`movzbl 0x89cd0(%rdx,%rax,1)`, bad base). The trace fires during boot recompile only when stats are active → likely a dangling `mcc_state` after `mcc_exit_state(js)`/`mcc_delete(js)` in `mccjit_recompile_common` (`mccjit_embed.c:213/227`), §0c UAF territory. `-run` + stats + JIT works, so it is embed-boot-specific. Note the panic counters (`mcc_stats_jit_recompile`/`kgc_hit`/`miss`/`poison`/`promote`) are count-only for exactly this reason — painting the panel per event re-entered before `mcc_enter_state`. **valgrind cannot run on this host** (illegal opcode in `_dl_start`), so use a sanitizer elsewhere or mcc `-b`.
- Perf note: the JIT is not a compute bottleneck — KGC-on adds ~5% at -O2 and ~2% at -O10 search, with only ~15-20 fns eligible after exclusions. But **`--stats -O10 -c src/mcc.c` with KGC-on is very slow (>2 min, not a crash)** because KGC double-executes every hot fn on a huge TU.
- Remaining §26: M6 hot-counter lazy trigger, M7 patchpoint, M8 hard-gate promotion (flip `MCC_AST_JIT_EVAL_GATE` on after clean self-host + fuzz soaks; extend the oracle to statement-level control flow), M4 static-link/structs/general-trampoline, M5b general signatures (N-arg/float/struct marshalling), M5c full slicing, the cross-arch port, and finally flipping the JIT default-on after the full bar + soak.

### Replay frame invariants (why two clamps exist)

Both were real, default-path miscompiles; changing either needs the self-host gate.

- **`ast_loc_low` frame low-water clamp.** The optimized re-emit sizes the frame as `-loc` at `gfunc_epilog`, but `ast_alloc_loc` replay assigns `loc` from the captured slot list; an optimization that changes the alloc count desyncs it so `loc` ends shallower than the deepest emitted slot, the frame under-covers, and deep locals below rsp are clobbered by calls. Clamping `loc` to the min-loc-seen is a no-op for monotonic faithful replays ⇒ byte-identical. Symptom class: results that **vary run-to-run on the same binary** (an uninitialized-stack read) at plain `-O1`/`-O2` while `-O0`/`-O3` and gcc agree. Locked by `ast/inline-frame`. It was masked in CI because `ast/replay-inline` forces `MCC_AST_INLINE=1` and inlining changes stack allocation.
- **`ast_temp_frontier` / `ast_alloc_temp_loc`.** `ast_alloc_loc` records AST-local offsets pre-transform (they are stored in AST nodes and must stay stable on replay), but transforms between record and replay ADD stack allocations the record never saw — the promotion callee-save area (`ast_promo_entry_init`) and `get_temp_local_var` temps. Both used the shared `loc` frontier, which at replay start sits where recorded offsets live, so a scratch/promo slot overlapped a recorded 8-byte local and a narrow store clobbered its high dword (bit-34-corrupt `AstArena*` → crash in `ast_node`). Replay scratch now gets a dedicated monotonic frontier seeded to `ast_locrec_min`, and the frontier reset + `ast_loc_low` seed happen before `ast_promo_entry_init` in the replay macro.
- **Diagnosis gotchas for this class:** crash-vs-silent-corruption is layout-sensitive, so opt level is NOT a clean bisect (`-O1`/`-O0` may "pass" while still corrupt); `-g` disables replay (`ast_try_active`) and hides it; allocation-level overlap detectors in `ast_alloc_loc` and `get_temp_local_var` finding **zero** overlaps is informative — it points at a replay-time store-size/slot-size interaction rather than a bad offset; ASan reports only benign arena leaks because it is a wrong-width access of valid memory, not OOB.

### Sanitizers

- **`-fsanitize=undefined` provider** (x86_64 ELF/Mach-O, trap mode). Inline checks in `x86_64-gen.c:gen_opi`, gated by `mcc_state->do_sanitize_undefined` (a plain `MCCState` field, NOT under `MCC_CONFIG_DIAG_RT`); `gen_ubsan_check(cc)` emits `j<cc> .ok; ud2; .ok:`. Covers signed +/-/* overflow (OF→jno), variable-count shift ≥width-or-negative (`cmp ecx,width; jb`), div-by-zero (`test div,div; jnz`). Trap mode = `ud2` → SIGILL, no runtime lib; default-off ⇒ default codegen byte-identical. Predefines `__MCC_SANITIZE_UNDEFINED__`; parsed in `libmcc.c`'s `MCC_OPTION_f` case (`san_tok_eq`). Tests: `tests/sanitize/ubsan/` (`run_ubsan.cmake` sweeps -O0..-O3). Deferred: null-deref, INT_MIN/-1 div, recover mode, ASan, arm64/riscv64.
- **Bug-hunting mcc with `mcc_s`** (the `sanitize` preset): mcc built under host ASan+UBSan, and the preset also defines `SYM_DEBUG` so `sym_free` calls real `mcc_free` instead of pooling on `sym_free_first`, deliberately exposing `Sym` use-after-free. `ASAN_OPTIONS=detect_leaks=0:abort_on_error=0 UBSAN_OPTIONS=halt_on_error=0 mcc_s -B<build> -O2 -c <file>` over the corpus, grep stderr for `runtime error|use-after-free|nonnull`. Regression lock: `sanitize-selfcheck` (full `tests/exec` × -O0..-O3 under `mcc_s`, 0 diags). **Trap this build once produced:** `sym_free` had the `ast_sym_defer` call *inside* `#ifndef MCC_SYM_DEBUG`, so `mcc_s` disabled the very deferral that keeps replay-referenced `type.ref` syms alive — a guaranteed false positive. A SYM_DEBUG-only finding must be checked against production semantics before it is believed.
- **OPEN — `-fasan-shadow` native reporter can DEADLOCK the instrumented process** (`runtime/lib/mccasan.c`, `on_sigill`) instead of reporting and `_exit(1)`: 0% CPU, survives SIGTERM because it is stuck inside the signal handler. Repro (flaky, ASLR-dependent): `int main(void){int*p=malloc(40);p[0]=1;return p[100];}` compiled `-fasan-shadow`. The handler reads the faulting address from `REG_RCX`, which for this heap case carried a 32-bit-truncated garbage value (`0x00000000fffffffa`); `s = shadow(addr)` then lands **outside** the mapped shadow region `[0x7fff8000, 0x8fff7000)` and the dump loop `for(c=-8..7) s[c]` reads unmapped memory → nested fault inside the handler → wedge (confirmed a genuine deadlock: 90 s blocked, 0.00 s user time, needed SIGKILL). It can hang the whole ctest run via `cli/asan_shadow_native_overflow`, `_native_use_after_free`, `_native_global_overflow`, `_native_stack_overflow`, `asan_shadow_manual_link` (no per-test timeout in their definitions); the `_clean` variants don't fault. Fixes to consider: validate the faulting address and that `s`/`s±8` land inside the mapped shadow before dumping; root-cause the truncated RCX (likely the inline probe not materializing the full 64-bit address); and wrap those cli tests in `TIMEOUT`/`timeout -s KILL` so a reporter bug cannot wedge CI. The file has been under parallel edit — coordinate before touching it.

### Register allocation (§36) and the §41 default-on evidence

- `ast_color_graph` (simplify/spill/select) lives in `mccast.c`'s public region (arch-neutral, unit-tested by asttool `ast/color`) and is wired into `ast_plan_promotion` behind `MCC_AST_COLOR`. Live-range register SHARING is sound but **restricted to straight-line functions** (no `AST_If`/`AST_Jump`) because node-index intervals are unsound across loop back-edges — the general case needs CFG liveness. Per-candidate `[firstref,lastref]` with first-ref-is-def (`AST_Store` target), live-in params pinned to entry, interference = interval overlap, entry-init loads only the first candidate per register and sharers take the value from their def.
- Two traps from building it: `ast_color_graph` must init `color[]` **before** the `k<=0` early return (XMM class k=0 when callful → garbage reg → crash), and `ast_promo_{off,typ,reg}` need `AST_PROMO_MAX*8` sizing because sharing promotes more candidates than there are registers.
- **Placement gotcha: functions used by asttool must live BEFORE the `#ifdef MCC_INTERNAL` block (~line 393) in `mccast.c`**, else asttool link-errors.
- §41 evidence for the eventual flip: all five default-off codegen passes (SETHI / BITFLAG / CPROP_JOIN / CSE_JOIN / CALL_WINDOW) forced on at -O2 over the whole `src/mcc.c` keep the fixpoint converging and the self-built mcc correct, so they are correctness-safe to default on; what remains is a byte-golden re-baseline held for sign-off, not a fix.

### Frontend / diagnostics

- **Caret diagnostics, default ON**, suppressed with `-fno-diagnostics-show-caret` (FlagDef in `options_f`, field `MCCState.diag_no_caret`, FD_INVERT):
  ```
  foo.c:12: error: ';' expected
  12 |     int x = 1
     |              ^
  ```
  Renderer `append_caret_context()` in `src/libmcc.c`, called from `error1()`. Deliberately conservative: it renders only when the line is fully recoverable from the file's in-memory buffer (source streams in `IO_BUF_SIZE`=8K chunks), not during macro expansion (`macro_ptr`), and not for explicit-line (`%i:`) diagnostics — otherwise it falls back to the old one-line form, so no regression. The caret marks the current lexer position (just past the token): right line/region, approximate column.
- Golden harnesses opt out: `tests/exec/runner.c` injects the flag after `-B`, and the two caret-sensitive cli cases (`static_assert_fail`, `c11_features_in_c99`) pass it per-command because their grep pattern also appears in echoed source. Coverage: `cli/diagnostics_caret`.
- **Color:** `-fdiagnostics-color[=always|never|auto]`, `-fno-diagnostics-color`, plus clang's `-fcolor-diagnostics`/`-fno-color-diagnostics`; field `MCCState.diag_color` (0=auto, 1=always, 2=never), parsed in the `-f` chain. `error:` bold red, `warning:` bold magenta, caret bold green. `diag_want_color()` never colors the `error_func` embed path, and auto follows `host_stderr_isatty()` (`mcchost.c`: POSIX `isatty(2)`; **Windows returns 0, so auto stays off there — use `=always`**). TTY-gating keeps piped test output plain ⇒ zero golden churn. Coverage: `cli/diagnostics_color`.

### Native arm64 CI failures (open)

The native-arm64 jobs (`.github/workflows/ci.yml`, `runs-on: ubuntu-24.04-arm`) surface failures that never ran on arm64 before (only the qemu cross matrix did, with a small subset and `MCC_TEST_BCHECK=0`): `linux-gcc`/arm64 (Debug, bcheck ON) fails **atomics + bounds**; `linux-clang-release`/arm64 (`-O2`, bcheck OFF) fails **atomics + complex** (plus `mcctest`/`parts`, which aggregate those). Common cluster = atomics; the extra cluster tracks host-compiler/config.

**qemu-user CANNOT reproduce these** — with a real arm64 glibc stage3 sysroot and the cross `mcc-arm64` under `qemu-aarch64`, `atomic_counter`/`atomic_gcc_style`/`sync_builtins` all PASS with genuine concurrency, under both the mcc-built (`.int`) and host-cc-built (mnemonic) `atomic.o`. qemu-user runs on x86-TSO and hides arm64 weak-memory bugs, so the likeliest atomic root cause is a **missing acquire/release barrier (`dmb`/`ldar`/`stlr`) in mcc's arm64 codegen**. Bounds cannot even be built for cross (`if(native)` gate, CMakeLists ~2204; `-b`/`-run` is native-JIT-only). **Ruled out:** the runtime is not mcc-miscompiled — on native, `MCC_EMBED_MCCRT=ON` → `_use_gcc=ON` (CMakeLists ~2266), so `atomic.o`/`bcheck.o`/`complex.o` are host-gcc/clang built. To fix: an arm64 host, or the `ctest --output-on-failure` assertion diffs from CI (the summary list alone doesn't say whether it is a wrong value, a crash, a BCHECK-message mismatch or a timeout).

### Session-workflow agreements (folded from cross-session memory 2026-07-30)
- **Push to `main` is classifier-gated.** The auto-mode classifier intermittently denies `git push origin main` (cites bypassing PR review). Always commit locally first; if a push is denied, do NOT work around it — surface a short note that the commit is local and ready. A fast-forward push after a clean `git pull --rebase` sometimes goes through where a diverged push was denied.
- **Multiple concurrent sessions push to `main`.** A plain `git pull` can report "Already up to date" while local is ahead, and the remote then advances mid-session from another loop. Before starting a big item, `git fetch` and check `git log origin/main` (grep the remote for the feature's key symbols) so two sessions don't independently build the same thing; before pushing, `git pull --rebase origin main`, and DROP a duplicate rather than force a conflicting merge (a whole loop was once wasted re-implementing an already-landed register extension).
- **The Windows JIT/AOT-parity effort is Windows-scoped.** Guard new code behind `_WIN32`/`MCC_HOST_WIN32` or the host-abstraction layer, keep POSIX byte-identical, and when triaging CI act only on Windows job failures unless the user asks otherwise — the POSIX paths already work and shouldn't be disturbed.

### Windows/PE CRT + i386 toolchain facts (folded from cross-session memory 2026-07-30)
- **i386-win32 toolchain (local, WoW64):** winlibs `i686-w64-mingw32` GCC (ucrt, real libgcc: `___chkstk_ms`/`___udivdi3`) at `C:/Users/llg/opt/mingw32`, side-by-side with the x86_64 scoop mingw. The scoop llvm-mingw `i686-w64-mingw32-gcc` is compiler-rt only (no libgcc) — use the winlibs one for i386 links. Cross build: `cmake --preset cross` then `ninja -C cmake-cross mcc-i386-win32 i386-win32-libmccrt.a`; `mcc-i386-win32.exe` is a host x86_64 binary emitting i386-win32 PE that runs natively on WoW64. Repro/soak: `tools/i386win32-soak.sh` (set `MCC_I386_MINGW` if not the default path) — proves the 3-stage AOT self-host fixpoint and runs the `jit_selftest_*` stub tail under `MCC_JIT_I386_STUBS=1`.
- **32-bit msvcrt.dll does NOT export the single-precision float math family** (`sqrtf`/`floorf`/`ceilf`/`expf`/`logf`/`log10f`/`powf`/`sinf`/`cosf`/`tanf`/`_hypotf`); the 64-bit msvcrt.dll does. `runtime/win32/lib/msvcrt.def` is shared across win32 arches, so binding one of those to a msvcrt import on i386 yields a PE that fails to LOAD with `STATUS_ENTRYPOINT_NOT_FOUND` (0xC0000139) before `main`. mcc's `libmingwex` peer is the i386-only shim `runtime/win32/lib/mccmath.c` — thin wrappers over the double msvcrt entries (present on 32-bit), pulled from `libmccrt.a` on demand so a real `-lm` still wins. **Do NOT add these float symbols to the shared def; keep the i386 shim.** UCRT is irrelevant to mcc's own output — mcc always targets msvcrt regardless of the CI winlibs-UCRT *reference* compiler.
- **The Windows exec runner has a real watchdog.** `tests/exec/runner.c` `run_capture` runs the child via cmd.exe inside a Job Object, polls the output pipe against `MCC_TEST_TIMEOUT`, and `TerminateJobObject`s the whole tree on deadline; `main()` sets `SEM_FAILCRITICALERRORS|SEM_NOGPFAULTERRORBOX` so a crashing/unloadable child returns its status promptly instead of hanging on a modal dialog. Without both, a wedged or unloadable child stalls the suite until ctest `--timeout` (the POSIX path already killpg's). Keep them.
