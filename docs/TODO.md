...
- Fix i386 `-fPIC` TLS codegen (miscompile, found via linux/386 Docker as the 32-bit sysroot): mcc-i386 has no GD/LDM codegen. Non-PIC is fine (emits `R_386_TLS_LE`, runs correctly). Under `-fPIC` it wrongly emits `R_386_GOT32X`/`R_386_PC32` for a global `__thread` (treating the TLS symbol as a regular GOT global) and `R_386_TLS_LE` for a static `__thread` — the linked PIE/exe **segfaults**. gcc-i386 `-fPIC` defaults to GD (`R_386_TLS_GD`+`___tls_get_addr`) for globals / LDM (`R_386_TLS_LDM`+`LDO_32`) for statics. Non-TLS `-fPIC` is unaffected. Repro: `mcc-i386 -fPIC -c` a `__thread` TU, link+run in `linux/386` docker. Fix = emit real i386 GD/LDM (or at least IE, or hard-error) under PIC so it stops producing crashing code. (mcc's *linker* already relaxes external GD/LDM objects — that path is the existing `tests/tls/run_models.cmake`, x86_64 only.)
