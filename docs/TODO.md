...
- **`selfhost-fixpoint-O3` — mcc built at `-O3` is not the same compiler as mcc built by clang.** Every macOS stage2 cell reds on it; `-O1`, `-O2`, `-Os`, `-gates` and all three `memmodel` levels pass, and it passed at the commit before the P5 default flip. Same object size, **three bytes**, one instruction, and it is semantically a no-op: for `p[3] = (unsigned char)v;` the clang-built mcc emits `ldrb w1, [x29, #0xa8]` and the mcc-built one emits `ldr w1, [x29, #0xa8]` — the following `strb` discards the upper bits either way. So the gate is right to fail and the damage is zero; what it has caught is that **mcc's `-O3` build of itself changes a codegen decision**, which is the fixpoint gate's whole job.
  Reduced to five lines, `be32.c`:
  ```c
  void be32(unsigned char *p, unsigned v) {
      p[0] = (unsigned char)(v >> 24), p[1] = (unsigned char)(v >> 16);
      p[2] = (unsigned char)(v >> 8),  p[3] = (unsigned char)v;
  }
  ```
  Compile that at `-O3` with mcc built at `-O0`, `-O1` or `-O2` and you get `ldrb`, the same as the clang-built mcc; built at `-O3`, `ldr`. **`MCC_AST_NARROW=0` makes every one of them agree** (on `ldrb`), so the decision that moves is the narrow pass's, and the question is which construct in mcc's own source mcc compiles differently at `-O3`. It is not the `-O3` gates: `MCC_AST_CYCLE=0`, `MCC_AST_INLINE=0`, both together, and even `MCC_AST_TEMPLATES=0` — which turns the whole AST layer off — all still diverge when the *build* is `-O3`. Do not start from the gate names; start from the two mcc binaries and diff what the narrow pass reads on this body.
