...
- Extend the native-versus-cross cross-check to riscv64 and i386. arm64 has it: measured natively in a `linux/arm64` container, `c2ok` identical to the cross reading, so C2 is cross/native-invariant there. Neither of the other two can run natively on this host.
