...
- **Already implemented and default-on, verified 2026-07-27 on this Windows host (mingw `cmake-build-debug` + MSVC `cmake-msvc`) — do not re-plan any of it:** the COFF `.o`/`.a` reader (`coff_load_object_file`), the Win64 `mccjit_make_kgc_stub_mixed` positional-ABI stub (`jit/selftest-mixed` runs unskipped on x86_64-win32), and the Win32 `MCC_EMBED_JIT` runtime (`mccjit_win32.h` shims `mmap`/`VirtualProtect`/`FlushInstructionCache`/SRWLock/CondVar/threads; `host_runmem_*` have WIN32 paths).
...
`-O4` out-of-process superopt driver is the best cell on ZERO of five kernels. Open and newly found: `-O1` executes the same instruction count as `-O0` on all five kernels while emitting more `.text` (that is the `atoi` copy — it has its own section below). Recorded as a negative result: `__builtin_sqrt`/`floor`/`fma`/`fmin` "do not link" is NOT an mcc defect — gcc fails the identical program the identical way without `-lm`, and the `__builtin_*` spellings inline IDENTICALLY to the plain names wherever the transform pipeline re-emits (a trivial single-call function keeps the libcall for both spellings because nothing triggers the optimized re-emit, which is a pipeline characteristic, not a builtin gap).
