...
- Implement i386 `-fPIC` TLS codegen (GD/LDM). Stopgap landed: `gen_gotpcrel` (i386-gen.c) now hard-errors on a TLS symbol instead of emitting the old `R_386_GOT32X` that made the linked PIE **segfault** — so no more silent miscompile (found via linux/386 Docker as the 32-bit sysroot; non-PIC `R_386_TLS_LE` path is fine and runs correctly). Remaining: emit real global-dynamic (`R_386_TLS_GD` + `___tls_get_addr`) for globals and local-dynamic (`R_386_TLS_LDM` + `R_386_TLS_LDO_32`) for statics under `-fPIC`, matching gcc-i386 (or at least initial-exec `R_386_TLS_GOTIE`), then drop the error. mcc's *linker* already relaxes external GD/LDM objects (`tests/tls/run_models.cmake`, x86_64 only); the gap is i386 PIC TLS *codegen*.
