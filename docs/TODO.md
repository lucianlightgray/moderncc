...
1. **Mach-O relocatable `.o` READER — plain objects LANDED 2026-07-27; ARCHIVES LANDED 2026-07-28 (on Linux, via `llvm-ar`); RELOCATIONS are the remaining blocker and are reserved for macOS.** `macho_object_type` + `macho_load_object_file` (`mccmacho.c`, modelled on `coff_load_object_file`) parse `MH_OBJECT`: `LC_SEGMENT_64` sections are mapped to mcc's ELF-internal names (`__TEXT,__text` → `.text`, `__data` → `.data`, `__bss` → `.bss`, `__const`/`__cstring` → `.rodata`, else `.<seg><sect>`) and merged like the COFF path; `LC_SYMTAB`/`nlist_64` symbols are registered via `set_elf_sym` with `N_UNDF`→`SHN_UNDEF`, `N_ABS`→`SHN_ABS`, `N_SECT`→ the merged section at the right offset. Hooked into the `default:` case of the MACHO branch of the `libmcc.c` loader switch, replacing `unrecognized file type`.
   ...
   **NOT done, and to be finished ON macOS:**
   ...
   - **Relocations — x86_64 DONE 2026-07-28, ON LINUX; arm64 still open.** Also wrongly reserved for macOS: `clang -target x86_64-apple-macos11 -c` runs on Linux and emits genuine Mach-O objects with genuine `X86_64_RELOC_*` entries, which is all the loader consumes.
     ...
     **`GOT` + `SUBTRACTOR` — DONE 2026-07-29, ON LINUX, both arches.** Also not macOS work: `clang -target {x86_64,arm64}-apple-macos11` emits `GOT_LD` / `GOTLDP`+`GOTLDPOFF` for an `extern` reference and `SUB`+`UNSIGND` pairs from a two-section `.s`, which is all the loader consumes. `GOT_LOAD`/`GOT` → `R_X86_64_GOTPCREL`, `GOT_LOAD_PAGE21`/`GOT_LOAD_PAGEOFF12` → `R_AARCH64_ADR_GOT_PAGE`/`R_AARCH64_LD64_GOT_LO12_NC` — the same ELF relocations mcc's own Mach-O WRITER already allocates GOT slots for in `check_relocs`, so the linker half needed nothing.

     **`SUBTRACTOR` is not the relocation the plan assumed.** The plan was to fold same-section pairs into the field and emit nothing. That case does not exist: the assembler folds a section-internal difference itself, and no relocation is emitted at all — the fold path would have been dead code. A real `SUBTRACTOR` always spans sections, and ELF has no sym-minus-sym relocation to map it to. What IS expressible: when the SUBTRACTOR's **subtrahend** `A` sits in the same section as the relocated field, `B - A = (B - P) + (P - A)` and `P - A` is a link-time constant, so the pair becomes one pc-relative relocation on `B` with that constant as its addend — `R_X86_64_PC32`/`PC64`, `R_AARCH64_PREL32`/`PREL64`. Anything else is a hard error. `R_AARCH64_PREL64` did not exist in `arm64-link.c` (only the `elf.h` constant) and was added: `code_reloc` → 0, `gotplt_entry_type` → `NO_GOTPLT_ENTRY`, applier `add64le(ptr, val - addr)`, mirroring `PREL32`.

     **`TLV` stays a refusal** — Mach-O thread-locals are descriptor-based and have no sound ELF-relocation image; guessing one yields a binary that links and reads the wrong address (`cli/macho_extern_tls_unsupported` records the gap). **`POINTER_TO_GOT` stays a refusal** — no `R_AARCH64_GOT_LD_PREL19`/`LD64_GOTPAGE_LO15` in the applier to map it to. **Scattered stays a refusal and is NOT a gap**: scattered relocations do not exist in the x86_64 or arm64 Mach-O ABIs, so the line above overstated it — refusing is the whole correct implementation.

     Guarded by `macho-got-sub-x86_64` / `macho-got-sub-arm64`. Both relocations were hard errors before, so "the link succeeded" proves nothing and every assertion is arithmetic: the GOT slot address is computed from the load instruction's OWN encoding (scoped to `_get`'s body, so it cannot match the right instruction shape in the wrong function), the slot must lie inside `__got`, and the eight bytes there must equal `llvm-nm`'s address for the target; the `SUBTRACTOR` fields must hold exactly `nm(_f) - nm(_d)` at 4 and 8 bytes, at two different offsets from `_d` so the two addends differ and a double-applied addend cannot pass; and the cross-section case must be REFUSED BY NAME, not merely fail. One parsing trap worth keeping: `llvm-objdump -s` prints the section twice, and the second dump ("Contents of (SEG,SECT) section") is numeric 32-bit words, not bytes — parsing both silently reverses every group of four bytes and a test that does so reports plausible-looking wrong values.

     Remaining: validation against a real `libmcc_jitengine.a` — that one genuinely wants macOS, because building the JIT engine for an apple triple needs the macOS SDK headers.
