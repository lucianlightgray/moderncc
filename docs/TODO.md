...
- i386: UBSan trap, ASan native-shadow, stack-protector, TLS GD/LDM, 64-bit div-magic helper, JIT stub tail, Tier-4 replay-inline.
- arm (armv7): raise from Tier-2 — self-host, UBSan trap, ASan native-shadow, stack-protector, JIT stub tail, replay-inline.
...
- UBSan `-recover` mode — **x86_64 / arm64 / riscv64 landed (gated OFF); x86_64 ELF+PE validated, arm64/riscv64 ELF validated under qemu**. New `do_sanitize_recover` gate (mcc.h) is set by `-fsanitize-recover=undefined|all` / `-fno-sanitize-trap=undefined|all` and cleared by `-fno-sanitize-recover=…` / `-fsanitize-trap=…` (libmcc.c ~2533, last-flag-wins). When ON, the per-arch emitter (`gen_ubsan_trap_or_call` x86_64-gen.c / `arm64_ubsan_emit_recover` arm64-gen.c / `riscv64_ubsan_emit_recover` riscv64-gen.c) swaps the trap (`ud2`/`brk`/`ebreak`) for a `call`/`bl` to `__ubsan_handle_<kind>_minimal` (kinds add_overflow / sub_overflow / mul_overflow / divrem_overflow / shift_out_of_bounds / type_mismatch_v1; arm64/riscv64 save the caller-saved GP set + link reg around the call and give sub/mul precise symbols); the minimal-ABI handlers (runtime/lib/mccubsan.c, auto-linked via mcc_add_support in mccpe.c + mccelf.c for all three arches) log to stderr, RETURN, and now register an `atexit` summary printing the recovered-event count — with opt-in `MCC_UBSAN_EXITCODE=N` (or `UBSAN_OPTIONS=exitcode=N`) forcing exit status N on any recovered UB. Default build is byte-identical (trap-mode object cmp'd clean-vs-patched on all three arches). Tests: `ubsan/recover_signed_add`; arm64/riscv64 recover programs survive UB + print diagnostics under qemu. Remaining: (1) i386/arm32 emit the call (else a warning keeps trap); (2) per-check recover sets (`-fsanitize-recover=shift` only) — today it's all-or-nothing per TU; (3) upgrade minimal→full ABI (emit `struct {SourceLocation,TypeDescriptor}` tables + operand values so the diagnostic names file:line and values) — large, needs `.rodata` descriptor emission; (4) x86_64 sub/mul still reuse the add_overflow symbol (cc-only dispatch can't tell +/-/* apart at the x86_64 check site — arm64/riscv64 already precise) — thread the op through; (5) cross runtime packaging: the cross `libmccrt.a` does not emit a per-arch `mccubsan.o` and `MCC_MCCRT_EXTRA_O` forces one colliding path (same gap as `mccasan.o`), so out-of-the-box cross `-fsanitize-recover` needs an `<arch>-mccubsan.o` staged for the driver to auto-discover — teach `mcc_build_mccrt` to emit/install it.
