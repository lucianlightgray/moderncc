...
x86_64-linux where all three pass. **All four rows below are now closed** — the fourth,
`superopt/global-reload`, closed on 2026-08-12, and the last two closed 2026-08-13. **What
replaced them is `ci/must-run-registered`** (N38), which was red here and in no table at all:
...
| ~~`rir/drop-ratchet`~~ **CLOSED 2026-08-13** | was: `regaddi` banked as dropping but absent from the report, which the cell said was either a measurement that did not run or a reason that stopped dropping. **It was the first, and the cell was right to refuse to guess.** Its `-I` list named `src/arch/x86_64` and `src/arch/i386` and never the target's own, so on arm64 `src/mcc.h`'s `#include "arm64-gen.h"` did not resolve and the compile failed — **and the cell checked only that the report file existed, not that mcc exited 0**, which it does not, because `MCC_RIR_PROD_OUT` is written either way. An empty histogram from a failed compile and an empty histogram from a clean one are the same bytes. Now takes `CPU`, checks the exit status, and runs **two arms**: a default build for the ratchet and a `-freg-disp` build for liveness. Green here: `0 opcode kind(s) dropped in a default build, 1 under -freg-disp`. It was missing from this table because every earlier sweep here filtered on `^rir-` (hyphen), which does not match `rir/drop-ratchet` (slash) — a reminder that a regex-selected sweep is only as complete as its regex |
...
| ~~**11**~~ **THE ZERO-DERIVATION HALF CLOSED 2026-08-13** | **`selfhost-optbench.py --check` can pass over zero derivations.** **Two vacuity paths, both now fatal.** `if args.check and levels:` meant an *empty* derivation skipped the comparison entirely — no PASS line, no FAIL, exit 0 — and is now a failure that says so. And because a flag with no measurable evidence is assigned `levels[f] = levels_now[f]` *by design*, comparing it against `src/mccopt.h` compares a value with itself; `--check` now requires at least one flag to have been derived from evidence rather than carried, and the PASS line reports **how many of how many** were. Verified by driving `derive_levels` with an all-`inert` row set: every `why` comes back `"unchanged: inert, nothing measured to move it on"`, the derived list is empty and the floor fires. **The stale-48 and `len(names)` halves are also DONE 2026-08-13, so what is left of this row is one item.** The docstring said **48** in five places while `flag_table()` yielded 16 — and 13 by the end of the day, once the ladder work moved three rows. **The number is now derived rather than written down anywhere**, because a constant in prose that no code reads is a claim that cannot fail, which is the same defect this tool exists to find in the ladder. And `flag_table()` parses `src/mccopt.h` with a regex: if that regex ever stops matching — a reformatted `MCC_OPT_ROW`, a renamed macro — it returns an empty table, every loop iterates zero times, and `--check` agrees with the ladder having compared nothing. `MIN_LADDER_FLAGS = 5` is a floor deliberately far below the real count, so only a parser failure or a wholesale collapse trips it; verified by feeding `flag_table()` a header it cannot match (0 < 5, fires) against the real one (13, clears). **And the last of it, the empty sample list classifying `inert`, is done too — so row 11 closes.** Every movement input was read with a `.get(k, 0.0)` default, so a flag no sample ever reached scored exactly like one that is genuinely inert, and `inert` then flowed into `derive_levels` as *"keep the level you have"*. There is now an `unmeasured` bucket, and `derive_levels` gives it its own reason (*"no sample reached this flag"*) rather than the `unchanged:` one. Both still keep the current level — there is no evidence to move on either way — but only one of them is a *result*, and the derived-evidence floor added earlier counts neither. Verified by classifying two synthetic flags, one with no inputs and one with measured zeros: `unmeasured` and `inert` respectively. **This is the same "absent reads as zero" thesis the cross-cutting research established, found in the tool that ranks the ladder.**  Original text follows. `derive_levels` assigns `levels[f] = levels_now[f]`, so an all-`inert` run prints *"src/mccopt.h matches the ladder"* having derived nothing; the docstring says **48** level-assignable flags in five places and `flag_table()` yields **16**; no floor on `len(names)`; an empty sample list classifies `inert`. The board already carries "`selfhost-optbench --check` was not re-run" as a caveat, which is the same hole one level up | medium (several floors) | **census trust** |
...
| **22** | **`rir-coverage.py`'s `wide` denominator is "the files that happened to compile"** — an `os.walk` of seven directories with no manifest, so a source dropping out silently shrinks the ratchet. Cheap first step already named: bank `sources=N` and fail when it moves. **THE COMPILE-FAILURE HALF IS DONE 2026-08-13.** `sources_{self,exec,wide}` was already banked and already fires on a sha move; what was printed and never compared was the *compile-failure count*, so a source dropping out shrank the denominator in silence. `failed` is now banked per level (**9** at `-O0`–`-O3`) and a rise is fatal, naming the offenders. Proved it bites: perturbed to 8 it reports *"9 source(s) now fail to compile against 8 banked, so every percentage above is over a denominator that just shrank"*. Enforced comparisons 5 → 6 per level. **The 9 are now named rather than counted** — `arch/arm64_encoding.c`, `arch/arm64_extasm.c`, `arch/winarm64_interlocked.c` (arch-gated), `pointers_arrays/array_assignment.c` (whole-array assignment, an intentional refusal mcc shares with gcc and clang), plus `features_c99_c11/builtins_extra.c`, `functions_abi/fastcall.c`, `functions_abi/gnu89_extern_inline_redef.c`, `lexical/trigraphs.c`, `types/std_gated_keywords.c`. None is silent breakage; the point is that nothing would have said so if a tenth appeared. **The `LOW_EXCLUDE` half is also DONE 2026-08-13, and the interesting part is which failure it covers.** Changing the `LOW_EXCLUDE` *string* was already caught — `corpus_config` compares it against the bank and skips at 77 with a clear message. What was NOT caught is the case the row actually names: the string stays and **the file is renamed**, so the config still matches the bank, the suffix match quietly stops matching, and the ratchet compares a full-corpus number against an excluded-corpus floor. `check_low_exclude` now verifies every entry still names a file that exists. Verified against that exact scenario rather than the one the config check already handles: with `src/mccgpu.c` moved away it reports *"LOW_EXCLUDE names 1 path(s) that no longer exist … these now exclude NOTHING"* | small (the floor) | **census trust** |
...
| ~~**24**~~ **CLOSED 2026-08-13 (the survivor-count half)** | **`stratsweep.sh` and `flagsweep.sh` drop subjects silently.** `$WORK/skipped` is written and never counted; the only floor is `n > 0`, so a miscompile breaking 30 of 31 subjects prints `PASS stratsweep-iso all: 22 strategy/ies x 1 subjects`. Both already print the survivor count — pin it. **`stratsweep.sh` is done**: `admit()` now records the collected count beside the admitted one (through `$WORK/collected`, because `admit` runs inside a command substitution and a variable cannot leave one), and `yield_floor` fails both the `iso` and `seq` arms when fewer than 80% of the collected corpus survives admission, printing the drop list. `MCC_STRATSWEEP_MIN_YIELD` overrides the fraction for a deliberately narrowed run. **Proved it bites** rather than assuming: forced to a 150% floor it exits 1 with *"only 31 of 31 collected subject(s) survived admission -- below the 150% floor of 46"*. 251 optfire/stratsweep cells still green. **`flagsweep.sh`'s half is DONE 2026-08-13 too, so this row closes.** Its silent drop is a different mechanism from `stratsweep`'s: a subject whose `-O0` reference build fails gets a `.refunbuildable` marker and is then skipped by *every* later `corpus_run` — all three levels and both flag states — while the only floor was `ran > 0`. The subjects are now named and any drop is fatal, which matches what the script already said about the total case (*"the -O0 reference build failed for all of them, which is a red"*); a partial failure is the same red with a smaller denominator. Proved by forcing one subject's reference build to fail in a copy of the script: **`FAIL … the -O0 reference build failed for subject(s): features_c99_c11/c11_threads … the 144 run(s) above are over a corpus that silently shrank`**, rc 1, where the unmodified script prints `PASS … 150 subject runs`. 201 flagsweep cells green | small | **gate strength** |
