...
- [ ] T-win-50020 [S] win-x64 — the Windows embed-JIT link: mcc's in-process linker cannot resolve the MSVC-CRT internals the cl-built engine blob references
      OWNER: win-x64 | STATE: IN_PROGRESS | SHA: 6ca4516f | TS: 2026-08-15T16:15Z
      REF: DETAILS.md#t-win-50009-resolved-the-smoke-harness-runs-on-windows-and-what-it-found | DEPS: — | NOTE: promoted from the T-win-50003 Bucket B note because its deferral reason ("behind lin's T-lin-10001 L2'") is STALE — L2' landed and lin's T-lin-10001 says no task depends on it any more. Every `--embed-jit` compile on Windows dies at link: `__report_rangecheckfailure` (the minimal repro, seen alone in the engines arms), plus `__security_cookie`/`__GSHandlerCheck`/`__isa_available` and the `__imp_*` ucrt import set in the full-subject arm. This ONE failure holds 13 of the 15 win suite reds (5 smoke incl. every jit engine arm, 4 fp opt-search cells cannot be re-verified without it, jit/replay-parity, mcctest-embedjit, runtime-bench-check, slice/cost's jit arm), T-lin-10030/win, and T-lin-10383's win half (--jit-always-gpu needs the jit route). Fix directions to weigh: synthesize the /GS symbols in the jit linker (cookie as data + no-op checks — but __GSHandlerCheck is an EH personality, weigh unwind correctness), resolve __imp_* against the loaded ucrt DLLs, or build the engine blob without /GS (cl /GS- on libmcc_jitengine — smallest surface, check what else /GS- moves). win-x64 has the repro; mccjit_embed.c expertise is lin's
