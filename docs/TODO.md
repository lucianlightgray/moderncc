...
- [ ] T-win-50015 [S] win-x64 — default `ms_bitfields = 1` on PE targets: mcc's plain bit-field layout is cross-TU-incompatible with every native Windows compiler
      OWNER: win-x64 | STATE: IN_PROGRESS | SHA: 901e103e | TS: 2026-08-15T19:35Z
      REF: DETAILS.md#t-win-50015-slice-1-the-fixture-exists-and-the-flip-found-two-algorithm-gaps | DEPS: — | NOTE: CLAIMED + deduped (was two Open rows). GAP (a) DONE + landed: mcc's ms-mode over-sized a zero-width-only union (`union{int :0;}`) to sizeof(int)=4 (struct outer 12 vs mingw 8) because struct_layout reduced a union bitfield member's size to (bits+7)>>3 only under pcc (gcc-mode); ms-mode (pcc=0) kept the full int. FIX (mccgen.c ~6247): `else if (bit_size==0) size=0` in the VT_UNION branch — provably gcc-mode-safe (dead when pcc=1), 471/471 struct/bitfield/exec cells green. New CI cell pe/ms-bitfield-sizing (tests/cross/pe-msbitfield-sizing.{c,sh}) diffs mcc -mms-bitfields sizeofs vs mingw (its Windows default) → outer 8 / just_zero 0 / differently_typed 12 / same_type_run 4, all match. GAP (b) ADJUDICATED — NOT a bug: the divergence is `s.ull31` (an `unsigned long long : 31` bit-field) promoting to unsigned under ms-mode vs signed under gcc-mode. cl.exe (MSVC, confirmed) promotes it to UNSIGNED — so mcc's ms-mode is CORRECT (matches MSVC); mingw/gcc give signed via the width-based GCC rule (the standard's bit-field-promotion rule only covers int/unsigned int, so extended-type bit-fields are impl-defined and MSVC≠GCC here). Same class as the banked f80 long-double rows. So the pe/x-oracle +1 divergence is EXPECTED MSVC-conformance → BANK it when the flip lands, do NOT change mcc. FLIP VERIFIED on the win side but it is a CROSS-PLATFORM COORDINATED landing, NOT unilateral from win-x64. Applied+tested the two edits, then REVERTED (uncommitted): (1) libmcc.c mcc_new, after the MACHO leading_underscore block, `#if defined MCC_TARGET_PE\n s->ms_bitfields = 1;\n#endif`; (2) smokerun.c msstruct pin → `#if MCC_HOST_WIN32` "msstruct 20 2 8 3 8 4 8 12 5\n" else the old ...4 5. With the flip a plain `{char;int:3;char}` sizes 12 (MSVC, was 4) and pass-msstruct prints the MSVC line — confirmed. BLOCKER for landing: the flip is target-keyed (MCC_TARGET_PE), so it moves the win32 keys of ast/o0-baseline (tools/o0_ab.sh:138 measures x86_64-win32/i386-win32/arm64-win32/arm-win32), and that cell runs ONLY on UNIX-non-WIN32 (CMakeLists:4275) — i.e. only a LINUX session can re-bank it. Landing must be ONE coordinated change: win-side (these 2 edits + register pe/bitfield-abi + bank the gap-(b) pe/x-oracle row as MSVC-conformance, all verified/ready here) PLUS the Linux o0-baseline win32-key re-bank — else Linux CI reds on the win32 columns. NEXT: a Linux session (or lin-x64 via CONTRACT) lands the flip + re-banks o0-baseline; win-x64 confirms pass-msstruct/pe-torture/pe-xoracle/pe-bitfield-abi green. Greens pass-msstruct + the msstruct half of smoke/native+strats once landed
...
## Open — claimable
- [ ] T-lin-10387 [P] Re-bank the per-platform lowerable floors for b1f912b5's benign source addition (rir-coverage self/wide nodes_pct)
      OWNER: — | STATE: OPEN | SHA: 496271d4 | TS: 2026-08-15T19:45Z
      REF: DETAILS.md#t-lin-10384-macho-red-resolved-source-dilution-not-r-arity-the-census-never-calls-the-ladder | DEPS: — | NOTE: T-lin-10384's +25 lines in ast_eval_ladder_scan (amalgamated into src/mcc.c) grow one banked body, shifting the self-census nodes_pct; PROVEN source-dilution not R_ARITY (ast_eval_ladder_scan called 0x during the census compile; probe evidence at REF). elf/lin ALREADY GREEN (§8 passed). /mac: rir-coverage RED on macho (nodes_pct 42.7748 < 42.8407) — re-bank the macho lowerable floors under the source-dilution justification (rir-coverage.py --update-bank-low on the Darwin box), then rir-coverage green + close. /win: check the PE arm's lowerable floor and re-bank if it moved (may be N/A if win has no PE lowerable census). Verify per child: rir-coverage green on that platform. Parent closes when /mac + /win done (/lin already satisfied)
  - [x] T-lin-10387/lin [P] elf lowerable floors — lin-x64
        OWNER: lin-x64 | STATE: DONE | SHA: 496271d4 | TS: 2026-08-15T19:45Z
        REF: DETAILS.md#t-lin-10384-macho-red-resolved-source-dilution-not-r-arity-the-census-never-calls-the-ladder | DEPS: — | NOTE: no rebank needed — elf stayed inside its floor; rir-coverage green in cmake-debug at the T-lin-10384 §8 (0/10070). Child DONE by construction
  - [x] T-lin-10387/mac [P] macho lowerable floors — mac-arm64
        OWNER: mac-arm64 | STATE: DONE | SHA: 1da24f27 | TS: 2026-08-15T19:58Z
        REF: DETAILS.md#t-lin-10387-mac-done-macho-lowerable-rebank-verified-benign | DEPS: — | NOTE: DONE at 1da24f27. Independently verified benign before lowering the floor: 0 pre-existing bodies changed their ok-bitmask (tu-independent diff); 3060-row churn is pure tu-field normalization; 28 excluded new bodies are corpus mix (msl_fmt_*/mcc_sched_*). coverage-bank.json macho nodes_pct 42.8407->42.7353 O0-O3, ZERO elf/pe lines in the diff (lin's /lin + win's /win untouched). rir-coverage PASS + rir-coverage-census PASS. Clears the one non-environmental red in T-lin-10385's §8. Parent stays open until /win
...
- [ ] T-lin-10364 [S] The wide census carried a pre-existing drift component that a0e26cff has now banked
      OWNER: — | STATE: OPEN | SHA: a0e26cff | TS: 2026-08-15T19:33Z
      REF: DETAILS.md#t-lin-10364-investigation-the-window-is-227-commits-classifier-unchanged-inventory-shortcut-unavailable | DEPS: — | NOTE: SCOPED (lin-x64, not claimed): real window is eee6c1f2..a0e26cff (227 commits, NOT the 2-commit bb5c0488^ reading); the census classifier (ast_low_node/walk) is UNCHANGED in it, so the drift is source-content reclassification of existing bodies; the body-inventory shortcut is DEAD (wide/elf inventory = 0 rows at eee6c1f2, first banked at the a0e26cff re-bank). Only path left = multi-build bisect (~8 build+census steps): build mcc, `MCC_RIR_CENSUS=1 rir-coverage.py <build> --corpus wide --levels O0 --opt-in` vs eee6c1f2's banked bodies_pct, mark on the pre-existing component
