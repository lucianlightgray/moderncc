...
| lin-x64   | Linux    | x64   | 10000–29999 | 10383   | 2026-08-15T15:15Z |
...
- [ ] T-lin-10359 [X] lin-x64 — `slice/cref-oracle-*` stalls on five programs when the host GPU is busy
      ...
      REF: DETAILS.md#t-lin-10359-slicecref-oracle-stalls-on-five-programs | DEPS: — | NOTE: RE-READ THE DIAGNOSTIC BEFORE TRUSTING IT. This row is written as "the same mechanism as T-lin-10073, arriving through the GPU instead of through wine", and T-lin-10073 RESOLVED 2026-08-15 by showing its own discriminator was wrong: a RESIDENT foreign wineserver is not the variable (6 passing cell-observations with one resident, across three full suites, one at loadavg 26.6) — an ACTIVE foreign client is. If this row inherited that shape, "the host GPU is busy" needs the same test: a resident compositor or an idle GPU process is not evidence. See DETAILS.md#t-lin-10073-resolved-residency-is-not-the-variable-an-active-client-is
