...
- Broaden the dg-error diagnostic tier toward gcc C99/C11. The clean pedantic-gated gaps are done (strict-c89 asm/typeof, GNU stmt-expr/case-range/label-as-value/omitted-`?:`, `$`-in-ident, `_Bool`/`_Complex`/`_Generic`/`_Alignas`/`_Alignof`/`_Noreturn`, variadic-macro, over-length string, msg-less `_Static_assert`, dup-qualifier, tentative-array, extra-tokens-after-directive). Remaining gaps found (gcc AND clang both diagnose, mcc silent) are each non-trivial: (a) ordered comparison of pointer with integer **zero** (`p > 0`) — mcc's combine-types collapses `<`/`>`/`==` to one `CMP_OP` and treats `0` as a null-ptr constant (mccgen.c ~2944), so the relational-vs-equality distinction is lost before the check; needs the specific op threaded through. (b) `-Wshift-count-negative`/`-overflow` on constant-folded shifts (`1<<-1`, `1<<40`) — independent of the runtime `<<` value-count trap in the gated §29/VLAT work, but a default-on warning; add at the shift const-fold site. (c) `-Wreturn-local-addr` (`return &local` / `return local_array`) — real bug detector, but default-on and needs false-positive-free auto-vs-static/global/param detection on vtop; corpus risk (e.g. tests/exec/expressions/compound_assignment.c returns `&base[idx]`). (d) `-Wundefined-internal` (static declared+used, never defined). (b)-(d) are default-on in gcc/clang, so landing them means auditing/regenerating the exec + errors_and_warnings goldens, not just a pedantic add.
