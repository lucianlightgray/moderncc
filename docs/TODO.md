...
1. **Mach-O relocatable `.o` READER — DONE on Linux, 2026-07-27..29.** Plain objects, `llvm-ar` ARCHIVES (BSD
   `#1/` extended names + the little-endian `__.SYMDEF` index, a la carte and selective), and RELOCATIONS for both
   arches: x86_64 `UNSIGNED`/`SIGNED`/`SIGNED_1/2/4`/`BRANCH`/`GOT`/`GOT_LOAD`, arm64 `BRANCH26`/`PAGE21`/
   `PAGEOFF12`/`ADDEND`/`GOT_LOAD_PAGE21`/`GOT_LOAD_PAGEOFF12`, plus `SUBTRACTOR` pairs on both. Anything else is a
   HARD ERROR by name rather than a skip. Guarded by `macho-structural`, `macho-archive`, `macho-reloc`,
   `macho-reloc-arm64`, `macho-archive-reloc`, `macho-got-sub-x86_64`, `macho-got-sub-arm64` — all run on Linux,
   all assert ARITHMETIC targets against `llvm-nm` rather than link success. Detail in git history.

   **Four traps worth keeping, because each one produced a passing test over broken output:**
   - **`reloff`/`nreloc` read and ignored** linked SUCCESSFULLY and left `e8 00 00 00 00` — a call to the next
     instruction. Nothing in the suite failed; the binary was simply broken. Assert resolved targets, not linkage.
   - **mcc's appliers ADD into the field** (`add32le`/`add64le`, REL semantics inherited from tcc), so passing the
     field's existing value as a RELA addend counts it TWICE. The addend must be 0 plus only the pcrel bias. A
     relocation test whose addend is ZERO cannot tell "applied once" from "applied twice".
   - **`llvm-objdump -s` prints each section twice**, and the second dump is numeric 32-bit words, not bytes.
     Parsing both silently reverses every group of four.
   - **clang folds small addends into the instruction** (`msg[5]` gets no `ARM64_RELOC_ADDEND`), so a LO12 handler
     that OVERWRITES the immediate silently reads `msg[0]`.

   **What genuinely still needs macOS** — and only this: validation against a real `libmcc_jitengine.a` and a
   real-world `clang -c` corpus, because building the JIT engine for an apple triple needs the macOS SDK headers.
   That is also the only place `--embed-jit` can be exercised for the osx triples. `TLV` and `POINTER_TO_GOT` stay
   deliberate refusals (no sound ELF image); scattered relocations do not exist in these two ABIs, so refusing them
   is the whole correct implementation, not a gap.
...
`int f(int x){return (x&0x7fffffff)%251;}` returns the correct 91 *in isolation* at every -O
level. Only the **in-context `%251`** is wrong, because it is strength-reduced by the signed
`ast_divmagic_try_signed` fold. That fold keeps the masked dividend `x` live across a 64-bit
magic-multiply and re-reads it (for the `+x` correction and the final `x - q*251`). On i386
it materializes `x` once into a reserved ltemp slot (`ast_divmagic_materialize`) so the reads
are stable — but that reservation is only honored by the **register-promotion** pass
(`ast_ltemp_off` poison, mccast.c:5353). The i386 **value-stack spill allocator does NOT
exclude ltemp slots**, so under `main`'s higher local pressure it reuses `xoff` (`-0x30`) for
the `mul`'s low-product spill, **clobbering the still-live dividend**. `f()` (low pressure)
gets a distinct spill slot (`-0x10` ≠ `-0x8`) and is correct; `main` collides → 131. This is
why "any large gate subset off fixes it" (changes the spill layout) and it's not ASan-catchable
...
-Isrc/formats -Isrc/objfmt -Isrc/arch/i386 -O0 -o mcc-i386 src/mcc.c`); compile with
`-I<b>/include -B<b>/include`. **The cmake i386 preset on an arm64 host emits an ARM64
`libmccrt.a`** (EM:183 — misdetected arch), which link-fails any case that references a
runtime float/int128 helper; build a real i386 runtime instead:
`for c in runtime/lib/*.c; do i686-linux-gnu-gcc -m32 -O2 -ffreestanding -Iruntime/lib
-Iinclude -c "$c"; done; ar rcs i386-librt.a *.o`. Link `i686-linux-gnu-gcc -m32 -static
<obj> i386-librt.a -lm`, run under `qemu-i386-static`; reference `i686-linux-gnu-gcc -m32
-O<n> -static <src> -lm`. With that, `d2 -O2` = 91 and all 14×4 match.
...
**The `.eh_frame` half is the part worth remembering.** The obvious change — pick a per-function section in `decl`
— is only half a feature, and the half it leaves out fails LOUDLY but only outside mcc: `mcc_eh_frame_fde` was
called with a hardcoded `text_section`, so with functions living in `.text.<name>` every FDE described a range in a
section its code was not in. The ranges then overlap, mcc's own linker does not care, and GNU ld rejects the object
outright with `.eh_frame_hdr refers to overlapping FDEs`. Passing the function's actual `cur_text_section` fixes it.
Diagnosis shortcut worth keeping: `-fno-asynchronous-unwind-tables` made the same object link, which isolates it to
`.eh_frame` in one step. That call also fixes a latent pre-existing bug for
`__attribute__((section(...)))` functions, whose FDEs pointed at `.text` too.

STABS needed nothing — `-g` relocations already follow `cur_text_section`, verified by reading `.rela.stab`, which
lists `.text.a`/`.text.b`/`.text.main` per function. Note `-g` is STABS here, not DWARF; a `readelf
--debug-dump=decodedline` check prints nothing either way and proves nothing.

Guarded by `cli/function-sections`, which asserts the PURPOSE rather than the section names — an unreferenced
`static` must survive a normal link and vanish under `--gc-sections`, the program must still print the right answer
through BOTH mcc's linker and the external one, and a control asserts the same static SURVIVES `--gc-sections`
without the flag, so the drop cannot be credited to the linker alone. Default path byte-identical: 271/271 objects
at `-O0` and `-O2`.

**This is the prerequisite for the item below.** mcc has no dead-static elimination, which is what blocks
`MCC_AST_INLINE_STATIC` from being flipped on. `-ffunction-sections` + an external `--gc-sections` now removes
unreferenced statics today (demonstrated on `unused_helper`), and an internal collector becomes a section-drop
rather than byte surgery inside a single `.text`.
...
## `MCC_AST_STOREVAL_CALL` — FLIPPED default-on at `-O2` 2026-07-29
It landed in `62bfb7b7` and had sat at `ast_env_gate(..., 0)` ever since, i.e. parked at the "validate the gated-ON
path" step of this file's own process. Measured before touching the default:
...
**Validated to the full bar**, weighted toward the standing warning on this exact region — `emit-at-marker` was
correct at `-O0`/`-O1` and MISCOMPILED at `-O2`/`-O3` here, caught only by counting evaluations:
`assign_value_effects.c` matches gcc at `-O0`/`-O1`/`-O2`/`-O3`/`-Os`; full ctest 7891/7891; 3-stage self-host
fixpoint byte-identical at the default, `-O3` AND `-O1`; 300-seed gate-swept differential fuzz vs gcc+clang
(298 agree, 0 miscompile, 2 dropped UB); and `-O0` == `-O2` runtime output over the whole exec corpus on arm64
(247), riscv64 (244), armv7-hardfloat (244) and i386 (249).

Ratchet baseline regenerated, 185 -> 183: the two closed gaps are `stmt_assign_call_arg` and
`stmt_assign_call_arg2`, exactly the shapes the gate targets, and nothing else moved. The `x86_64-win32` baseline
needed no edit — it never listed those two, and after the regen the two baselines agree on this file.
