...
- UBSan trap + `-recover` — **all 5 ELF arches landed (gated OFF); x86_64 ELF+PE validated, arm64/riscv64/i386/arm validated under qemu**. i386/arm had NO UBSan checks before, so their support added the underlying trap checks (div0, shift≥width, signed add/sub overflow; i386 also signed mul) plus the recover routing; `-fsanitize=undefined` trap mode now works on i386/arm (incl. PE, pure trap), and `-fsanitize-recover=undefined` works on i386/arm ELF. The `do_sanitize_recover` gate (mcc.h) is set by `-fsanitize-recover=undefined|all` / `-fno-sanitize-trap=undefined|all`, cleared by the inverse (libmcc.c ~2533, last-flag-wins). When ON, the per-arch emitter (`gen_ubsan_trap_or_call` x86_64-gen.c + i386-gen.c / `arm64_ubsan_emit_recover` arm64-gen.c / `riscv64_ubsan_emit_recover` riscv64-gen.c / `arm_ubsan_emit_recover` arm-gen.c) swaps the trap (`ud2`/`brk`/`ebreak`/`udf`) for a `call`/`bl` to `__ubsan_handle_<kind>_minimal` (kinds add/sub/mul/divrem_overflow, shift_out_of_bounds, type_mismatch_v1; all save the caller-saved GP set + link/return reg around the call; all emit precise per-op symbols, add/sub/mul distinct). The minimal-ABI handlers (runtime/lib/mccubsan.c, auto-linked via mcc_add_support in mccpe.c + mccelf.c for all ELF arches) log to stderr, RETURN, and register an `atexit` summary printing the recovered-event count — opt-in `MCC_UBSAN_EXITCODE=N` (or `UBSAN_OPTIONS=exitcode=N`) forces exit status N on any recovered UB. Default build is byte-identical (no `-fsanitize` ⇒ zero UBSan emission on every arch). Tests: `ubsan/recover_signed_add`; per-arch qemu freestanding matrices (real overflow/oob/div0 detected + recovered + continue, zero false positives). Remaining: (1) per-check recover sets (`-fsanitize-recover=shift` only) — today all-or-nothing per TU; (2) upgrade minimal→full ABI (emit `struct {SourceLocation,TypeDescriptor}` tables + operand values so the diagnostic names file:line and values) — large, needs `.rodata` descriptor emission; (3) cross runtime packaging: the cross `libmccrt.a` does not emit a per-arch `mccubsan.o` and `MCC_MCCRT_EXTRA_O` forces one colliding path (same gap as `mccasan.o`), so out-of-the-box cross `-fsanitize-recover` needs an `<arch>-mccubsan.o` staged for the driver to auto-discover — teach `mcc_build_mccrt` to emit/install it; (4) coverage gaps: arm32 signed-mul overflow not checked (needs SMULL + regalloc); i386/arm div INT_MIN/−1 overflow not checked (only div-by-zero); i386/arm-PE recover (ELF-only today).
