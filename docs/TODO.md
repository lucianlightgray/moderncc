...
1. **Mach-O relocatable `.o` READER — plain objects LANDED 2026-07-27; ARCHIVES LANDED 2026-07-28 (on Linux, via `llvm-ar`); RELOCATIONS are the remaining blocker and are reserved for macOS.** `macho_object_type` + `macho_load_object_file` (`mccmacho.c`, modelled on `coff_load_object_file`) parse `MH_OBJECT`: `LC_SEGMENT_64` sections are mapped to mcc's ELF-internal names (`__TEXT,__text` → `.text`, `__data` → `.data`, `__bss` → `.bss`, `__const`/`__cstring` → `.rodata`, else `.<seg><sect>`) and merged like the COFF path; `LC_SYMTAB`/`nlist_64` symbols are registered via `set_elf_sym` with `N_UNDF`→`SHN_UNDEF`, `N_ABS`→`SHN_ABS`, `N_SECT`→ the merged section at the right offset. Hooked into the `default:` case of the MACHO branch of the `libmcc.c` loader switch, replacing `unrecognized file type`.
   ...
   **NOT done, and to be finished ON macOS:**
   ...
   - **Relocations — x86_64 DONE 2026-07-28, ON LINUX; arm64 still open.** Also wrongly reserved for macOS: `clang -target x86_64-apple-macos11 -c` runs on Linux and emits genuine Mach-O objects with genuine `X86_64_RELOC_*` entries, which is all the loader consumes.
     ...
     **IN PROGRESS 2026-07-29 — `GOT` + `SUBTRACTOR`, both arches, on Linux.** Confirmed emittable here: `clang -target x86_64-apple-macos11 -c -O1` on an `extern int` reference gives `GOT_LD`, and the arm64 target gives `GOTLDP`/`GOTLDPOFF`, so neither needs macOS. Planned scope, and what is deliberately NOT in it: `GOT_LOAD`/`GOT` → `R_X86_64_GOTPCREL` and `GOT_LOAD_PAGE21`/`GOT_LOAD_PAGEOFF12`/`POINTER_TO_GOT` → `R_AARCH64_ADR_GOT_PAGE`/`R_AARCH64_LD64_GOT_LO12_NC` (the same ELF relocations mcc's own Mach-O WRITER already emits for GOT slots, so the linker half exists); `SUBTRACTOR` only for the pair whose two symbols land in the SAME section, where `B - A` is a link-time constant that can be folded into the field with no relocation emitted at all, and a hard error otherwise. **`TLV` stays a refusal** — Mach-O thread-locals are descriptor-based and have no sound ELF-relocation image, and guessing one produces a binary that links and reads the wrong address (`cli/macho_extern_tls_unsupported` already records the gap). **Scattered stays a refusal too, and is not a gap**: scattered relocations do not exist in the x86_64 or arm64 Mach-O ABIs at all, so the line above overstates it — refusing is the whole correct implementation.
