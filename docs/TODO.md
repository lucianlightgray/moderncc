...
- macOS CI process-starvation: the runner hits `RLIMIT_NPROC` (fork/posix_spawn `Resource temporarily unavailable`) under `ctest -j3` when the fork-heavy differential cells run concurrently — `fuzz/smoke`+`fuzz/matrix-*` each spawn mcc+gcc-16+clang per program×opt×gate, `parts/*` spawn a 3-way build, and `jit_selftest_fork` builds in parallel. An EAGAIN mid-run manufactured phantom failures: a fuzz program run that can't spawn returns a 125-127 code indistinguishable from the generator's `acc & 0x7f`, and since mcc runs last it was always blamed ("mcc diverges" on seeds that pass in isolation); parts reported a reference build-fail as "diverged". Harness-level mitigation landed (fuzz `mcc_diverges` now confirms a divergence reproduces across retries; parts retries builds/runs on transient spawn markers). Durable fix still open: cap peak concurrency so EAGAIN can't occur — a shared ctest `RESOURCE_LOCK` across `fuzz/*`+`parts/*` (at most one fork-heavy differential at a time) or a lower `-j` on the macOS cell.
