...
- [ ] T-lin-10092 [P] Record a clean full native suite number on each platform
  ...
  - [ ] T-lin-10092/win [P] Record a clean full native suite number on each platform — win-x64
        OWNER: win-x64 | STATE: IN_PROGRESS | SHA: b57019f9 | TS: 2026-08-15T18:18Z | NOTE-3: T-win-50020 DONE (ce66eecc) — the embed-JIT link now works (mingw blob), so the embed-JIT subset of the 13 Bucket-B reds is now PAID (jit/replay-parity was one of them; now green). A FOURTH requote is owed on a full-suite run of the mingw-blob build (rebuild ALL test targets under vcvars64, then ctest): expect below 15, remaining = msstruct (T-win-50015) + slice/fault (T-win-50019) + the T-lin-10359 cref-oracle stall. Not re-run yet (a full win build+suite is ~2h). | NOTE-2: THIRD QUOTE 2026-08-15: 15/9406 with the c-torture corpus LIVE in-suite (35 -> 17 -> 15 today) - DETAILS.md#t-lin-10092-win-requote-c-2026-08-15-15-of-9406-with-the-corpus-live. libtest-extra fixed; 13 steady Bucket-B/embed-JIT+msstruct reds + slice/fault (T-win-50019) + the cref-oracle in-suite stall = T-lin-10359 corroborated on the RTX 2060 (passes standalone). Skips 2032 = llvm-test-suite corpora only
