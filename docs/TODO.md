...
## JIT Windows / i386-PE
- Un-gate the i386-PE KGC/FP(x87)/mixed stub tail (landed gated behind MCC_JIT_I386_STUBS, default OFF). Needs the M8 bar:
...
- i386: ASan native-shadow, stack-protector, 64-bit div-magic helper, JIT stub tail, Tier-4 replay-inline.
- arm (armv7): raise from Tier-2 — self-host, ASan native-shadow, stack-protector, JIT stub tail, replay-inline.
- arm64: mode-6 object-output dispatch, in-place patch row, flip `opt_promote` on. (TLS: `-fPIC`/`-fPIE` now emits TLSDESC — runtime-proven with external ld under qemu-aarch64; LE unchanged. FOLLOW-UP: teach `arm64-link.c` to handle/relax the TLSDESC relocs — mcc's own arm64 linker currently errors "unhandled relocation type" on `-fPIC` TLS, so an external aarch64 ld is required until then. Mirror how `i386-link.c` relaxes i386 GD/LDM→LE.)
