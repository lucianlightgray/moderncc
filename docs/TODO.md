...
**Nine hazards, stated rather than buried.**
...
1. **The break-even lanes are pinned by hand** in `tools/loop-census.py:125` —
   ...
   integer lane source, are adjudicated against exactly that frozen region.
...
7. **`tools/fmt-census.py`'s site census is unguarded, and the claim that the tool is
...
8. **The self-compile census total is not deterministic.** Three runs of the identical
...
9. **`--alias-oracle` numbers are properties of a ceiling, not of a workload.** They now
